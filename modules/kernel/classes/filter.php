...
class Filter
{
    ...
    public function products($_offset = NULL, $_limit = NULL, $_orderby = NULL, $_desc= NULL)
    {
        ...
        $set_ids = $this->data['sets'] ? $this->data['sets'] : array(); 
    ...
    public function check_product($product_id)
    {
        ...
        {
            $catalogs = explode(',',$this->data['catalogs']);

            $cache = Cache::instance('memcache');
            $cache_key = 'product_catalogs1_' . $product_id;
            if( ! ($product_catalogs = $cache->get($cache_key)))
            {
                $product_catalogs = DB::query(Database::SELECT,"SELECT catalog_id FROM catalog_products WHERE product_id = ".$product_id)->execute()->as_array('catalog_id','catalog_id');
