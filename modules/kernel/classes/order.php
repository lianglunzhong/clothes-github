...
//			.Site::instance()->get('cc_payment_id');        
        $order->ordernum = (string) ($order->id + 1000000) . (string) (Site::instance()->get('cc_payment_id') + 1);
        $order->save();
        return $order->id;
    }
    ...
    public function clean_largesses()
    {
        ...
        // remove orderitems
        DB::delete('order_items')->where('order_id', '=', $this->data['id'])->where('is_gift', '=', '1')->execute();
    ...
    public function add_payment($payment)
    {
        ...
        return DB::insert('order_payments', array_keys($payment))
    ...
    public function add_remark($remark)
    {
        ...
        return DB::insert('order_remarks', array_keys($remark))
    ...
    public function add_history($history)
    {
        ...
        return DB::insert('order_histories', array_keys($history))
    ...
    public function products_c()
    {
        ...
        $products = DB::query(Database::SELECT,'select oi.* from order_items oi left join products p on p.id = oi.product_id where oi.order_id ='.$this->id. ' and p.visibility >0 and p.status>0 ')->execute()->as_array();
    ...
    public function add_shipment($shipment, $items)
    {
        ...
        foreach ($items as $item)
        {
            ...
            DB::update('order_items')->set(array('status' => 'shipped', 'tracking_number' => time()))->where('product_id', '=', $item['item_id'])->where('order_id', '=', $this->id)->execute();
            if (!$success)
            {
                // roll back
    ...
    public function add_item($item)
    {
        ...
        $ret = DB::insert('order_items', array_keys($item))
            ->values($item)
            ->execute();
        if (!$ret)
            return $ret;
