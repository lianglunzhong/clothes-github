...
class Shipment
{
    ...
    public function _load($id)
    {
        ...
        $data = array( );
        $result = DB::select()->from('order_shipments')
            ->where('id', '=', $id)
            ->execute()
            ->current();
    ...
    {
        $items = DB::select()->from('order_shipmentitems')->where('shipment_id', '=', $shipment_id)->execute();
        return $items;
    }
    ...
    public function update_shipment($shipment_id, $data)
    {
        ...
        $updated = DB::update('order_shipments')->set($data)->set(array('updated'=>time()))->where('id', '=', $shipment_id)->execute();
        if($updated)
        {
            $order = DB::select('order_id')->from('order_shipments')->where('id', '=', $shipment_id)->execute()->current();
            if ($order['order_id'])
            {
                $order_data = array();
