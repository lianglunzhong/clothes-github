...
class Product
{
    ...
    public function all_catalog($on_menu=0)
    {
        ...
        $cata = DB::query(Database::SELECT, 'SELECT cp.catalog_id FROM catalog_products cp LEFT JOIN catalogs ca ON (cp.catalog_id = ca.id) WHERE cp.product_id = '.$this->data['id'].' AND ca.visibility = 1 '.( ! is_null($on_menu) ? "AND ca.on_menu=$on_menu" : '').'')->execute('slave')->as_array();
        //guo add
            if(count($cata))
            {
                if(count($cata) ==1)
    ...
    public function cover_image()
    {
        if(is_array($this->data['configs']) AND isset($this->data['configs']['default_image']) AND $this->data['configs']['default_image'] != '')
        {
            ...
            {
                $image = DB::select('id', 'obj_id', 'type', 'suffix', 'status')->from('images')->where('id', '=', $image_id)->execute()->current();
                $cache->set($cache_key, $image, 21600);
            }
            if(!empty($image) AND $image['obj_id'] == $this->data['id'] AND $image['type'] == kohana::config('upload.product_image.type'))
            {
                $data['id'] = $image['id'];
    ...
    {
        $cache = Cache::instance('memcache');
        $cache_key = 'prodct_images_' . $this->data['id'];

        //添加一个小时memcache
        if(! $data = $cache->get($cache_key) )
        {
            $data = DB::query(1, "SELECT id,suffix,status FROM images WHERE type = '".kohana::config('upload.product_image.type')."' AND obj_id = '".$this->data['id']."';")->execute()->as_array();
    ...
    public function related_products()
    {
        ...
        $similar_id = DB::select('id')->from('catalogs')->where('link', '=', 'similar-styles')->execute()->get('id');
        if($similar_id)
        {
            $similar = DB::query(1, 'SELECT DISTINCT p.product_id FROM catalog_products p LEFT JOIN catalogs c ON p.catalog_id = c.id WHERE c.id = ' . $similar_id . ' ORDER BY p.position DESC')->execute();
    ...
    public function set_basic($p_data, $product_id = NULL)
    {
        ...
        $same_url_product = ORM::factory('product' . $this->lang)->where('link', '=', $data['link'])->find();
        $same_sku_product = ORM::factory('product' . $this->lang)->where('sku', '=', $data['sku'])->find();
    ...
    public function product_time()
    {
        ...
        $date_first_result = DB::query(DATABASE::SELECT, 'SELECT * FROM products ORDER BY created ASC LIMIT 0, 1')->execute();
        $date_last_result = DB::query(DATABASE::SELECT, 'SELECT * FROM products ORDER BY created DESC LIMIT 0, 1')->execute();
    ...
    public function tags()
    {
        ...
        if( ! ($data = $cache->get($key)))
        {
            $result = DB::select('tag_id')->from('tag_product')
                ->where('product_id', '=', $this->data['id'])
                ->execute();
    ...
    public function get_stocks()
    {
        if($this->data['stock'] == -1)
        {
            ...
            $stocks = DB::select()->from('product_stocks')
                ->where('product_id', '=', $this->data['id'])
                ->where('isdisplay', '=', 1)
                ->execute();
            foreach($attributes as $attribute)
            {
                ...
                {
                    if(trim($stock['attributes']) == trim($attribute) OR strpos($stock['attributes'], $attribute) !== false)
                    {
    ...
    {
        $cache = Cache::instance('memcache');
        $key = "12-22-2trans/product/".$this->data['id'].'/'.$this->lang;
