...
class Coupon
{
	...
	public function limit_dec($amount)
	{
		...
		return DB::query(Database::UPDATE, "UPDATE coupons SET `limit` = `limit` - $amount WHERE id=".$this->get('id'))
				->execute();
	}
	...
	{
		$amount = intval($amount);
		return DB::query(Database::UPDATE, "UPDATE coupons SET `used` = `used` + $amount WHERE id=".$this->get('id'))
				->execute();
	}
