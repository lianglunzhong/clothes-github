    ...
    */
    ...
    public static function delete($key)
    {
        ...
        if($customer_id = Customer::logged_in()){
            DB::delete('cartproduct')->where('customer_id', '=', $customer_id)->and_where('item_id', '=', $arr[0])->and_where('attribute', '=', $arr[1])->execute();
...
// config product
    ...
    $products = Session::instance()->get('cart_products', array());
        ...
        if(!isset($customer_id)){
            ...
            if($customer_id){
                $carproducts = DB::select()->from('cartproduct')->where('customer_id','=',$customer_id)->where('is_cart','=','1')->execute(); 

            foreach($carproducts as $k1 =>$v1){
                $v2= self::getkey($v1['item_id'].'_'.$v1['attribute']);
                $data = self::getdata($v1['item_id'].'_'.$v1['attribute']);
    ...
    public static function get($order = NULL)
    {
        ...
        if (!empty($products))
        {
            ...
            foreach ($products as $key => $product)
            {
                ...
                {
                    $isnumber = 1;
                    if(preg_match("/\d/", $product['attributes']['Size'])) //If size is shoes
                    {
                        if(strpos($product['attributes']['Size'], '-') !== FALSE)
                        {
                            $isnumber = 0;
                        }
                    ...
                    if (!empty($stock))
                    {
                        ...
                        elseif ($product['quantity'] > $stock['stocks'])
                        {
                            $products[$key]['quantity'] = $stock['stocks'];
        ...
        {
            $product = DB::select('id')->from('products')
...
//        $shipping = Carrier::instance($post['shipping_method'])->get($carrier_param);
    ...
    public static function cookie_get(){
        ...
        if($customer_id = Customer::logged_in()){
            $cookieproducts = DB::select()->from('cartproduct')->where('customer_id','=',$customer_id)->where('is_cart','=',0)->order_by('created','DESC')->execute();
            $arr = array();
            foreach ($cookieproducts as $value) {
                if (!Product::instance($value['item_id'])->get('visibility') OR !Product::instance($value['item_id'])->get('status') OR Product::instance($value['item_id'])->get('stock')==0){ 

                }else{
                    $key = self::getkey($value['item_id'].'_'.$value['attribute']);
                    $arr = self::getdata($value['item_id'].'_'.$value['attribute']);
                    $cookie_products[$key] = $arr;
                    $cookie_products[$key]['c_id'] = $value['item_id'];                
                }    
            }
        }else{
            $cookie_id = Kohana_Cookie::get('Customer_login_id');
            ...
            if ($cookie_id) {
                $cookieproducts = DB::select()->from('cartproduct')->where('customer_id','=',$cookie_id)->where('is_cart','=',0)->order_by('created','DESC')->execute();
                $arr = array();
                foreach ($cookieproducts as $value) {
                    if (!Product::instance($value['item_id'])->get('visibility') OR !Product::instance($value['item_id'])->get('status') OR Product::instance($value['item_id'])->get('stock')==0){ 
    ...
    public static function cookie2cart($key){
        ...
        $newkey = $key;//new cartproduct table
        $key = self::getkey($key);
    ...
    public static function getkey($key)
    {
        ...
        $data['id'] = $arr[0];
        $data['items'][] = $arr[0];
        $data['type'] = Product::instance($arr[0])->get('type');
    ...
    public static function getdata($key)
    {
        ...
        $brr['items'][] = $arr[0];
        $brr['quantity'] = 1;
        $brr['type'] = Product::instance($arr[0])->get('type');
        $brr['configs'] = '';
        $brr['price'] = 0;
    ...
    public static function saveforlater($newkey)
    {
        ...
        ->and_where('attribute','=',$p[1])
        ->execute()->current();
        if($cookieproducts){
            DB::update('cartproduct')->set(array('is_cart'=>0))
    ...
    public static function saveforcart($newkey)
    {
        ...
        ->execute()->current();
        if($cookieproducts){
            DB::update('cartproduct')->set(array('is_cart'=>1))
            ->where('id', '=', $cookieproducts['id'])->execute();
        }
    }
    ...
    public static function invalid($customer_id){
        $cookie_products = DB::select('item_id','id')->from('cartproduct')
        ...
        if(count($cookie_products)>0){
            foreach ($cookie_products as $cookie_product) {
                ...
                if (!$product->get('visibility') OR !$product->get('status') OR $product->get('stock')==0 ){
                    DB::delete('cartproduct')->where('id', '=', $cookie_product['id'])->execute();
                }
            }
        }
    }
