        ...
        public function set($data)
        {
                ...
                else
                {
                    ...
                    if($dataid)
                    {
                        ...
                        $upcustomer = DB::update('customers')->set($update)->where('id', '=', $data['id'])->execute();
                        if($upcustomer)
                        {
                            return $data['id'];
                        }
...
/*                $customer = ORM::factory('customer')
            ...
            //    $customer->loaded() ? $customer_id = $customer->id : $customer_id = 0;
                ...
                if($customer_id)
                {
                    ...
                    DB::update('customers')->set($update)->where('id', '=', $customer_id)->execute();
                }
                return $customer_id;
        }
        ...
        {
                $wishlists =  DB::query(Database::SELECT, 'SELECT w.id,w.product_id from wishlists w left join products p on w.product_id = p.id where w.customer_id = '.$this->data['id'] . ' and p.visibility != 0 order by w.id desc ')->execute('slave')->as_array();  
        ...
        {
                $addresses = DB::select()->from('addresses')
                        ->where('customer_id', '=', $this->data['id'])
                        ->order_by('id', 'DESC')
                        ->execute()->as_array();
        ...
        {
                $result = DB::select('id')->from('orders')
        ...
        public function point_records()
        {
                ...
                foreach( $records as &$record )
                {
                        if($record['status'] == 'pending' && $now - $record['created'] > Kohana::config('points.pending_duration', 5184000))
                        {
                                ...
                                $update = DB::update('point_records')
        ...
        public function add_point_payment($payment)
        {
                ...
                return DB::insert('point_payments', array_keys($payment))
                        ->values(array_values($payment))
                        ->execute();
        }
        ...
        public function add_point($point)
        {
                ...
                return DB::insert('point_records', array_keys($point))
        ...
        {
                return DB::query(Database::UPDATE, "UPDATE customers SET commissions=commissions+$amount WHERE id=".$this->get('id'))
                        ->execute();
        }
        ...
        {
                return DB::query(Database::UPDATE, "UPDATE customers SET points=points+$amount WHERE id=".$this->get('id'))
                        ->execute();
        }
