...
class Mail
{
    ...
    {
        $lang = DB::select('lang')->from('customers')->where('email', '=', $rcpt)->execute()->get('lang');
    ...
    public static function Sendcustomer($data, $mail_lang, $type, $from=NULL, $is_bcc=NULL, $output=FALSE, $isSMTP=false)
    {

        if( Kohana::config('webpower.'.$mail_lang.'.active.action')==1 && Kohana::config('webpower.'.$mail_lang.'.group.'.$type))
        {
            $new_type = Kohana::config('webpower.'.$mail_lang.'.group.'.$type);
            //WP-ORDER_UNPAID Mail
            ini_set('soap.wsdl_cache_enabled','0');
            $client = new SoapClient(Kohana::config('webpower.'.$mail_lang.'.admin.url'),array('encoding'=>'utf-8','features'=>SOAP_SINGLE_ELEMENT_ARRAYS));
            $login = array('username' => Kohana::config('webpower.'.$mail_lang.'.admin.username'), 'password' => Kohana::config('webpower.'.$mail_lang.'.admin.password') );
            try {
                $result = $client->addRecipients(   
                    $login,
                    Kohana::config('webpower.'.$mail_lang.'.admin.campagin'),              //campagin
                    array($new_type),             //GroupID  
                    $data,
                    true,
                    true
                );
                    ...
                    Kohana_log::instance()->add('RECIP-ERROR', $exception);
                }
        }
    }
}
