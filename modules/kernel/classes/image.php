...
class Image
{
    ...
    public static function delete_uploads($id)
    {
        $image = ORM::factory('image', $id);
        if($image->loaded())
        {
            $dir = DOCROOT.'uploads'.DIRECTORY_SEPARATOR.'1'.DIRECTORY_SEPARATOR.'pimages'.DIRECTORY_SEPARATOR;
            ...
            $dir1 = DOCROOT.'uploads'.DIRECTORY_SEPARATOR.'1'.DIRECTORY_SEPARATOR.'thumbnails';
            foreach($sizes as $i)
            {
                $file_name = $image->id."_".$i.".".$image->suffix;
                $file = $dir1.DIRECTORY_SEPARATOR.$file_name;
                if(file_exists($file))
                {
                    unlink($file);
                }   
            ...
            $dir2 = DOCROOT.'uploads'.DIRECTORY_SEPARATOR.'1'.DIRECTORY_SEPARATOR.'thumbnails1';
            foreach($sizes as $i)
            {
                $file_name = $image->id."_".$i.".".$image->suffix;
                $file = $dir2.DIRECTORY_SEPARATOR.$file_name;
                if(file_exists($file))
                {
                    unlink($file);
                }   
    ...
    {
        $sizeArr = array(
            1 => 270, 2 => 420, 3 => 75, 7 => 192, 4 => 200,10 => 800
        );
        return $sizeArr;
    }
