...
<div class="site-content">
    <div class="main-container clearfix">
        <div class="container">
                ...
                <div class="fll">
                    ...
                    foreach ($crumbs as $key => $crumb):
                        ...
                        <?php if ($key != count($catalog->crumbs()) - 1): ?>
                            &gt; <span><a href="<?php echo $crumb['link']; ?>"  ><?php echo $crumb['name']; ?></a></span>
                        <?php else: ?>
            ...
            <div <?php
                    $phonebannerarr = array('black-friday1','cyber-monday1','christmas-sale1');
            ...
            <div class="visible-xs <?php
                    $phonebannerarr = array('black-friday11','cyber-monday','boho-chic');
            ?>">
                <p class="mb25 img-active">
                    <img src="<?php echo STATICURL;?>/simages/<?php echo $pimage_src; ?>" alt="" usemap="#<?php echo $map; ?>" >
                </p>        
                <?php
                if ($map)
                {
            ...
            ?>
                ...
                <div class="font18 mt20 ml20 hide" id="no_result">Sorry, no results found.</div>
                    ...
                    for ($i = 0; $i < $_limit; $i++)
                    {
                        ...
                        <li class="pro-item col-xs-6 col-sm-3">
                            ...
                            <?php if ($product_inf['type'] != 0): ?>
                                <a  id="<?php echo $product_id; ?>" class="btn-qv quick_view"  data-reveal-id="myModal">Quick View</a>
                            <?php endif; ?>
                            <div class="add-wish">
                            <?php if(!$customer_id = Customer::logged_in()){ ?>
                            <div class="add_to_wishlist" data-product="<?php echo $product_id; ?>">
                                <a class="wish-title" data-reveal-id="myModal2" id="wish1_<?php echo $product_id; ?>">Add to wishlist
                                <i class="fa fa-heart add_wishlist" id="wish_<?php echo $product_id; ?>"></i></a>
                            </div>
                            <?php }else{ ?>
                            <div class="add_to_wishlist" data-product="<?php echo $product_id; ?>">
                                <a class="wish-title" id="wish1_<?php echo $product_id; ?>">Add to wishlist
                                <i class="fa fa-heart add_wishlist" id="wish_<?php echo $product_id; ?>"></i></a>
                            </div>
                            <?php } ?>
                            </div>
            ...
            {
                ?>
                <div class="font18 mt20">Sorry, no results found.</div>
                <?php
            }
        }
        ?>
            ...
            success: function(product){
                ...
                $.each(product,function(i,pdata){
                    ...
                    <li class="pro-item col-xs-6 col-sm-3">\
                        ...
                        <a id="' + pdata['product_id'] + '" class="btn-qv quick_view" attr-lang="<?php echo LANGUAGE; ?>" data-reveal-id="myModal">Quick View</a>';
                        product_li +=  
                        '<div class="add-wish">';
                        <?php if(!$customer_id = Customer::logged_in()){ ?>
                        product_li += '<div class="add_to_wishlist" data-product="' + pdata['product_id'] + '">\
                            <a class="wish-title" data-reveal-id="myModal2" id="wish1_' + pdata['product_id'] + '">Add to wishlist <i class="fa fa-heart add_wishlist" id="wish_' + pdata['product_id'] + '"></i></a></div>';

                        <?php }else{ ?>
                        product_li +=  
                            '<div class="add_to_wishlist" data-product="' + pdata['product_id'] + '">\
                                <a class="wish-title" id="wish1_' + pdata['product_id'] + '">Add to wishlist \
                ...
                success:function(rs){
                    if(rs.success == 1)
                    {
                        ...
                        var str="";
                         ...
                         str +="<span>Hello, "+rs.firstname+"!</span>";
                         str +="</div>";
                ...
                for(size in filters['size'])
                {
                    if(filters['size'][size] > 0)
                    {
                        ...
                        if(typeof filters['size_title'][size] != 'undefined')
                            size_value = filters['size_title'][size];
                ...
                for(filter_type in current_filter)
                {
                    if( typeof current_filter[filter_type] != 'undefined')
                    {
                        ...
                        bar_html += '<li class="item-l" id="' + bar_item_id + '" data-value="' + current_filter[filter_type]['value'] + '">\
                            <a href="#"><i class="fa fa-close"></i><span>' + current_filter[filter_type]['title'] + '</span></a>\
                        </li>';
...
<script type="text/javascript">
    ...
    if(isset($_limit))
    {
        for ($i = 0; $i < $_limit; $i++)
        {
            ...
            $cover_image = Product::instance($product_id)->cover_image();
            $product_inf = Product::instance($product_id)->get();
            $product_catalog = Product::instance($product_id)->default_catalog();
            $cataname = Catalog::instance($product_catalog)->get("name");
            $search = array('product_id' => $product_id);
            $plink = Product::instance($product_id)->permalink(); ?>
         
            ga('ec:addImpression', {
            'id': '<?php echo $product_inf["sku"]; ?>', 
            'name': "<?php echo $product_inf["name"]; ?>",
            'category': "<?php echo $cataname; ?>",  
            'brand': 'Choies',
            'variant': '<?php echo $product_inf["filter_attributes"]; ?>', 
            'list': '<?php echo $name; ?>',    
...
/*        window.onresize = function () {  
        ...
        function adjustPosition(position) {   
            ...
            <?php if($catalog_link === 'summer-pretty-sale' || $catalog_link === 'happy-thanksgiving-sale'){ ?> 
            var imageWidth =  1200;   
            var imageHeight = 500;  
         <?php   }    ?>
