	...
	<div class="container">
		<div class="crumbs">
			<div>
				...
				<a href="<?php echo LANGPATH; ?>/customer/summary" class="visible-xs-inline hidden-sm hidden-md hidden-lg"> > KONTOÜBERSICHT</a> > Bestellhistorie
			</div>
		</div>
    ...
    ?>
               ...
               <?php
                ...
                ?>
                                ...
                                <?php if ($outstock): ?><p class="red">(Ausverkauft)</p><?php endif; ?>
                                <p>
                                    <?php
					...
					if($order['refund_status'])
						{
                            ...
                            if($status == 'prepare refund'){
                               $status = 'Erstattung Vorbereiten';
                            }elseif($status == 'partial refund'){
                               $status = 'Teilweise Zurückerstattet';
                            }else{
            ...
            ?>
            <a href="<?php echo LANGPATH; ?>/order/view/<?php echo $order['ordernum']; ?>" class="a-underline mb10">Bestelldetails</a>     
			<?php }elseif($order['payment_status'] == 'cancel'){ ?>
			 <a href="<?php echo LANGPATH; ?>/order/view/<?php echo $order['ordernum']; ?>" class="a-underline mb10">Bestelldetails</a>
            <?php }elseif (!$order['refund_status'] AND $amount > 0 AND $order['payment_status'] == 'new'){ ?>
            <a href="<?php echo LANGPATH; ?>/order/view/<?php echo $order['ordernum']; ?>" class="btn btn-primary btn-sm mb10">BEZAHLEN</a>
            <?php }else{ ?>
            <a href="<?php echo LANGPATH; ?>/order/view/<?php echo $order['ordernum']; ?>" class="a-underline mb10">Bestelldetails</a>
            <?php } ?>
            <?php if($order['shipping_status'] == 'shipped' OR $order['shipping_status'] == 'partial_shipped'){ ?>
                    ...
                    if ($order['payment_status'] == 'success' OR $order['payment_status'] == 'verify_pass')
                    {
                        if($p['erp_line_status'] == 1)
                        {
                            ...
                            {
                            ?>
                            <a href="<?php echo $plink; ?>#review-list" class="a-underline">Rezensiert</a>
                            <?php
                            }
                            else
                            {
                        ...
                        {
                        ?>
                            <a href="<?php echo LANGPATH; ?>/review/add/<?php echo $p['product_id']; ?>" class="a-underline review-link">Rezensieren</a>
                        <?php
                        }
                    }
                    ?>
			<span class="JS_shows_btn1 trackjs a-underline" id="<?php echo $order['ordernum']; ?>"><a href="<?php echo LANGPATH; ?>/tracks/customer_track?id=<?php echo $order['ordernum']; ?>">Bestellung Verfolgen</a>

	            			<div class="JS_shows1 track-order-hidecon hide">
		                		<p>Verfolgung Informationen wie folgt sind kürzlich aktualisiert, <a href="<?php echo LANGPATH; ?>/tracks/customer_track?id=<?php echo $order['ordernum']; ?>" class="red">klicken Sie bitte hier</a>, um alle Details zu prüfen. </p>
					...
					if (data.result == "noData") {
						...
						for (var i = 0; i < item.length; i++) {
							if (typeof(item[i]['history']) != "undefined") {
								$("#history" + code).append("<li class=\"first\"><span class=\"btn-black-sm\">Paket" + (i + 1) + "</span></li>");
