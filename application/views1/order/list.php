	...
	<div class="container">
		<div class="crumbs">
			<div>
				...
				<a href="<?php echo LANGPATH; ?>/customer/summary" class="visible-xs-inline hidden-sm hidden-md hidden-lg"> > my account</a> > order history
			</div>
		</div>
...
<?php echo View::factory('customer/left'); ?>
    ...
    ?>
            ...
            ?>
            <a href="<?php echo LANGPATH; ?>/order/view/<?php echo $order['ordernum']; ?>" class="a-underline mb10">Order Details</a>     
			<?php }elseif($order['payment_status'] == 'cancel'){ ?>
			 <a href="<?php echo LANGPATH; ?>/order/view/<?php echo $order['ordernum']; ?>" class="a-underline mb10">Order Details</a>
            <?php }elseif (!$order['refund_status'] AND $amount > 0 AND $order['payment_status'] == 'new'){ ?>
            <a href="<?php echo LANGPATH; ?>/order/view/<?php echo $order['ordernum']; ?>" class="btn btn-primary btn-sm mb10">To pay</a>
            <?php }else{ ?>
            <a href="<?php echo LANGPATH; ?>/order/view/<?php echo $order['ordernum']; ?>" class="a-underline mb10">Order Details</a>
            <?php } ?>
            <?php if($order['shipping_status'] == 'shipped' OR $order['shipping_status'] == 'partial_shipped'){ ?>
                    ...
                    if ($order['payment_status'] == 'success' OR $order['payment_status'] == 'verify_pass')
                    {
                        if($p['erp_line_status'] == 1)
                        {
                            ...
                            {
                            ?>
                            <a href="<?php echo $plink; ?>#review-list" class="a-underline">Reviewed</a>
                            <?php
                            }
                            else
                            {
                        ...
                        {
                        ?>
                            <a href="/review/add/<?php echo $p['product_id']; ?>" class="a-underline review-link">Review</a>
                        <?php
                        }
                    }
                    ?>
			<span class="JS_shows_btn1 trackjs a-underline" id="<?php echo $order['ordernum']; ?>"><a href="/tracks/customer_track?id=<?php echo $order['ordernum']; ?>">Track Order</a>

	            			<div class="JS_shows1 track-order-hidecon hide">
		                		<p>Tracking Details as follows are recently updated, please<a href="/tracks/customer_track?id=<?php echo $order['ordernum']; ?>" class="red">click here</a> to check all detals </p>
