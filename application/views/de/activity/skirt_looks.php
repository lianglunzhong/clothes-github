...
<section id="main">
	...
	<img src="/images/activity/skirt_looks/de_skirt_look.jpg" border="0" usemap="#Map" />
            <map name="Map" id="Map">
<area shape="rect" coords="0,0,510,299" href="http://www.choies.com/de/skirt?sort=1" />
<area shape="rect" coords="515,1,1023,298" href="http://www.choies.com/de/skirt?sort=2" />
            </map>
    </div>
    ...
    <div class="layout">
        <ul class="skirt">
            ...
            <li>
                ...
                <div class="ski"><a class="vi" target="_blank" href="<?php echo $value['product_url'];?>">Details Sehen >></a><span class="ge JS_popwinbtn4" title="<?php echo $value['link_sku'];?>" onclick="load_data('<?php echo $value['link_sku'];?>')" >Kollokation</span></div>
            </li>
            <?php } ?>
        </ul>
...
function load_data(skus){
    $.post(
        '/activity/ajax_skirt_looks?lang=de',
        {
            skus:skus,
        },
        function (data) {
            ...
            for (var i = item.length - 1; i >= 0; i--) {
                ...
                str += '<li><input type="checkbox" name="check['+item[i]['n']+']" title="size'+item[i]['n']+'" class="checkbox" checked="checked" id="checkout'+item[i]['product_id']+'" /> <label for="checkout'+item[i]['product_id']+'">IN DEN WARENKORB</label><input type="hidden" name="item['+item[i]['n']+']" value="'+item[i]['product_id']+'" /><a href="'+item[i]['sku_link']+'"  target="_blank"><img src="'+item[i]['img_src']+'" /></a><a href="'+item[i]['sku_link']+'" class="name">'+item[i]['product_name']+'</a><p class="price">'+price+'</p>'+item[i]['options']+'<p class="center"><a href="'+item[i]['sku_link']+'" class="btn22_black" target="_blank">Details</a></p></li>';
            };
    ...
    <div class="look_pro">
        <form action="<?php echo LANGPATH; ?>/cart/add_more" method="post" class="form3" id="form">
            ...
            <div class="center mt50">
                <input type="submit" value="IN DEN WARENKORB" class="btn40_16_red" /><a href="" class="a_underline add_wishlist">MEINE WUNSCHLISTE</a>
            </div>
        </form>
        <script>
            $("#form").validate({
