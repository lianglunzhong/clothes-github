...
<div class="m-banner">
  <div class="banner-list">
    <ul class="fix">
        <li><a href="/mobile/catalog/pastel-hues"><img src="/images/mobile/banner4.jpg" alt="" /></a></li>
    	<li><a href="/mobile/catalog/shoes"><img src="/images/mobile/banner3.jpg" alt="" /></a></li>
    </ul>
  </div>
...
<nav id="shoes" class="main-navigation fam reveal-modal">
	...
	<ul>
		...
		<?php
        $catalog1 = DB::select('id')->from('catalogs')->where('link', '=', 'shoes')->execute()->current();
        $apparels = Catalog::instance($catalog1['id'])->children();
       	foreach ($apparels as $catalog):
        	?>
      		<li><a href="<?php echo LANGPATH; ?>/mobile/catalog/<?php echo Catalog::instance($catalog)->get('link'); ?>"><?php echo ucfirst(Catalog::instance($catalog)->get('name')); ?></a></li>
            <?php
       endforeach;  ?>
	</ul>
</nav>
...
<nav id="apparels" class="main-navigation fam reveal-modal">
	...
	<ul>
		...
		<?php
        $catalog1 = DB::select('id')->from('catalogs')->where('link', '=', 'apparels')->execute()->current();
        $apparels = Catalog::instance($catalog1['id'])->children();
        foreach ($apparels as $catalog):
          	?>
            <li><a href="<?php echo LANGPATH; ?>/mobile/catalog/<?php echo Catalog::instance($catalog)->get('link'); ?>"><?php echo ucfirst(Catalog::instance($catalog)->get('name')); ?></a></li>
...
<nav id="occasion-dresses" class="main-navigation fam reveal-modal">
	...
	<ul>
		<?php
     	$catalog2 = DB::select('id')->from('catalogs')->where('link', '=', 'occasion-dresses')->execute()->current();
...
<nav id="accessory" class="main-navigation fam reveal-modal">
	...
	<ul>
		...
		<?php
        $catalog3 = DB::select('id')->from('catalogs')->where('link', '=', 'accessory')->execute()->current();
...
<nav id="mans" class="main-navigation fam reveal-modal">
	...
	<ul>
		...
		<?php
        $catalog2 = DB::select('id')->from('catalogs')->where('link', '=', 'men-s-collection')->execute()->current();
       $dresses = Catalog::instance($catalog2['id'])->children();
        foreach ($dresses as $catalog):
           	?>
            <li><a href="<?php echo LANGPATH; ?>/mobile/catalog/<?php echo Catalog::instance($catalog)->get('link'); ?>"><?php echo ucfirst(Catalog::instance($catalog)->get('name')); ?></a></li>
...
<nav id="brands" class="main-navigation fam reveal-modal">
	...
	<ul>
		<?php
     	$brands = DB::select('name', 'link')->from('catalogs')->where('is_brand', '=', 1)->where('visibility', '=', 1)->where('on_menu', '=', 0)
...
<nav id="outlet" class="main-navigation fam reveal-modal">
	...
	<ul>
		...
		<?php
        $catalog4 = DB::select('id')->from('catalogs')->where('link', '=', 'outlet')->execute()->current();
