<ul class="filter-bar cf">
    ...
    if ($catalog_id == 92 OR $parent_id == 92)
    {
        $childrens = array(
            ...
            array('Black Dresses', '/dresses-c-92/all/all/color_8'),
            array('Lace Dresses', '/dresses-c-92/all/all/Detail_124'),
            array('Bodycon Dresses', '/dresses-c-92/all/all/Silhouette_62'),
            array('Maxi Dresses', '/dresses-c-92/all/all/dresses-c-92-Length_73'),
            array('Skater Dresses', '/dresses-c-92/all/all/Silhouette_69'),
            array('Floral Dresses', '/dresses-c-92/all/all/Pattern-Type_82'),
            array('Chiffon Dresses', '/dresses-c-92/all/all/Material_19'),
            array('Long Sleeve Dresses', '/dresses-c-92/all/all/Sleeve-Length_56'),
            array('Party Dresses', '/dresses-c-92/all/all/Shop-Looks_35'),
            array('Homecoming Dress', '/homecoming-dress-c-454'),
        );
    }
    else
    {
    ...
    <li class="item-l choice" style="width:120px;">
        <div class="choice-hd">
            <?php echo isset($_GET['sort']) ? $sort_by[$_GET['sort']]['name'] : 'Default'; ?>
...
?>
    <li id="JS_filterAll" class="item-r filter-all">
        All Filters <i class="fa fa-caret-down"></i>
    </li>
</ul>
...
<div class="filter-list" style="display:none;">
    ...
    <div class="item choice">
        ...
        if ($colors)
        {
            ...
            foreach ($colorArr as $color)
            {
                $href2 = array();
                $do_filters = $filters;
                $do_filters['color'] = strtolower($color);
                ...
                <li class="choice-option <?php if($val == $current_color) echo 'on'; ?>">
                    ...
                    <a href="<?php echo LANGPATH . $href1 . implode('__', $href2) . $limit_link; ?>" title="<?php echo ucfirst($color); ?>">
    ...
    if (!empty($sorts))
    {
        ...
        ?>
            ...
            <div class="item choice">
                <div class="choice-hd">
                    <?php echo ucfirst($sort_name); ?> <i class="fa fa-chevron-down"></i>
                </div>
                <?php
                $f_image = '';
                    ...
                    foreach ($attributes as $attr)
                    {
                        ...
                        <li class="choice-option">
                            <a href="<?php echo LANGPATH . $href1 . implode('__', $href2) . $limit_link; ?>">
                                ...
                                if ($f_image)
                                {
                                    echo '<img src="' . $f_image . '/' . str_replace(' ', '-', strtolower($attr)) . '.png" title="' . ucfirst($attr) . '" alt="' . ucfirst($attr) . '" />';
                                }
                                else
                                    echo ucfirst($attr);
                                ?>
...
if (!empty($custom_filter['filter_attirbutes']) OR !empty($custom_filter['price_range']) OR $has_size):
    ...
    <div class="filter-selected">
        ...
        ?>
        <span class="item">
            <a href="<?php echo LANGPATH . $uris[0] . '/' . $uris[1]; ?>"><span class="clearall">Clear All</span></a>
        </span>
    </div>
    <?php
endif;
