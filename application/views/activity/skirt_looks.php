...
<section id="main">
	...
	<img src="/images/activity/skirt_looks/en_skirt_look.jpg" border="0" usemap="#Map" />
            <map name="Map" id="Map">
<area shape="rect" coords="1,2,511,304" href="http://www.choies.com/skirt?sort=1" />
<area shape="rect" coords="513,3,1022,298" href="http://www.choies.com/skirt?sort=2" />
            </map>
    </div>
    ...
    <div class="layout">
        <ul class="skirt">
            ...
            <li>
                ...
                <div class="ski"><a class="vi" target="_blank" href="<?php echo $value['product_url'];?>">View Detail >></a><span class="ge JS_popwinbtn4" title="<?php echo $value['link_sku'];?>" onclick="load_data('<?php echo $value['link_sku'];?>')" >Get the Look </span></div>
            </li>
            <?php } ?>
        </ul>
    ...
    $.post(
        ...
        function (data) {
            ...
            for (var i = item.length - 1; i >= 0; i--) {
                ...
                str += '<li><input type="checkbox" name="check['+item[i]['n']+']" title="size'+item[i]['n']+'" class="checkbox" checked="checked" id="checkout'+item[i]['product_id']+'" /> <label for="checkout'+item[i]['product_id']+'">Add to Bag</label><input type="hidden" name="item['+item[i]['n']+']" value="'+item[i]['product_id']+'" /><a href="'+item[i]['sku_link']+'" target="_blank"><img src="'+item[i]['img_src']+'" /></a><a href="'+item[i]['sku_link']+'" class="name">'+item[i]['product_name']+'</a><p class="price">'+price+'</p>'+item[i]['options']+'<p class="center"><a href="'+item[i]['sku_link']+'" class="btn22_black" target="_blank">View Full Details</a></p></li>';
            };
