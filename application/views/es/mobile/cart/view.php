...
$cpromotions = DB::select()->from('cpromotions')
...
foreach ($cart['products'] as $key => $product):
	...
	<div class="product row">
		...
		<div class="mobile-three">
			...
			<span class="style-number">artículo# : <?php echo Product::instance($product['id'])->get('sku'); ?></span>
			<div class="rship"></div>
			<ul>
			<?php
    ...
    <?php if (isset($cart['largesses'])){
        ...
        <div class="product row">
            ...
            <div class="mobile-three">
                ...
                <span class="style-number">artículo# : <?php echo Product::instance($largess['id'])->get('sku'); ?></span>
                <div class="rship"></div>
                <ul>
                    <?php
    ...
    <?php if (!empty($cart['largesses_for_choosing'])): ?>
        <?php foreach ($cart['largesses_for_choosing'] as $largesses_for_choosing): ?>
            ...
            foreach ($largesses_for_choosing['largesses'] as $key => $largesses_for_choosing_product):
                ...
                if($stock == -1)
                {
                    $stocks = DB::select(DB::expr('SUM(stocks) AS sum'))->from('product_stocks')->where('product_id', '=', $key)->execute()->get('sum');
                    if(!$stocks)
                        continue;
                }
                $num++;
                ...
                <form method="POST" action="/mobilecart/largess_add">
                    <div class="product row"> 
                            ...
                            <div class="mobile-three">
                                        ...
                                        <span class="style-number">artículo# : <?php echo Product::instance($key)->get('sku'); ?></span>
                            <ul>
                                <?php
