<ul class="filter-bar cf">
    ...
    $sorts = DB::select('sort', 'attributes')
            ...
            ->order_by('sort')
            ->execute()->as_array();
    $parent_id = DB::select('parent_id')->from('catalogs')->where('id', '=', $catalog_id)->execute()->get('parent_id');
    if (empty($sorts))
    {
        $sorts = DB::select('sort', 'attributes')
                ->from('catalog_sorts')
    ...
    if ($catalog_id == 92 OR $parent_id == 92)
    {
        $childrens = array(
            ...
            array('Vestidos Negros', LANGPATH . '/dresses-c-92/all/all/color_8'),
            array('Vestidos De Encaje', LANGPATH . '/dresses-c-92/all/all/Detail_124'),
            array('Vestidos Ajustados', LANGPATH . '/dresses-c-92/all/all/Silhouette_62'),
            array('Vestidos Maxi', LANGPATH . '/dresses-c-92/all/all/dresses-c-92-Length_73'),
            array('Vestidos Skater', LANGPATH . '/dresses-c-92/all/all/Silhouette_69'),
            array('Vestidos Florales', LANGPATH . '/dresses-c-92/all/all/Pattern-Type_82'),
            array('Vestidos De Gasa', LANGPATH . '/dresses-c-92/all/all/Material_19'),
            array('Vestidos De Mangas Largas', LANGPATH . '/dresses-c-92/all/all/Sleeve-Length_56'),
            array('Vestidos De Fiesta', LANGPATH . '/dresses-c-92/all/all/Shop-Looks_35'),
            array('Vestido por Fiesta', LANGPATH . '/homecoming-dress-c-454'),
        );
    }
    else
    {
    ...
    $scroll = 1;
    ?>
    <li style="float:left;margin-top: 5px;"><div>Ordenar por:&nbsp;</div></li>
    <li class="item-l choice" style="width:155px;">
        <div class="choice-hd">
            <?php
    ...
    <li class="item-l pick" style="width:auto;">
        ...
        if (!empty($gets1))
        {
            ...
            $gets1[] = 'pick=1';
        ?>
        <a href="<?php echo $_SERVER['REDIRECT_URL'] . '?' . implode('&', $gets1); ?>" class=""> <i class="icon icon-pick"></i> Elección de Celebridad</a>
    </li>
...
if(!empty($size_filter))
{
    ...
    <li class="item-r choice">
        <div class="choice-hd">
            Talla <i class="fa fa-caret-down"></i>
        </div>
...
?>
    <li id="JS_filterAll" class="item-r filter-all">
        Todos los filtros <i class="fa fa-caret-down"></i>
    </li>
</ul>
...
<div class="filter-list" style="display:none;">
    ...
    ?>
        <div class="item choice">
            <div class="choice-hd">
                Categoría <i class="fa fa-chevron-down"></i>
            </div>
    ...
    if(!empty($size_filter))
    {
        ...
        <div class="item choice">
            <div class="choice-hd">
                Talla <i class="fa fa-chevron-down"></i>
            </div>
    ...
    <div class="item choice">
        <div class="choice-hd">
            Precio <i class="fa fa-chevron-down"></i>
        </div>
...
if (!empty($custom_filter['filter_attirbutes']) OR !empty($custom_filter['price_range']) OR $has_size):
    ...
    <div class="filter-selected">
        ...
        if($has_size)
        {
            ...
            ?>
            <span class="item">
                <a href="<?php echo $href . $limit_link; ?>"><i class="fa fa-times-circle fa-lg"></i><?php echo 'Talla:'.$size_filter[$links[1]]; ?></a>
            </span>
            <?php
        }
        if (isset($custom_filter['price_range']))
        {
            ...
            ?>
            <span class="item">
                <a href="<?php echo $href . $limit_link; ?>"><i class="fa fa-times-circle fa-lg"></i><?php echo 'Precio:$' . $custom_filter['price_range'][0] . ' ~ $' . $custom_filter['price_range'][1]; ?></a>
            </span>
            <?php
        }
        ...
        ?>
        <span class="item">
            <a href="<?php echo LANGPATH . $uris[0] . '/' . $uris[1]; ?>"><span class="clearall">Borrar todo</span></a>
        </span>
    </div>
    <?php
endif;
