<link rel="canonical" href="/<?php $catalog_link = $catalog->get('link');
echo $catalog_link;
?>" />
...
<section id="main">
    ...
    <div class="w_index_free">
        <div class="layout index_free">
            <a href="<?php echo LANGPATH; ?>/shipping-delivery" class="a1">ENVÍO GRATIS MUNDIAL</a>
            <?php 
            $user_id = Customer::logged_in();
            if($user_id){?>
                <a href="<?php echo LANGPATH; ?>/vip-policy" class="a2">-15% DEL PRIMERO PEDIDO</a>
            <?php }else{ ?>
                <a href="<?php echo LANGPATH; ?>/customer/login" class="a2">-15% DEL PRIMERO PEDIDO</a>
            <?php } ?>
            <a href="<?php echo LANGPATH; ?>/daily-new" class="a3">-10% DE NOVEDADES</a>
        </div>
    </div>
    ...
    <section class="layout fix">
        <section id="container" class="flr">
            ...
            if ($show)
            {
                ...
                <ul class="JS_tab3 detail_tab hotnew_box fix" style="margin-top:0;">
                    <?php
                    if ($show == 1)
                        echo '<li class="on">LOS MÁS CANDENTES </li>';
                    elseif ($show == 2)
                        echo '<li class="on">NUEVAS LLEGADAS</li>';
                    elseif ($show == 3)
                        echo '<li class="on">LOS MÁS CANDENTES </li><li>NUEVAS LLEGADAS</li>';
                    ?>
                </ul>
                        ...
                        <div class="bd no2">
                            <div class="JS_carousel5 pro_listcon product_carousel">
                                <ul class="fix">
                                    ...
                                    {
                                        $new_product = Product::instance($new);
                                        ?>
                                        <li>
                ...
                <article class="filter_page">
                    <ul class="select fix">
                        ...
                        if (!empty($gets1))
                        {
                            ...
                            $gets1[] = 'pick=1';
                        ?>
                        <span><a class="icon_pick" href="<?php echo $_SERVER['REDIRECT_URL'] . '?' . implode('&', $gets1); ?>" title="Elección de Celebridad"></a></span>
                        <li class="last">
                            <?php
                            echo $pagination;
                            ?>
                    ...
                    <div class="results_box fix">
                        ...
                        if (!empty($custom_filter)):
                            ...
                            <a href="<?php echo LANGPATH . $uris[0] . '/' . $uris[1] . '/' . $uris[2]; ?>"><span class="clearall flr">Borrar Todo</span></a>
                            <?php
                        endif;
                        ?>
                    </div>
                        ...
                        for ($i = 0; $i < $_limit; $i++):
                            ...
                            <?php
                                ...
                                ?>
                                    <div class="pic">
                                        <a href="<?php echo Product::instance($product_id, LANGUAGE)->permalink(); ?>">
                                            <div class="pic1"><img data-original="http://img3.choies.com/pimg/192/<?php echo $images[0]['id'].'.'.$images[0]['suffix']; ?>" src="/images/loading.gif" alt="<?php echo 'Fashion ' . $catalog->get('name'); ?>" title="<?php echo $product_inf['name']; ?>" /></div>
                                            <?php
                                            if (isset($images[1]))
                                            {
                                                ?>
                                                <div class="pic2" style="display:none;"><img data-original="http://img2.choies.com/pimg/192/<?php echo $images[1]['id'].'.'.$images[1]['suffix']; ?>" src="/images/loading.gif" alt="<?php echo 'Fashion ' . $catalog->get('name'); ?>" title="<?php echo $product_inf['name']; ?>" /></div>
                                                <?php
                                            }
                                            ?>
                                        </a>
                                ...
                                else
                                {
                                    ...
                                    <div class="pic">
                                        <a href="<?php echo Product::instance($product_id, LANGUAGE)->permalink(); ?>">
                                            ...
                                            {
                                                ?>
                                                <div class="pic2" style="display:none;"><img src="http://img3.choies.com/pimg/192/<?php echo $images[1]['id'].'.'.$images[1]['suffix']; ?>" alt="<?php echo 'Fashion ' . $catalog->get('name'); ?>" title="<?php echo $product_inf['name']; ?>" /></div>
                                                <?php
                                            }
                                            ?>
                                        </a>
                                ...
                                <?php if ($product_inf['type'] != 0): ?>
                                    <a href="#" id="<?php echo $product_id; ?>" class="quick_view JS_shows1 hide">VISTA RÁPIDA</a>
                                <?php endif; ?>
                                ...
                                <p class="price fix">
                                    ...
                                    if ($orig_price > $price)
                                    {
                                        ...
                                        <br/><span class="off"><?php echo $off; ?>% MENOS</span>
                                        <?php
                                    }
                                    else
                                    {
            ...
            <article class="filter_page">
                <ul class="select fix">
                    <li><a class="icon_pick" href="<?php echo $_SERVER['REDIRECT_URL'] . '?' . implode('&', $gets); ?>">Elección de Celebridad</a></li>
                    <li class="last">
                        <?php
                        echo $pagination;
                        ?>
        ...
        <!-- aside -->
        <?php
        $occasion_dresses = DB::select('id')->from('catalogs')->where('name', '=', 'Occasion Dresses')->execute()->get('id');
        if (in_array($catalog_link, array('weekly-new', 'daily-new')) OR $crumbs[0]['id'] == $occasion_dresses)
        {
            echo View::factory('/es/catalog_left');
        }
        else
        {
            echo View::factory('/es/catalog_left_1')
...
<div class="mybag hide" id="mybag" style="position: fixed;top: 10px;right: 140px; z-index:100px;z-index: 1000; background: #FFFFFF; border-color: #F6F6F7; border-style: solid;border-width: 8px 4px 4px;">
    <div class="add_tit" style="margin-top:0px;">¡EXITO! ARTÍCULO AÑADIDO A BOLSA</div>
        ...
        $("#formAdd").submit(function(){
            $.post(
                ...
                function(product)
                {
                    ...
                    cart_view = cart_view.replace('one size', 'talla única');
...
if ($limited == 1)
{
    ...
    for ($i = $limit, $j = 0; $i < count($products); $i++, $j++)
    {
        ...
        {
            $stocks = DB::select(DB::expr('SUM(stocks) AS sum'))->from('product_stocks')->where('product_id', '=', $product_id)->execute()->get('sum');
            if (!$stocks)
            {
