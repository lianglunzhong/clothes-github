<aside id="aside" class="fll">
    ...
    {
        $banners = DB::select()->from('banners')->where('visibility', '=', 0)->and_where('alt', '=', 'List')->and_where('lang', '=', LANGUAGE)->execute()->as_array();
    ...
    {
        $catalog_id = DB::select('id')->from('catalogs')->where('link', '=', $catalog_link)->execute()->get('id');
        if (!$catalog_id)
        {
            $catalog_id = 40;
        }
    }
    $crumbs = Catalog::instance($catalog_id, LANGUAGE)->crumbs();
    $catalogs = ORM::factory('catalog' . LANGUAGE)
                ...
                if (!empty($children))
                {
                    ...
                    <ul class="menu JS_menu <?php echo!$on ? ' hide' : ''; ?>">
                        <?php if ($catalog->link != 'occasion-dresses'): ?>
                            <li><a <?php if ($on AND isset($links[1]) AND $links[1] == 'new') echo 'style="color:#F66;"' ?> href="<?php echo LANGPATH; ?>/<?php echo $catalog->link; ?>/new">Novedades</a></li>
                        <?php endif; ?>
                        <?php foreach ($children as $sub_catalog): ?>
                            <li><a <?php if ($on AND $catalog_link == Catalog::instance($sub_catalog)->get('link')) echo 'style="color:#F66;"' ?> href="<?php echo Catalog::instance($sub_catalog, LANGUAGE)->permalink(); ?>"><?php echo Catalog::instance($sub_catalog, LANGUAGE)->get('name'); ?></a></li>
                        <?php endforeach; ?>
                    </ul>
                    <?php
                }
    ...
    <div class="category_box">
        <h3 class="line">MARCAS</h3>
        <div class="leftlist JS_leftlist">
            <?php
            $brands = ORM::factory('catalog' . LANGUAGE)
                ->where('site_id', '=', Site::instance()->get('id'))
                ->where('visibility', '=', 1)
                ->where('on_menu', '=', 0)
                ->where('parent_id', '=', 0)
