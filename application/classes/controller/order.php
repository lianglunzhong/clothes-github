...
class Controller_Order extends Controller_Webpage
{
    ...
    public function action_view($ordernum = NULL)
    {
        ...
        if ($_POST)
        {
            ...
            if ($delete_product)
            {
                foreach ($order->products() as $product)
                {
                    ...
                    {
                        DB::delete('order_items')->where('id', '=', $product['id'])->execute();
                    }
                }
            ...
            DB::update('orders')->set(array('payment_method' => $payment_method, 'is_pre_order' => 1))->where('ordernum', '=', $ordernum)->execute();
            ...
            {
                $orderData = DB::select()->from('orders')->where('ordernum', '=', $ordernum)->execute()->current();
                if(!$orderData['billing_address'])
                {
                    ...
                    DB::update('orders')->set($billing_address)->where('id', '=', $orderData['id'])->execute();
                }
        ...
        if ($order->get('id'))
        {
            $order_message = DB::select('message')->from('order_messages')->where('order_id', '=', $order->get('id'))->execute()->get('message');
            ...
            $this->template->content = View::factory(LANGPATH . '/order/detail')
    ...
    public function action_paynow($ordernum)
    {
        ...
        if ($order->get('id') AND in_array($order->get('payment_status'), array('new', 'partial_paid', 'failed')))
        {
            ...
            $template = View::factory(LANGPATH . '/order_paynow')
...
// save customer shipping address
        ...
        else
        {
            ...
            {
                DB::update('addresses')->set(array('cpf' => $cart['shipping_address']['shipping_cpf']))
...
// set amount
        ...
        if($cart['amount']['items'] >= 79 && !in_array($cart['shipping_address']['shipping_country'], $no_express_countries))
        {
...
// set product
            ...
            foreach ($cart['products'] as $key => $value) {
                DB::delete('cartproduct')->where('customer_id', '=', $customer_id)->and_where('item_id', '=', $value['id'])->and_where('attribute', '=', $value['attributes']['Size'])->execute();
            }
            ...
            if(strlen($cart_message) > 0)
            {
                ...
                DB::insert('order_messages', array_keys($array))->values($array)->execute();
            }
        ...
        {
            DB::delete('orders')->where('id', '=', $order_id)->where('email', 'IS', NULL)->execute();
    ...
    public function action_edit_address()
    {
        ...
        if(!$customer_id)
            $this->request->redirect(LANGPATH . '/customer/orders');
        $order_id = DB::select('id')->from('orders')->where('customer_id', '=', $customer_id)->where('id', '=', $id)->execute()->get('id');
        if(!$order_id)
            $this->request->redirect(LANGPATH . '/customer/orders');
        if($_POST)
        {
            ...
            if($type == 'shipping')
            {
                ...
                DB::update('orders')->set($shipping_address)->where('id', '=', $order_id)->execute();
            ...
            elseif($type == 'billing')
            {
                ...
                DB::update('orders')->set($billing_address)->where('id', '=', $order_id)->execute();
                $return = Arr::get($_POST, 'return', LANGPATH . '/orders/view/' . Order::instance()->get('ordernum'));
                $this->request->redirect($return);
            }
        }
    ...
    public function action_set_message()
    {
        if($_POST)
        {
            ...
            if($message OR $order_id)
            {
                ...
                $has = DB::select('id')->from('order_messages')->where('order_id', '=', $order_id)->execute()->get('id');
                if($has)
                    $result = DB::update('order_messages')->set($array)->where('id', '=', $has)->execute();
                else
                    $result = DB::insert('order_messages', array_keys($array))->values($array)->execute();
                if($result)
                {
