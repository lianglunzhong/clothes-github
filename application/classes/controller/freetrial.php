...
class Controller_Freetrial extends Controller_Webpage
{
            ...
            else
            {
                ...
                else
                {
                    $attachment = array(
                        ...
                        'link' => 'http://www.choies.com/freetrial/add?fbshare',
                        'description' => 'Complete 2 tasks and then have a chance to get the free trial',
                        'picture' => 'http://www.choies.com/cimages/freetrial.jpg',
                    );
    ...
    public function action_reports()
    {
        ...
        if($id)
        {
            ...
            $this->template->content = View::factory(LANGPATH . '/freebie/freetrial_reports_detail')->set('report', $report);
        }
        else
        {
            ...
            $reports = DB::select()->from('freetrial_reports')
                    ...
                    ->execute()->as_array();
            $this->template->content = View::factory(LANGPATH . '/freebie/freetrial_reports_list')
                ->set('reports', $reports)
