...
class Controller_Tracks extends Controller_Webpage
{
    ...
    public function action_track_order()
    {
        $lang=LANGUAGE;
        $seoinfo=array(
                "de"=>array("title"=>"Bestellung Verfolgen"),
                "es"=>array("title"=>"Rastrear Pedido"),
                "fr"=>array("title"=>"Suivi de Commande"),
                "ru"=>array("title"=>"Отслеживать Заказ"),
                );
        if($lang!=""){
            $this->template->title = $seoinfo[$lang]['title'];
        }else{
            $this->template->title = "Track Order";
        }
        $this->template->content = View::factory(LANGPATH . '/track');
    }

    //guo test 17track
    public function action_track_ordertest()
    {
        $this->template->content = View::factory(LANGPATH . '/track1');
    }
    ...
    public function action_ajax_orderdata()
    {
        ...
        {
            $data = DB::query(DATABASE::SELECT, "SELECT O.ordernum,O.created,O.currency,O.amount_order,O.amount_shipping,O.refund_status,O.payment_status,
                O.shipping_address,O.shipping_zip,O.shipping_phone
                FROM `orders` O left join `order_shipments` P ON O.id=P.order_id
    ...
    public function action_ajax_pagedata()
    {
        ...
        if ($code && strlen($code) < 20)
        {
            ...
            $data = DB::query(DATABASE::SELECT, "SELECT O.ordernum,O.created,O.currency,O.amount_order,O.amount_shipping,O.refund_status,O.payment_status,P.tracking_code,P.tracking_link,O.shipping_country,O.shipping_state,O.shipping_city,
                ...
                FROM `order_shipments` P LEFT JOIN `orders` O ON P.order_id=O.id
            ...
            if (count($data) > 0)
            {
                ...
                $url = "http://v5-api.17track.net:8044/handlertrack.ashx";
                $a = 'dhl';
                if(array_key_exists(strtolower($value['carrier']),$et_list)){
                    ...
                    $post = array(
                        "nums" => $value['tracking_code'],
                        "et" => $et,
                        "pt" => 0,
                    );
                   $response = Toolkit::curl_paytrack($url, $post);
                    ...
                    if ($response)
                    {
                        ...
                        {//查询成功
                            ...
                            {//国际邮政
                                ...
                                {
                                    if ($cvalue['a'] == $resp_arr['dat'][0]['track']['c'])
                                    {
                                        $tmp['dest_country'] = $cvalue['b'];
                                    }
                                    if ($cvalue['a'] == $resp_arr['dat'][0]['track']['b'])
                                    {
                                        $tmp['send_country'] = $cvalue['b'];
                                    }
                                }
                                $tmp['status'] = $resp_arr['dat'][0]['track']['e'] == 0 ? "Not Found" :
                                        ($resp_arr['dat'][0]['track']['e'] == 10 ? "Transporting" :
                                                ($resp_arr['dat'][0]['track']['e'] == 30 ? "Pick Up" :
                                                        ($resp_arr['dat'][0]['track']['e'] == 40 ? "Delivered" : "unknow")));
                                $tmp['history'] = $resp_arr['dat'][0]['track']['z1'];
    ...
    public function action_ajax_pagedata1()
    {
        $lang = Arr::get($_GET, 'lang', '');
        if (!($customer_id = Customer::logged_in()))
        {
                            ...
                            {//国际邮政
                                ...
                                $tmp['status'] = $resp_arr['dat'][0]['track']['e'] == 0 ? "Not Found" :
                                        ($resp_arr['dat'][0]['track']['e'] == 10 ? "Transporting" :
                                                ($resp_arr['dat'][0]['track']['e'] == 30 ? "Pick Up" :
                                                        ($resp_arr['dat'][0]['track']['e'] == 40 ? "Delivered" : "unknow")));
                                $tmp['history'] = $resp_arr['dat'][0]['track']['z2'];
                                //$tmp['history_send'] = $resp_arr['dat']['x'];                          
                            }
                            else
                            {
    ...
    public function action_customer_track(){
       ...
       if (count($data)>0) {
        ...
        $this->template->content = View::factory(LANGPATH . '/order/track_detail')->set('datas',$datas);
    }
                ...
                if($response){
                    ...
                    if($et==0){//国际邮政
                        if(isset($resp_arr['dat']['e'])&&$resp_arr['dat']['e']==30){//pick up
                            $update_order = DB::update('orders')->set(array('shipping_status' => 'pickup'))
                        ...
                        if(isset($resp_arr['dat']['e'])&&$resp_arr['dat']['e']==40){//delivered
                            ...
                            $update_order = DB::update('orders')->set(array('deliver_time' => $delivery_time, 'shipping_status' => 'delivered'))
                                ->where('site_id','=',$this->site_id)
                                ->where('id','=',$value['id'])
                                ->execute();
