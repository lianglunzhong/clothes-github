...
//        if (strlen($cache_content) > 100 AND !isset($_GET['cache']))
        ...
        else
        {
            ...
            if($this->language && $this->language != 'de' && $this->language != 'fr')
            {
                $result = DB::select()->from('banners')
            ...
            $this->template->content = View::factory(LANGPATH . '/index')
                ->set('banners', $banners)
    ...
    public function action_catalog()
    {
        ...
        {
            $redirects = isset($_GET['redirect']) ? '?redirect=' . $_GET['redirect'] : '';
            Request::Instance()->redirect(URL::site(Request::Instance()->uri . $redirects, 'http'));
        }
        $link = $this->request->param('id');
        if(strpos($link, '-c-') === False AND $link != 'daily-new')
        {
            $catalog = ORM::factory('catalog' . $this->language)
                ->where('link', '=', $link)
                ->where('site_id', '=', $this->site_id)
                ->where('visibility', '=', 1)
                ->find();
        ...
        else
        {
            ...
            if(strpos($uri1,"week1") || strpos($uri1,"week2") || strpos($uri1,"month")){
                ...
                $the_url = isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : 'http://www.choies.com/';
            ...
            {
                $this->template->content = View::factory(LANGPATH . '/activity/' . $catalog->link)->set('catalog', Catalog::instance($catalog->id));
            }
            else
            {
                $catalog = Catalog::instance($catalog->id);
                ...
                if ($catalog_link == 'daily-new')
                {
                    ...
                    $allowarr = array("week1","week2","month",92,621,953,961,53,52);
                    $numberallow = array(92,621,953,961,53,52);
                    ...
                    elseif(in_array($uriArr[1],$numberallow))
                    {
                        if (!isset($uriArr[0]))
                            $to = $today + 86400;
                        else
                    ...
                    */
                    ...
                    {
                        if(in_array($uriArr[1],$numberallow))
                        {
                            $catalog_instance = Catalog::instance($uriArr[1]);
                            $postersql = $catalog_instance->getpostsql();
                            $count_products = DB::query(Database::SELECT, 'SELECT COUNT(distinct p.id) as count FROM products p INNER JOIN catalog_products c ON p.id = c.product_id ' . $join_sql . ' WHERE c.catalog_id IN (' . $postersql . ') AND p.visibility = 1 AND p.status=1 AND p.stock <> 0 AND p.display_date >= ' . $from . ' AND p.display_date < ' . $to)->execute('slave')->get('count');
                        }
                        else
                        {
                            $catalog_instance = Catalog::instance($uriArr[1]);
                    ...
                    elseif($uriArr[0]=='week2')
                    {
                        $to = $today;
                        $from = strtotime("-7 day");
                    }
                    elseif($uriArr[0]=='month')
                    {
                        if (!isset($uriArr[0]))
                            $to = $today + 86400;
                        else
                    ...
                    elseif(in_array($uriArr[0],$numberallow))
                    {
                        if (!isset($uriArr[0]))
                            $to = $today + 86400;
                        else
                    ...
                    $cache_count_products1_key = 'site1_catalog_choies_newin' .LANGUAGE.'/'.$uriArr[0].$new_range;
                    ...
                    {
                        if(in_array($uriArr[0],$numberallow))
                        {
                            $catalog_instance = Catalog::instance($uriArr[0]);
                            $postersql = $catalog_instance->getpostsql();
                            $all_products3 = DB::query(Database::SELECT, 'SELECT distinct p.id FROM products p INNER JOIN catalog_products c ON p.id = c.product_id ' . $join_sql . ' WHERE c.catalog_id IN (' . $postersql . ') AND p.visibility = 1 AND p.status=1 AND p.stock <> 0 AND p.display_date >= ' . $from . ' AND p.display_date < ' . $to. $sql . ' ORDER BY p.position ' . $queue . ', id DESC ' . $new_range)->execute('slave')->as_array();
                        }
                        else
                        {
                            //$catalog_instance = Catalog::instance($uriArr[0]);
                ...
                {
                    //top sellers
                    $_top_sellers = DB::select('product_id')->from('catalog_products')->where('catalog_id', '=', 32)->execute()->as_array();
                    $new_date = 86400 * 14;
                }
                $top_sellers = array();
                $top = 0;
                ...
                {
                    $flash_sales = array();
                    $flashs = DB::select('product_id')->from('spromotions')->where('type', '=', 6)->where('expired', '>', time())->execute()->as_array();
                ...
                {
                    $template = DB::select('meta_title', 'meta_keywords', 'meta_description')->from('catalogs')->where('link', '=', 'new-in')->execute()->current();
                ...
                $this->template->content = View::factory(LANGPATH . '/catalog_1')
                    ->set('usdarr',$usdarr)
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    $parent_id = DB::select('parent_id')->from('catalogs')->where('id', '=', $cata_id)->execute('slave')->get('parent_id');
    ...
    public function action_product()
    {
        ...
        else
        {
            ...
            if($product_id)
            {
                $product = ORM::factory('product' . $this->language)
                    ->where('id', '=', $product_id)
                    ->where('visibility', '=', 1)
                    ->where('site_id', '=', $this->site_id)
                    ->find();
            ...
            if (toolkit::is_our_url($referer))
            {
                ...
                if($parsepath)
                {
                    ...
                    $current_catalog = DB::select('id')->from('catalogs')->where('link', '=', $link)->execute()->get('id');                    
                }
                else
                {
                    $current_catalog = 0;              
            ...
            if (!$product->loaded())
            {
                ...
                {
                    $productinfo = DB::select('id')->from('products')->where('link', '=', $link)->where('site_id', '=', $this->site_id)->execute('slave')->current();
                    $pid = $productinfo['id'];
                }
                $product = Product::instance($pid);
                $current_catalog = $product->default_catalog();
                $the_url = 'http://www.choies.com/';
                if(!$current_catalog)
                {
                    $the_url1 = $the_url.LANGPATH;
                }
            ...
            $product = Product::instance($product->id);
...
//                $product->daily_clicks();
            ...
            $celebrity_images = DB::select()->from('celebrity_images')->where('product_id', '=', $product->get('id'))->and_where('type','in',array(1,3))->order_by('position')->execute()->as_array();
            $link_images = DB::select()->from('celebrity_images')->where('product_id', '=', $product->get('id'))->and_where('type','in',array(2,3))->order_by('position')->execute()->as_array();
            //filter attributes
            $filter_sorts = array();
            ...
            if ($product->get('type') == 0)  //type 0 simple product
            {
                ...
                $this->template->content = View::factory(LANGPATH . '/product_prdress')
                    ->set('product', $product)
...
//                                        $continent = $continents[$countries[$country_code]];
                ...
                if($product->get('stock') == -1)
                {
                    if($product->get('set_id') == 2)
                    {
                        ...
                        {
                            $stock = DB::select('stocks')->from('product_stocks')
                ...
                //reviews list
                $reviews = DB::select()->from('reviews')->where('product_id', '=', $product_id)->where('is_approved', '=', 1)->order_by('time', 'DESC')->execute()->as_array();
                $overall_sum = 0;
                $quality_sum = 0;
                $price_sum = 0;
                $fitness_sum = 0;
                ...
                foreach($reviews as $review)
                {
                    ...
                    $quality_sum += $review['quality'];
                    $price_sum += $review['price'];
                    $fitness_sum += $review['fitness'];
                }

                $reviews_data = array(
                    'overall' => $overall_sum ? round($overall_sum / $review_count, 1) : 0,
                    'quality' => $quality_sum ? round($quality_sum / $review_count, 1) : 0,
                    'price' => $price_sum ? round($price_sum / $review_count, 1) : 0,
                    'fitness' => $fitness_sum ? round($fitness_sum / $review_count, 1) : 0,
                );
                $related_products = array();
                $flash_sales = DB::query(Database::SELECT, 'SELECT c.product_id, s.price FROM catalog_products c LEFT JOIN spromotions s ON c.product_id=s.product_id 
                    WHERE c.catalog_id = 290 AND s.type = 6 AND s.expired - unix_timestamp() BETWEEN 50400 AND 691200 ORDER BY c.position DESC, s.expired LIMIT 0, 7')
                    ->execute()->as_array();
                ...
                if($product->get('brand_id'))
                {
                    ...
                    $brands = DB::select('id', 'name')->from('brands')->where('id', '=', $brand_id)->execute()->current();
                }
                else
                {
                    $brands = array();
                ...
                $show_ship_tip = 1;
                $this->template->content = View::factory(LANGPATH . '/product')
                    ->set('tags',$tags)
        ...
        {   
            ...
            if($product_id)
            {
                ...
                if(empty($product->description)){
                    ...
                    $has_des = 0;
                    $pro = Product::instance($product_id);
...
/*                Site::add_404_log();
            ...
            Product::instance($product->id)->set_view_history();

            if ($cid)
            {
                Site::instance()->add_flow($cid, 'product', $product->sku);
            }
            //update click rate
//                DB::query(Database::UPDATE, 'UPDATE products SET clicks = clicks + 1 WHERE id = ' . $product->get('id'))->execute();
//                $product->daily_clicks();
            $this->template->title = $product->name . ' | ' . 'Choies';
            $this->template->keywords = $product->name;
            ...
            $setlang=Set::instance($product->set_id)->get('name');
            $setlangs = isset($setsLang[$setlang]) ? $setsLang[$setlang] : $setlang;
            if($setlangs)
            {
                if(LANGUAGE=='de'){
                    $this->template->description = 'Kaufen Sie ' . $product->name . ' jetzt online. Choies.com bietet die neuesten Fashion Damen ' . $setlangs .' zu günstigen Preisen mit kostenlosem Versand.';
                }elseif(LANGUAGE=='es'){
                    $this->template->description = 'Compra ' . $product->name . ' online ahora, choies.com ofrece ' . $setlangs . ' de la última moda de mujeres en precio bajo con envío gratis.';
                }elseif(LANGUAGE=='fr'){
                    $this->template->description = 'Achetez ' . $product->name . ' maintenant en ligne, Choies.com offrir femmes les dernières ' . $setlangs . ' de la mode à bas prix avec livraison gratuite.';
                }elseif(LANGUAGE=='ru'){
                    $this->template->description = 'Продать для ' . $product->name . ' онлайн now.Choies.com предложить последние модные женские ' . $setlangs . ' по низким ценам с бесплатной доставкой.';
                }
            }
            $this->template->fb_sku = $product->id;
            $this->template->fb_catalog = $product->set_id;

            $celebrity_images = DB::select()->from('celebrity_images')->where('product_id', '=', $product->id)->and_where('type','in',array(1,3))->order_by('position')->execute()->as_array();
            $link_images = DB::select()->from('celebrity_images')->where('product_id', '=', $product->id)->and_where('type','in',array(2,3))->order_by('position')->execute()->as_array();            
            //filter attributes
            $filter_sorts = array();
            $small_filter = array();
                ...
                if (!empty($sorts))
                {
                    foreach ($sorts as $sort)
                    {
                        ...
                        if($attr)
                        {
                            $filter_sorts[strtoupper($sort['sort'])] = $attr;
                            $lang = LANGUAGE;
                            $arrlang = array('de','es','fr','ru');
                            if(in_array($lang,$arrlang))
                            {
                                $sattr = DB::query(Database::SELECT, 'SELECT s.' . LANGUAGE . ' AS small FROM attributes_small s LEFT JOIN attributes a ON s.attribute_id = a.id WHERE a.name = "' . $attr . '"')->execute()->get('small');
            ...
            if ($product->type == 0)  //type 0 simple product
            {
                // include the php script  
                $gi = geoip_open(APPPATH . "classes/GeoIP.dat", GEOIP_STANDARD);
                // 获取国家代码 
                ...
                $attributes = Product::instance($product->id)->set_data();
                $color = $fabric = array();
                $fabricName = '';
                $size = array();
                $delivery_time = '';
            ...
            elseif ($product->type == 3)  //type 3 simple config product
            {
...
//                                        $continent = $continents[$countries[$country_code]];
                ...
                $set_brief = Set::instance($product->set_id)->get('brief');
                ...
                //产品的tag
                $tags = Product::instance($product->id)->tags();

                //VIP TYPES 
                $vipconfig = Site::instance()->vipconfig();

                $product_ins = Product::instance($product->id, LANGUAGE);
    ...
    public function action_search()
    {
        ...
        if ($catalog_id)
        {
            ...
            if ($catalog->get('id') AND $catalog->get('visibility'))
            {
                ...
                $catalog_join = ' LEFT JOIN catalog_products cp on (cp.product_id = p.id) ';
        ...
        // echo $keywords;exit;
        $result = DB::query(DATABASE::SELECT, "SELECT id,amount FROM searchwords
            WHERE words='" . $keywords . "'")->execute()->current();
        if ($result['id'])
        {
            $amount = $result['amount'] + 1;
            DB::update('searchwords1')->set(array('amount' => $amount))->where('id', '=', $result['id'])->execute();
        }
        else
        {
            DB::insert('searchwords1', array('words', 'site_id', 'amount'))
                ->values(array($keywords, $this->site_id, 1))
                ->execute();
        }
...
/*        if(LANGUAGE != '')
        ...
        if(strlen($keywords) == 2)
        {
            $result = DB::query(DATABASE::SELECT, "SELECT count(p.id) as num FROM products" . $lang_table . " p " . $catalog_join . "
                ...
                AND p.visibility = 1 AND p.status = 1 AND p.site_id = " . $this->site_id)->execute()->current();
        }
        ...
        if(strlen($keywords) == 2)
        {
           $result = DB::query(DATABASE::SELECT, "SELECT p.id, p.visibility AS score FROM products" . $lang_table . " p " . $catalog_join . "
                        WHERE p.name LIKE '%".$keywords."%'" . $sql . $catalog_in . "
                        AND p.visibility = 1 AND p.status = 1 AND p.site_id = " . $this->site_id . " ORDER BY p.status DESC,
        ...
        {
            $result = DB::query(DATABASE::SELECT, "SELECT p.id,MATCH(p.name,p.description,p.sku,p.keywords) AGAINST('" . $keywords . "') AS score FROM products" . $lang_table . " p " . $catalog_join . "
                        WHERE MATCH (p.name,p.description,p.sku,p.keywords) AGAINST ('" . $keywords . "' IN BOOLEAN MODE) " . $sql . $catalog_in . "
                        AND p.visibility = 1 AND p.status = 1 AND p.site_id = " . $this->site_id . " ORDER BY p.status DESC,
                        " . $orderby . " " . $queue . ", p.id DESC limit " . $pagination->offset . "," . $pagination->items_per_page)
                ->execute();
        ...
        $this->template->content = View::factory(LANGPATH . '/search')
            ->set('keywords', $q)
...
/*            $tstr = strtotime('midnight');
         ...
         //   echo $cache_content;
            ...
            $doc = ORM::factory('doc' . $this->language)
                ->where('site_id', '=', $this->site_id)
                ->where('link', '=', $link)
                ->find();
            ...
            if ($doc->content == '')
            {
                ...
                $this->template->content = View::factory(LANGPATH . '/doc/' . $link)->set('index_banners', $index_banners);
            }
            else
            {
                ...
                $template = View::factory(LANGPATH . '/doc/doc')
    ...
    public function action_404()
    {
        ...
        // $code = DB::select('code')->from('coupons')->where('ip', '=', $ip)->where('limit', '>', 0)->where('expired', '>', time())->execute()->get('code');
        // if(!$code)
        // {
        //     $max_id = DB::select(DB::expr('MAX(id) AS max_id'))->from('coupons')->execute()->get('max_id');
        //     $max_id ++;
        ...
        //         'ip' => $ip
        //     );
        //     $insert = DB::insert('coupons', array_keys($coupon))->values($coupon)->execute();
        //     if ($insert)
        //         $code = $coupon_code;
        //     else
        //         $code = '';
        ...
        $this->template->content = View::factory(LANGPATH . '/404')
            ->set('referer', $referer)
            ->set('code', $code);
    }
    ...
    public function action_lookbook()
    {
        ...
        if ($id)
        {
            ...
            {
                $ids = explode('-', $id);
                $c_images = DB::select()->from('celebrity_images')->where('id', '=', $ids[0])->where('type','in',array(1,3))->execute()->current();
                if ($c_images)
                {
                    $count = DB::select(DB::expr('COUNT(*) AS count'))->from('lookbook_reviews')->where('lookbook_id', '=', $ids[0])->and_where('type', '=', 1)->execute()->get('count');
                    $pagination = Pagination::factory(array(
                            'current_page' => array('source' => 'query_string', 'key' => 'page'),
                            'total_items' => $count,
                            'items_per_page' => 3,
                            'view' => LANGPATH . '/pagination'));
                    $reviews = DB::select()->from('lookbook_reviews')
                    ...
                    $this->template->content = View::factory(LANGPATH . '/lookbooks_product1')
            ...
            else
            {
                ...
                if (0)
                {
                    $count = DB::select(DB::expr('COUNT(*) AS count'))->from('lookbook_reviews')->where('lookbook_id', '=', $id)->where('type', '=', 0)->execute()->get('count');
                    $pagination = Pagination::factory(array(
                            'current_page' => array('source' => 'query_string', 'key' => 'page'),
                            'total_items' => $count,
                            'items_per_page' => 3,
                    ...
                    foreach ($images as $pid => $image)
                    {
                        if ($pid == 'main')
                            continue;
                        $pname = Product::instance(Product::get_productId_by_sku($pid))->get('name');
                    }
                    $this->template->title = "Looks of $pname | Choies Street Fashion";
                    $this->template->keywords = '';
                    $this->template->description = "It's amazing to see Stars with $pname in Choies. Come here to find the suitable one for you!";
                    $this->template->content = View::factory(LANGPATH . '/lookbooks_product')
        ...
        $cache_content = Cache::instance('memcache')->get($cache_key);
            ...
            {
                $count = DB::query(Database::SELECT, 'SELECT COUNT(DISTINCT product_id) AS count FROM celebrity_images WHERE site_id=1 and type in (1,3) and is_show = 1')->execute()->get('count');
                    ...
                    $pagination = Pagination::factory(array(
                            ...
                            'items_per_page' => 32,
                            'view' => LANGPATH . '/pagination_r'));      
            $c_images = DB::query(Database::SELECT, 'SELECT DISTINCT product_id FROM celebrity_images WHERE site_id=1 and type in (1,3) and is_show = 1 ORDER BY id DESC LIMIT ' .$pagination->offset . ',' . $pagination->items_per_page)->execute()->as_array();
                
                        $arr = array();
                $customer_id = Customer::logged_in();
                foreach($c_images as $k=>$v){
            $c_images[$k]['wish'] = DB::query(Database::SELECT, 'SELECT count(product_id) as wish FROM wishlists WHERE site_id=1 and product_id = '.$v['product_id'] . ' group by product_id')->execute()->current();  
                    array_push($arr,$v['product_id']);
                }
                
             if($customer_id){
                $wishlists = DB::select('product_id')->from('wishlists')->where('customer_id', '=', $customer_id)->where('product_id', 'IN', $arr)->execute()->as_array();
             }else{
                 $wishlists = array();
             }          
                ...
                $this->template->content = View::factory(LANGPATH . '/lookbooks_catalog')
    ...
    public function action_lookbook_ajax()
    {
        if ($_POST)
        {
            ...
            $limit = $index * 20;
            $lookbookArr = array();
            $c_images = DB::query(Database::SELECT, 'SELECT DISTINCT product_id FROM celebrity_images WHERE site_id=1 AND type IN(1, 3) ORDER BY id DESC LIMIT ' . $limit . ', 20')->execute()->as_array();
            $j = 0;
            foreach ($c_images as $p)
            {
                if(Product::instance($p['product_id'])->get('set_id') == 502)
                    continue;
                $c = DB::select('id', 'image')->from('celebrity_images')->where('product_id', '=', $p['product_id'])->where('type','in',array(1,3))->order_by('position')->execute()->current();
...
//          }
    ...
    public function action_lookbook_review()
    {
        if ($_POST)
        {
            ...
            if ($data['content'])
            {
                $result = DB::insert('lookbook_reviews', array_keys($data))->values($data)->execute();
                if ($result)
                {
    ...
    {
        $this->template->content = View::factory(LANGPATH . '/trends');
    }
    ...
    function action_ajax_product()
    {
        if ($_POST)
        {
            ...
            else
            {
                ...
                $attributes = $product->get('attributes');
                $attr_sizes = $product->get_attributes();
                ...
                if(in_array($set_id, $showinch))
                {
                    ...
                    $str = array();
                    $newstr = array();
                    foreach($attr_sizes as $attribute)
                    {
                        $str[$attribute] = '';
                        if(isset($briefsArr[$attribute]))
                        {
                            $briefsArr[$attribute]=explode(',',$briefsArr[$attribute]);
                            foreach($briefsArr[$attribute] as $k=>$v)
                            {
                                $sizes1=explode(':',$v);
                                ...
                                if(strpos($sizes1[1],'-'))
                                {
                                    ...
                                    $yi=round($sizes1[1][0] * 0.39370078740157, 1);
                                    $er=round($sizes1[1][1] * 0.39370078740157, 1);
                                    $c[$attribute][]=$sizes1[0].':'.$yi.'-'.$er.'inch';
                                }
                                else
                                {
                                    $i = round($sizes1[1] * 0.39370078740157, 1);
                    ...
                    $data['brief'] = '';
                    foreach($attr_sizes as $ke=>$va)
                    {
                        if($newstr[$va])
                        {
                            $data['brief'] .=$va.':'.$newstr[$va].'<br/>';
                            
                        }
                        elseif($va=='one size')
                        {//如果他是One size 尺寸
                            $brief=substr($brief,12);//去掉onesize
                            $onebrief=explode(',',$brief);
                            ...
                            $data['brief']= 'one size:'.$newnonec.'<br/>';
                        }
                    }
                }
                else
                ...
                //reviews list
                $reviews = DB::select()->from('reviews')->where('product_id', '=', $id)->where('is_approved', '=', 1)->order_by('id', 'DESC')->execute()->as_array();
                $overall_sum = 0;
                $review_count = count($reviews);
                foreach($reviews as $review)
                {
                ...
                $reviews_data = $overall_sum ? round($overall_sum / $review_count, 1) : 0;
                $overalls = explode('.', $reviews_data);
                if(!isset($overalls[1]) || !$overalls[1])
                {
                    ...
                    if($overalls[0] == 0)
                        ...
                        $reviews_data = $overalls[0];
                }
                else
                {
                    $star_num = $overalls[0] + 0.5;
                    $reviews_data = $overalls[0] . '.5';
                }
                //reviews list
                $data['review_count']=$review_count;
                $data['reviews_data']=$reviews_data;
                $data['wishlists']= DB::select(DB::expr('COUNT(id) AS count'))
                                        ->from('wishlists')
                                        ->where('site_id', '=', 1)
                                        ->where('product_id', '=', $id)
                                        ->execute()->get('count');
                ...
                if ($data['stock'] == -1)
                {
                    ...
                    if (!empty($attr_sizes))
                    {
                        ...
                        $stock_num = 0;
                        $a=0;
                        foreach($attr_sizes as $v=>$a){
                            if(strpos($a, 'US') !== FALSE){
                                $a=1;
                            }
                        }
                        $data['w']=$attr_sizes;
                        ...
                        if ($one_size)
                        {
                            $stocks = isset($product_stocks['one size']) ? $product_stocks['one size'] : 0;
                            if($stocks > 0)
                            {
                                $stock_num ++;
                        ...
                        elseif ($a)
                        {
                            ...
                            {
                                $size = trim($size);
                                $stocks = isset($product_stocks[$size]) ? $product_stocks[$size] : 0;
    ...
    function action_ajax_product1()
    {
        if ($_REQUEST)
        {
            ...
            else
            {
				...
				if($usersession){
					//获取is_vip
					$is_vip=DB::query(Database::SELECT, 'SELECT is_vip FROM `customers` where id='.$usersession['id'].' ')->execute('slave')->current();
				}
...
//                }
    ...
    public function action_ajax_accept(){
        ...
        if($data_id){
            //select a.product_id from catalog_products as a,products as b where a.catalog_id = 45 and a.product_id = b.id and b.visibility = 1 and b.status = 1 order by a.id desc limit 12
