...
class Controller_Review extends Controller_Webpage
{
    ...
    public function action_add()
    {
        ...
        if ($_POST)
        {
            ...
            if ($user)
            {
                ...
                $data['user_id'] = $user;
                $data['firstname'] = DB::select('shipping_firstname')->from('orders')->where('id', '=', $data['order_id'])->execute()->get('shipping_firstname');
                $review_re = Review::instance($product_id)->set($data);
                if (is_int($review_re))
                {
                    $item_id = Arr::get($data, 'item_id', 0);
                    DB::update('order_items')->set(array('erp_line_status' => 1))->where('id', '=', $item_id)->execute();
        ...
        $this->template->content = View::factory(LANGPATH . '/order/review')
    ...
    public function action_globebill()
    {
        ...
        //返回地址
        $ReturnURL = 'http://' . Site::instance()->get('domain') . '/review/globebill_return';
        //remark
        // $Remark = $client_ip;
...
//                    "post_url" => 'https://payment.globebill.com/Interface',
        ...
        $iframe = '<iframe id="payment_insite_iframe" width="100%" height="400" scrolling="no" frameborder="no" src="https://' . Site::instance()->get('domain') . '/review/insite_globebill/?' . $query . '"></iframe>';
        $this->template->content = View::factory(LANGPATH . '/cart/check_out3')
...
//                                }
        ...
        elseif ($isPush == '')
        {
            if (substr($Result, 0, 5) == 'I0061')  //排除订单号重复(I0061)的交易
            {
                ...
                if ($is_pay_insite)
                {
                    echo '<script language="javascript">top.location.replace("http://' . Site::instance()->get('domain') . LANGPATH . '/payment/success/' . $BillNo . '");</script>';
                }
                else
                {
            ...
            else
            {
                ...
                if ($order['payment_status'] == 'success' OR $order['payment_status'] == 'verify_pass')
                {
                    echo '<script language="javascript">top.location.replace("http://' . Site::instance()->get('domain') . LANGPATH . '/payment/success/' . $order['ordernum'] . '");</script>';
                    exit;
                }
...
//          }
    ...
    public function action_facebook()
    {
        ...
        print_r($fqlResult);
        $this->template->content = View::factory(LANGPATH . '/facebook');
    }
    ...
    public function action_catalog()
    {
        ...
        echo 'SELECT DISTINCT s.product_id, s.price, s.expired 
                    FROM spromotions s LEFT JOIN catalog_products c ON s.product_id=c.product_id 
                    WHERE c.catalog_id IN(' . implode(',', $children) . ') AND s.type = 6 AND s.expired + 36000 > ' . time() . ' 
