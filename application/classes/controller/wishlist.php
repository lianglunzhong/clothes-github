...
class Controller_Wishlist extends Controller
{
    ...
    public function action_add()
    {
        ...
        if($return == 'cart')
        {
           ...
           // DB::delete('cartcookies')->where('customer_id', '=', $customer_id)->and_where('key', '=', $key)->execute();//cartcookie
            DB::delete('cartproduct')->where('customer_id', '=', $customer_id)->and_where('item_id', '=', $cart_product['id'])->and_where('attribute', '=', $cart_product['attributes']['Size'])->execute();//cartcookie
        }
        else
        {
        ...
        $data['created'] = time();
		//获取产品 link1
		$link = DB::query(Database::SELECT, 'select link FROM products where id = "'.$product['id'].'"')->execute()->current();
    ...
    public function action_ajax_add()
    {
        ...
        else
        {
			...
			$id = DB::query(Database::SELECT, 'select id FROM wishlists where customer_id = "'.$customer_id.'" and product_id="'.$product_id.'"')->execute()->get('id');
			if(!$id){
				$data = array();
            $data['site_id'] = Site::instance()->get('id');
            $data['customer_id'] = $customer_id;
            ...
            if (Wishlist::set($data))
            {
                ...
                $wishlists = $cache->get($keywishlists) + 1;
                //设置点击后的数量
