...
class Controller_Address extends Controller_Webpage
{
    ...
    public function action_add()
    {
        ...
        if ($_POST)
        {
            ...
            if ($data['is_default'])
                DB::update('addresses')->set(array('is_default' => 0))->where('customer_id', '=', $customer_id)->execute();
        ...
        $this->template->content = View::factory(LANGPATH . '/address/add')
    ...
    public function action_edit()
    {
        ...
        $this->template->content = View::factory(LANGPATH . '/address/edit')
    ...
    public function action_ajax_edit()
    {
        ...
        if ($_POST)
        {
            ...
            elseif ($aid)
            {
                ...
                {
                    DB::update('addresses')->set(array('is_default' => 0))->where('customer_id', '=', $customer_id)->execute();
                    DB::update('addresses')->set(array('is_default' => 1))->where('id', '=', $aid)->execute();
                }
        ...
        $countries = Site::instance()->countries(LANGUAGE);
        echo View::factory(LANGPATH . '/address/ajax_edit')
            ->set('address', $address)
            ->set('countries', $countries)
...
$addres  .="<li class='mb20' id=\"m_address_li_".$value['id']."\">";
            ...
            if($is_cart != 2)
            {
                ...
                if ($aid == 'new' || $aid == '')
                {
                    if ($address['is_default'])
                        DB::update('addresses')->set(array('is_default' => 0))->where('customer_id', '=', $customer_id)->execute();
                    $result = DB::insert('addresses', array_keys($address))->values($address)->execute();
                    $aid = $result[0];
                    $data['success'] = 1;
                    $data['address_id'] = $aid;
                ...
                elseif ($aid)
                {
                    ...
                    DB::update('addresses')->set($address)->where('id', '=', $aid)->execute();
                    $data['success'] = 1;
    ...
    public function action_ajax_delete1()
    {
        if ($_POST)
        {
            ...
            {
                $customer_id = Customer::logged_in();
                $delete = DB::delete('addresses')->where('id', '=', $address_id)->and_where('customer_id', '=', $customer_id)->execute();
                if($delete)
                {
                    $data['success'] = 1;
    ...
    public function action_ajax_data()
    {
        if ($_POST)
        {
            ...
            {
                $addresses = DB::select()->from('addresses')->where('site_id', '=', $this->site_id)->where('id', '=', $id)->execute()->current();
    ...
    public function action_set_default()
    {
        if ($_POST)
        {
            ...
            else
            {
                ...
                $address_customer = DB::select('customer_id')->from('addresses')->where('id', '=', $address_id)->execute()->get('customer_id');
                if (!$address_customer)
                {
                ...
                {
                    DB::update('addresses')->set(array('is_default' => 0))->where('customer_id', '=', $customer_id)->execute();
                    DB::update('addresses')->set(array('is_default' => 1))->where('id', '=', $address_id)->execute();
