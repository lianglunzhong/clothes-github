...
class Controller_Cart extends Controller_Webpage
{
    ...
    public function action_view()
    {
        ...
        foreach($cart['products'] as $ck=>$p)
        {
            ...
            {
                $attributes = DB::select('attributes', 'stocks')->from('product_stocks')->where('product_id', '=', $p['id'])->where('stocks', '>', 0)->execute();
                $attrs = array();
                $stocks = array();
                foreach ($attributes as $a)
                {
                    $size = trim($a['attributes']);
                    if($set == 2)
                    {
                        $att = explode('/', $size);
                        if(isset($att[$index]))
        ...
        if(!in_array($p['id'],$giftarr)){
            if($customer_id){
                ...
                $datas['attribute'] = $p['attributes']['Size'];

                $cookieproducts = DB::select()->from('cartproduct')->where('customer_id','=',$customer_id)
                    ->and_where('item_id','=',$p['id'])
                    ->and_where('attribute','=',$p['attributes']['Size'])
                    ->execute()->current();
                if($cookieproducts){
                    ...
                    ->values(array('customer_id'=> $customer_id,
                        ...
                        'qty'=>$p['quantity'],
                        'attribute'=>$p['attributes']['Size'],
                        'created'=>time(),))
                    ->execute();
                }
            }
                                        }
/*            $expired = DB::select('expired')->from('spromotions')->where('product_id','=',$p['id'])
    ...
    //fb 获取金额用
      $cart_save = 0;
        ...
        $this->template->content = View::factory(LANGPATH . '/cart/view')
            ->set('cart', $cart)
    ...
    {
        Site::instance()->add_clicks('continue');
    ...
    public function action_point()
    {
        if ($_POST)
        {
            ...
            $banned = DB::select('id')->from('catalogs')->where('name', '=', 'Points banned')->execute()->get('id');
        ...
        if ($coupon_code)
        {
            ...
            if (!$errno)
            {
               ...
               if(!empty($cart_promotion_logs))
                ...
                $data['saving'] = Site::instance()->price($saving, 'code_view');
    ...
    public function action_ajax_shipping()
    {
        ...
        $count_address = DB::select(DB::expr('COUNT(id) AS count'))->from('addresses')->where('customer_id', '=', $customer_id)->execute()->get('count');
        ...
        foreach ($site_shipping as $key => $price)
        {
            ...
            elseif($cart['amount']['items'] >= 79 && $cart_extra['extra_fee']<=0 && !$cart_extra['extra_flg'])
            {
                $site_shipping[2]['price'] = 0;
            }
        }
    ...
    public function action_shipping_billing()
    {
        ...
        if (!$has_address)
        {
            ...
            if (!empty($default_address))
            {
                ...
                // $shipping['price'] = 0;
                // Cart::shipping($shipping);
                echo '<script language="javascript">top.location.replace("http://' . $_SERVER['HTTP_HOST'] . LANGPATH . '/cart/check_out");</script>';
                exit;
            }
        }
        ...
        $this->template->content = View::factory(LANGPATH . '/cart/shipping_billing')
            ->set('cart', $cart)
...
//                        $template = View::factory(LANGPATH . '/cart_confirm')
...
//                        $this->request->response = $template;
            $this->template->content = View::factory(LANGPATH . '/cart/confirm')
                ->set('points_avail', $customer->points() - $cart['points'])
                ->set('amount_left', $cart['amount']['total'])
                ->set('cart', $cart);
        }
    ...
    public function action_add()
    {
                  ...
                  if(preg_match("/\d/", $size))
                    {
                        ...
                        {
                            $isnumber = 0;
                            $newsizearr = explode('-',$size);
                            $newsize = $newsizearr[0].'-UK'.$newsizearr[1];
                        }
                       
                        if(empty($newsize))
                        {
                            $newsize = $post['attributes']['Size'];
                        }
                        if($isnumber)
                        {
                            $product_stocks = DB::select('stocks')
                                ...
                                ->where('attributes', '=', $post['attributes']['Size'])
                                ->execute()->get('stocks');  
                        }
                        else
                        {
        ...
        DB::query(Database::UPDATE, 'UPDATE products SET add_times=add_times+1 WHERE id=' . $post['id'])->execute();    
        }     
    ...
    public function action_ajax_cart()
    {
        ...
        if ($data['count'] > 0)
        {
            ...
            {
                $img = Product::instance($product['id'], LANGUAGE)->cover_image();
                $transname = '';
                if(LANGUAGE == 'de' or LANGUAGE == 'fr')
                {
                    $transname = Product::instance($product['id'], LANGUAGE)->transname();  
                }
                $product_name = !empty($transname) ? $transname : Product::instance($product['id'], LANGUAGE)->get('name');

                $cart_view .= '<li>';
                $cart_view .= '<a class="mybag-pic" href="' . Product::instance($product['id'], LANGUAGE)->permalink() . '" ><img src="' . image::link(Product::instance($product['id'])->cover_image(), 3) . '" alt="" /></a>';
                $cart_view .= '<div class="mybag-info">';
                $cart_view .= '<a class="mybag-info-name" href="' . Product::instance($product['id'], LANGUAGE)->permalink() . '" title="' . Product::instance($product['id'], LANGUAGE)->get('name') . '">' . $product_name . '</a>';
                $cart_view .= ' <span>' . Site::instance()->price($product['price'], 'code_view');
                $p_price = Product::instance($product['id'], LANGUAGE)->get('price');
...
//                  $cart = Cart::get();
            ...
            if ($customer_id)
            {
                ...
                $this->template->content = View::factory(LANGPATH . '/cart/check_out2')
                    ->set('cart', $cart)
                    ->set('countries', $countries)
            ...
            {
                $this->template->content = View::factory(LANGPATH . '/cart/check_out1')->set('cart', $cart);
            }
        }
    }
 ...
 //guo  FB获取金额
        ...
        $this->template->content = View::factory(LANGPATH . '/cart/checkout11')
            ->set('cart', $cart)
            ->set('countries', $countries)
            ->set('countries_top', $countries_top)
            ->set('default_address', $default_address)
...
/*
 ...
 //guo  FB获取金额
        ...
        $this->template->type = 'purchase';
        $this->template->checkprice = $checkprice;
        $this->template->content = View::factory(LANGPATH . '/cart/checkout')
            ->set('cart', $cart)
    ...
    public function action_edit_address()
    {
        ...
        if ($_POST)
        {
            ...
            {
                Message::set(__('address_modify_do_fail'), 'error');
                echo '<script language="javascript">top.location.replace("http://' . $_SERVER['HTTP_HOST'] . $return . '");</script>';
                exit;
            }
            if ($aid == 'new')
            {
            ...
            elseif ($aid)
            {
                ...
                $shipping_address['phone'] = Arr::get($_POST, 'shipping_phone', '');
                $shipping_address['cpf'] = Arr::get($_POST, 'shipping_cpf', '');
                DB::update('addresses')->set($shipping_address)->where('id', '=', $aid)->execute();
                Cart::shipping_billing($_POST);
        ...
        $address = DB::select()->from('addresses')->where('id', '=', $id)->execute()->current();
        $countries = Site::instance()->countries(LANGUAGE);
        $carriers = Site::instance()->carriers();
        foreach ($carriers as $c)
        {
        ...
        echo View::factory(LANGPATH . '/cart/address')->set('address', $address)->set('countries', $countries)->set('carrier', $carrier);
        exit;
    }
    ...
    public function action_delete_address()
    {
        if ($_POST)
        {
            ...
            {
                $customer_id = Customer::logged_in();
                DB::delete('addresses')->where('id', '=', $address_id)->and_where('customer_id', '=', $customer_id)->execute();
            }
        }
        $this->request->redirect(LANGPATH . '/cart/shipping_billing');
    }
    ...
    public function action_ajax_add()
    {
        if ($_POST)
        {
            ...
            $post['items'][] = $post['id'];
            $post['type'] = Arr::get($_POST, 'type', 0);
            $size = Arr::get($_POST, 'size', '');
            if (!$size)
            {
                echo json_encode(0);
                exit;
            }
            $color = Arr::get($_POST, 'color', '');
            $type = Arr::get($_POST, 'attrtype', '');
            if ($post['quantity'] == 0)
            {
                echo json_encode(0);
                exit;
            ...
            if ($post['type'] == 3)
            {
                ...
                if (!$size AND !$color AND !$type)
                {
                    $product = Product::instance($post['id'], $lang);
                    $attributes = $product->get('attributes');
                    if (!empty($attributes))
                    {
                        echo json_encode(0);
                ...
                else
                {
                    if ($size)
                    {
                        $post['attributes']['Size'] = trim($size);
                    }
                    if ($color)
                    {
                        $post['attributes']['Color'] = trim($color);
                    }
                    if ($type)
                    {
                ...
                if (Product::instance($post['id'])->get('stock') == -1)
                {
                    ...
                    if(preg_match("/\d/", $size))
                    {
                        ...
                        if($isnumber)
                        {
                            if($asize == 'US' or $asize == 'UK' or is_numeric($asize))
                            {
                                $product_stocks = DB::select('stocks')
                                    ->from('product_stocks')
                                    ->where('product_id', '=', $post['id'])
            ...
            elseif ($post['type'] == 0)
            {
                ...
                else
                {
                    ...
                    if ($_POST['size'] == 'CUSTOM SIZE' AND $_POST['custom_size'])
                    {
                        $attr['size'] = trim($_POST['custom_size']);
                    }
                    else
                    {
                        $attr['size'] = trim($_POST['size']);
                    }
                    $attr['color'] = $_POST['color'];
                    $attr['delivery time'] = $_POST['delivery_time'];
                    $post['attributes'] = $attr;
    ...
    public function action_product_edit()
    {
        if($_POST)
        {
            ...
            if($key)
            {
                ...
                // Cart::delete($key);
                $key1 = $product['id'].'_'.$product['attributes']['Size'];
                // Cart::delete($key);
    ...
    public function action_add_to_cart()
    {
        ...
        if($id)
        {
            ...
            DB::query(Database::UPDATE, 'UPDATE products SET add_times=add_times+1 WHERE id=' . $id)->execute();
        ...
        //密码长度过短或过长
            ...
            else
            {
                ...
                if (!Customer::instance()->is_register($data['email']))
                {
                    ...
                    $isflag = DB::select('id')->from('customers')->where('email', '=', $data['email'])->where('flag', '=', 1)->execute('slave')->get('id');
                    if(!empty($isflag))
                    {
                        $data['id'] = $isflag;
                    }
            ...
            Cartcookie::add2cart($cart_data);//将商品信息存入COOKIE
            DB::query(Database::UPDATE, 'UPDATE products SET add_times=add_times+1 WHERE id=' . $product[0])->execute();
            Product::instance($product[0])->daily_add_times();
            Site::instance()->add_clicks('add_to_cart');
            $cart = Cartcookie::get(); 
            echo $cart ? trim('success') : 'Failed to add a shopping cart!';exit;
    ...
    public function action_collection(){
        ...
        $ok = DB::insert('wishlists', array_keys($insetArr))->values($insetArr)->execute();
        echo "ok";
        exit;
        }
