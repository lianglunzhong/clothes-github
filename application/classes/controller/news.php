...
class Controller_News extends Controller
{
	...
	public function action_list()
	{
		...
		$this->request->response = View::factory(LANGPATH . '/news_list')
				->set('data', $data)
	...
	public function action_details()
	{
		...
		$this->request->response = View::factory(LANGPATH . '/news_details')
				->set('news', $news)
				->set('page', $page)
				->set('show', $show)
				->render();
	...
	public function action_sitemap()
    {
        ...
        echo View::factory(LANGPATH . '/doc/sitemap',$content_data);
    }
    ...
    public function action_sendmail()
    {
        ...
        $result = DB::query(DATABASE::SELECT, 'SELECT id,`email`,`shipping_firstname`,`ordernum`,`currency`,`amount`, `created`, `amount_shipping`,`shipping_firstname`,`shipping_lastname`,`shipping_address`,`shipping_city`,`shipping_state`,`shipping_zip`,`shipping_country`,`shipping_phone` FROM `orders`  
                            WHERE payment_status = "new" AND is_active = 1 AND created BETWEEN ' . $datefrom . ' AND ' . $dateto . ' AND site_id=1')
                ->execute();
        foreach ($result as $o)
        {
            if ($key > 20)
...
/*        $key = 0;
        ...
        foreach ($result as $val)
        {
            ...
            if ($rcpt AND !empty($wishlists))
            {
                ...
                $body = View::factory(LANGPATH . '/customer/mail_wishlist')->set('wishlists', $wishlists);
    ...
    public function action_sendmail1()
    {
		...
		$customers = DB::query(DATABASE::SELECT, 'SELECT id,email,vip_start,vip_end FROM customers WHERE vip_level>=0 and is_vip>0 and vip_end>='.$time.' and vip_end<'.$end_time.' ORDER BY id')->execute('slave')->as_array();
		if(count($customers)>0){
				foreach($customers as $k=>$v){
					if($i==1){
						...
						$result1 = DB::query(Database::SELECT, 'SELECT id,customer_id,email,sum(amount/rate) as sum_amount FROM `orders` where customer_id='.$v['id'].' and payment_status="verify_pass" and refund_status<>"refund" and is_active=1 and verify_date > '. $last_time .' and  verify_date <= '. $vip_ok['verify_date'] .'')->execute('slave')->current();
						$newvip_id=DB::query(Database::SELECT, 'SELECT id,firstname,lastname,email,is_vip,vip_start,vip_end FROM `customers` where id='.$v['id'].' ')->execute('slave')->current();
						$mail_params=array();
						...
						DB::update('customers')->set(array('vip_level'=>-1))->where('id', '=', $v['id'])->execute();
					}
				}
			}
		exit;
    ...
    public function action_shuiail()
    {
        ...
        if($order_id)
        {
            ...
            $trans_id = DB::select('trans_id')->from('order_payments')->where('order_id', '=', $order_id)->execute()->get('trans_id');
            $succeed = 525;
