    ...
    public function action_login()
    {
        ...
        if ($_POST)
        {
            ...
            if ($customer_id = Customer::instance()->login($data))
            {
                ...
                if($ppec_status == 0)
                {
                    ...
                    if ($insert)
                    {
                        ...
                        DB::insert('customer_coupons', array_keys($c_coupon))->values($c_coupon)->execute();
                        ...
                        $porders = DB::select('amount', 'currency', 'shipping_firstname')->from('orders')->where('customer_id', '=', $customer_id)->where('payment_status', '=', 'verify_pass')->execute();
                        $points = 0;
                        ...
                        if($send)
                        {
                            DB::update('customers')->set(array('ppec_status' => 1))->where('id', '=', $customer_id)->execute();
                        }
                    }
                }
                ...
                $datas['attribute'] = $value['attributes']['Size'];

                        $cookieproducts = DB::select()->from('cartproduct')->where('customer_id','=',$customer_id)
                            ->and_where('item_id', '=', $value['id'])
                            ->and_where('attribute', '=', $value['attributes']['Size'])
                            ->execute()->current();
                        if($cookieproducts){
                    DB::update('cartproduct')->set($datas)
                    ->where('id', '=', $cookieproducts['id'])->execute();
                        }else{
                    DB::insert('cartproduct', array('customer_id','item_id','qty','attribute','created',))
                    ->values(array('customer_id'=> $customer_id,
                        'item_id'=>$datas['item_id'],
                        'qty'=>$value['quantity'],
                        'attribute'=>$value['attributes']['Size'],
                        'created'=>time(),))
                    ->execute();
            ...
            else
            {
                ...
                $has_email = DB::select('id')->from('customers')->where('email', '=', $data['email'])->execute('slave')->get('id');
                if($has_email)
...
//        }
        ...
        $this->template->content = View::factory(LANGPATH . '/login')
            ->set('referer', $referer);
    }
    ...
    public function action_ajax_login()
    {
        ...
        if ($_POST)
        {
            ...
            if ($customer_id = Customer::instance()->login($data))
            {
                ...
                $firstname = DB::select('firstname')->from('customers')->where('id', '=', $customer_id)->execute('slave')->get('firstname');
				if(!$firstname){
					$firstname='Choieser';
				}
                $return['success'] = 1;
    ...
    public function action_ajax_register()
    {
        ...
        if ($_POST)
        {
            ...
            else
            {
                ...
                {
                    if ($data['ip'] != 0)
                        $has_ip = DB::select('id')->from('customers')->where('ip', '=', $data['ip'])->execute()->get('id');
                    else
                        $has_ip = 0;
                    ...
                    if ($customer_id = Customer::instance()->set($data))
                    {
                        ...
                        $celebrity = DB::select('id', 'customer_id')->from('celebrits')->where('email', '=', $data['email'])->execute()->current();
                        if (!empty($celebrity) AND $celebrity['customer_id'] == 0)
                        {
                            DB::update('celebrits')->set(array('customer_id' => $customer_id))->where('id', '=', $celebrity['id'])->execute();
                        }
...
//                                        }
    ...
    public function action_logout()
    {
        ...
        if (Arr::get($_SERVER, 'HTTPS', 'off') != 'on')
        {
            ...
            Request::Instance()->redirect('http://' . $domain . LANGPATH);
        }
        else
        {
            Request::instance()->redirect(LANGPATH);
            ...
            {               
                ...
                if (!Customer::instance()->is_register($data['email']))
                {
                    if ($data['ip'] != 0)
                        ...
                        $has_ip = 0;
                    $data['lang'] = $this->language;
                    $isflag = DB::select('id')->from('customers')->where('email', '=', $data['email'])->where('flag', 'in', array('1', '4'))->execute()->get('id');
                    if(!empty($isflag))
                    {
                        $data['id'] = $isflag;
                    }
                    ...
                    if ($customer_id = Customer::instance()->set($data))
                    {
                        ...
                        $array = array(
                                ...
                                array('name'=>'coupon_code','value'=>$mail_params['coupon_code']),
                                array('name'=>'expired','value'=>$mail_params['expired'])
                            );
                        $mail_lang = 'active';
                        $postarr[]['fields'] = $array;
                        Mail::Sendcustomer($postarr,$mail_lang,'NEWREGISTER');
...
//        }
    ...
    public function action_profile()
    {
        ...
        if ($_POST)
        {
            ...
            $old = DB::select('firstname', 'lastname', 'country', 'birthday', 'give_points')->from('customers')->where('id', '=', $customer_id)->execute()->current();
            if (Customer::instance($customer_id)->profile($data))
            {
//                                $ip = ip2long(Request::$client_ip);
//                                if($ip != 0)
//                                        $hasip = DB::select('id')->from('customers')->where('ip', '=', $ip)->and_where('give_points', '=', 1)->execute()->get('id');
//                                else
//                                        $hasip = 0;
                $points_message = '';
                $hasip = DB::select('give_points')->from('customers')->where('id', '=', $customer_id)->execute()->get('give_points');
                if (!$hasip)
                {
                    if (!$old['give_points'] AND (!$old['firstname'] OR !$old['lastname'] OR !$old['country'] OR !$old['birthday']))
                    {
                        ...
                        {
                            // 完善信息送500积分
                            // $result1 = DB::query(Database::UPDATE, 'UPDATE customers SET points=points+500,give_points=1 WHERE id=' . $customer_id)->execute();
...
/*                            if ($result1 AND $result2)
        ...
        $this->template->content = View::factory(LANGPATH . '/customer/profile')
    ...
    public function action_password()
    {
        ...
        $this->template->content = View::factory(LANGPATH . '/customer/password')->set('customer', $customer);
    }
    ...
    public function action_orders()
    {
        ...
        $orders = $orders = DB::select()->from('orders')
        ...
        $this->template->content = View::factory(LANGPATH . '/order/list')
            ->set('orders', $orders)
    ...
    public function action_orders222()
    {
		...
		echo '<pre>';
		print_r($orders);
        $this->template->content = View::factory(LANGPATH . '/order/list11')
            ->set('orders', $orders)
            ->set('pagination', $pagination->render());
    }
    ...
    public function action_unpaid_orders()
    {
        ...
        $count = DB::select(DB::expr('COUNT(id) AS count'))->from('orders')
        ...
        $this->template->content = View::factory(LANGPATH . '/order/unpaid')
            ->set('orders', $orders)
            ->set('pagination', $pagination->render());
    }
    ...
    public function action_wishlist()
    {
        ...
        $this->template->content = View::factory(LANGPATH . '/customer/wishlist')
            ->set('pagination', $pagination->render())
    ...
    public function action_address()
    {
        ...
        $this->template->content = View::factory(LANGPATH . '/address/list')
...
//                                $customer = Customer::instance($customer_id)->login_action();
                ...
                $mail_params['new_password'] = 'http://' . Site::instance()->get('domain') . '/customer/reset_password?token='.$string;
                $mail_params['firstname'] = Customer::instance($customer_id)->get('firstname');
                if(!$mail_params['firstname'])
                    $mail_params['firstname'] = 'customer';
                $mail_params['email'] = Customer::instance($customer_id)->get('email');
        ...
        $this->template->content = View::factory(LANGPATH . '/customer/forgot_password');
    }
    ...
    public function action_contact_us()
    {
        ...
        $this->request->response = View::factory(LANGPATH . '/doc_contact-us')
    ...
    public function action_reset_password()
    {
        ...
        if ($return)
        {
            ...
            $this->template->content = View::factory(LANGPATH . '/customer/reset_password')
                ->set('title', Site::instance()->get('meta_title'))
                ->set('keywords', Site::instance()->get('meta_keywords'))
                ->set('description', Site::instance()->get('meta_description'));
        }
    ...
    public function action_product_request()
    {
        ...
        $this->request->response = View::factory(LANGPATH . '/customer_product_request')
            ->set('title', Site::instance()->get('meta_title'))
            ->set('keywords', Site::instance()->get('meta_keywords'))
            ->set('description', Site::instance()->get('meta_description'))
            ->render();
    ...
    public function action_summary()
    {
        ...
        $this->template->content = View::factory(LANGPATH . '/customer/summary')
    ...
    public function action_dropshipping()
    {
        ...
        $this->request->response = View::factory(LANGPATH . '/customer_dropshipping')
            ->set('title', Site::instance()->get('meta_title'))
    ...
    public function action_affiliate_id()
    {
        ...
        $this->request->response = View::factory(LANGPATH . '/customer_affiliate_id')
    ...
    public function action_affiliate_paypal()
    {
        ...
        $this->request->response = View::factory(LANGPATH . '/customer_affiliate_paypal')
    ...
    public function action_affiliate_commission()
    {
        ...
        $customer = Customer::instance(Customer::logged_in());
        $this->request->response = View::factory(LANGPATH . '/customer_affiliate_commission')
    ...
    public function action_points()
    {
        ...
        $this->request->response = View::factory(LANGPATH . '/customer_points')
    ...
    public function action_points_history()
    {
        ...
        $records = DB::query(Database::SELECT, 'SELECT amount,status,type,order_id,created FROM `point_records` WHERE amount > 0 AND customer_id=' . $customer . $sql)->execute()->as_array();
        $payments = DB::query(Database::SELECT, 'SELECT amount,order_num,order_id,created FROM `point_payments` WHERE customer_id=' . $customer . $sql)->execute()->as_array();
        ...
        $pagination = Pagination::factory(array(
                ...
                'items_per_page' => 8,
                'view' => LANGPATH . '/pagination'));
        $result = DB::query(Database::SELECT, 'SELECT customer_id,amount,status,type,order_id,created FROM `point_records` WHERE customer_id=' . $customer . ' AND amount > 0 UNION ALL SELECT customer_id,amount,note,order_num,order_id,created FROM `point_payments` WHERE customer_id=' . $customer . $sql . ' LIMIT ' . $pagination->items_per_page . ' OFFSET ' . $pagination->offset)->execute()->as_array();
        //title
        $lang=LANGUAGE;
        $seoinfo=array(
        ...
        $this->template->content = View::factory(LANGPATH . '/customer/points_history')
            ->set('pagination', $pagination->render())
            ->set('datas', $result)
            ->set('records', $records)
    ...
    public function action_blog_show()
    {
        ...
        $cele_id = DB::select('id')->from('celebrits')->where('email', '=', $email)->execute()->get('id');
        if (!$cele_id)
        {
        ...
        $count2 = DB::select(DB::expr('COUNT(DISTINCT ordernum,sku) AS count'))->from('celebrity_products')->where('celebrity_id', '=', $cele_id)->execute()->get('count');
        ...
        $allshow = DB::query(Database::SELECT, 'SELECT orders.ordernum,order_items.sku FROM order_items LEFT JOIN orders ON orders.site_id = order_items.site_id AND orders.id = order_items.order_id 
                                WHERE orders.customer_id = ' . $customer_id . ' AND orders.payment_status = "verify_pass" AND order_items.status <> "cancel"')
                ->execute()->as_array();
        $showeds = DB::select(DB::expr('DISTINCT ordernum,sku'))->from('celebrity_products')->where('celebrity_id', '=', $cele_id)->execute()->as_array();
        ...
        $showed = DB::select(DB::expr('DISTINCT ordernum,sku'))->from('celebrity_products')->where('celebrity_id', '=', $cele_id)->limit($pagination2->items_per_page)->offset($pagination2->offset)->order_by('id', 'DESC')->execute()->as_array();
        //title
        $lang=LANGUAGE;
        $seoinfo=array(
        ...
        $this->template->content = View::factory(LANGPATH . '/customer/blog_show')
            ->set('showed', $showed)
            ->set('toshow', $toshow)
            ->set('cele_id', $cele_id)
    ...
    public function action_edit_url()
    {
        if ($_POST)
        {
            ...
            else
            {
                ...
                {
                    if ($urls[$key])
                        DB::update('celebrity_products')->set(array('url' => $urls[$key]))->where('id', '=', $id)->execute();
                    else
                        DB::delete('celebrity_products')->where('id', '=', $id)->execute();
                }
...
//                                        }
    ...
    public function action_coupons()
    {
        ...
        $count = DB::query(Database::SELECT, 'SELECT COUNT(c.id) AS count FROM customer_coupons c LEFT JOIN coupons p ON c.coupon_id=p.id 
            WHERE p.code <> "" AND c.coupon_id=p.id AND c.site_id=' . $this->site_id . ' AND c.customer_id=' . $customer_id)
                ->execute()->get('count');
        $pagination = Pagination::factory(array(
                'current_page' => array('source' => 'query_string', 'key' => 'page'),
                'total_items' => $count,
        ...
        $coupons = DB::query(Database::SELECT, 'SELECT p.code,p.value,p.item_sku,p.type,p.target,p.product_limit,p.catalog_limit,p.created,p.expired 
            FROM customer_coupons c LEFT JOIN coupons p ON c.coupon_id=p.id 
        ...
        $this->template->content = View::factory(LANGPATH . '/customer/coupon')
            ->set('pagination', $pagination->render())
            ->set('coupons', $coupons)
            ->set('used', $used)
            ->set('us', $us);
            ...
            foreach ($cartcookie as $key => $value) {
                ...
                foreach ($value['products'] as $p) {
                    ...
                    <td>
                        ...
                        <p style=" margin:10px 0;"><a href="'.$product_link.'" target="_blank"><img src="http://www.choies.com/images/docs/cartcookie/edmwaitlist_03.jpg"></a></p>
                    </td>';
                }
                echo $value['email']."<br>";
    ...
    public function action_me_and_choies()
    {
        ...
        //Points history
        $records = DB::query(Database::SELECT, 'SELECT amount,status,type,order_id,created FROM `point_records` WHERE amount > 0 AND customer_id=' . $customer_id . ' ORDER BY created DESC')->execute()->as_array();
        $payments = DB::query(Database::SELECT, 'SELECT amount,order_num,order_id,created FROM `point_payments` WHERE customer_id=' . $customer_id . ' ORDER BY created DESC')->execute()->as_array();
        $activated = 0;
        $Rewarded = 0;
        ...
        $this->template->content = View::factory(LANGPATH . '/customer/me_and_choies')
            ->set('customer', $customer)
