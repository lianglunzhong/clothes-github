...
class Controller_Brand extends Controller_Webpage
{
	...
	public function action_list()
	{
        ...
        else
        {
            ...
            if($id == 220)
            {
                $count_products = DB::query(Database::SELECT, 'SELECT COUNT(*) AS counts FROM products WHERE brand_id = "" AND visibility = 1 AND status = 1 AND set_id NOT IN (340,365,374,378,390)')->execute()->get('counts');
            }
            else
            {
                $count_products = DB::query(Database::SELECT, 'SELECT COUNT(*) AS counts FROM products WHERE brand_id = ' . $id . ' AND visibility = 1 AND status = 1 AND set_id NOT IN (340,365,374,378,390)')->execute()->get('counts');                
            }
            ...
            if($id == 220)
            {
                $products = DB::query(Database::SELECT, 'SELECT id FROM products WHERE brand_id ="" AND visibility = 1 AND status = 1 AND set_id NOT IN (340,365,374,378,390) 
					ORDER BY ' . $orderby . ' ' . $queue . ',display_date desc, id desc LIMIT ' . $pagination->items_per_page . ' OFFSET ' . $pagination->offset)
					->execute()->as_array();  
            }
            else
            {
				$products = DB::query(Database::SELECT, 'SELECT id FROM products WHERE brand_id = ' . $id . ' AND visibility = 1 AND status = 1 AND set_id NOT IN (340,365,374,378,390) 
					ORDER BY ' . $orderby . ' ' . $queue . ',display_date desc, id desc LIMIT ' . $pagination->items_per_page . ' OFFSET ' . $pagination->offset)
					->execute()->as_array();
            }
            $brands = DB::select()->from('brands')->where('id', '=', $id)->execute()->current();
            $this->template->content = View::factory(LANGPATH . '/brand_list')
                ->set('brands', $brands)
    ...
    public function action_brandpage()
    {
        ...
        {
            $brands = DB::select('id','name','label')->from('brands')->where('id', '>', 33)->execute()->as_array();
