...
//		print_r(Cart::get());exit;
	...
	public function action_forgetPassword(){
		...
		if($customer_id = Customer::instance()->is_register($email))
		{
			...
			$mail_params['new_password'] = "<a href='http://".Site::instance()->get('domain')."/customer/reset_password?token=".$string."'>http://".Site::instance()->get('domain')."/customer/reset_password?token={$string}</a>";
...
//		}		 
	...
	static function getProductsByCatalog(Catalog $_catalog,$_offset=null,$_limit=null,$_orderby=null,$_desc=null,$_currency=null){	
		...
		$products = DB::select()->from('products')->where('id','IN',$ids)->order_by($_orderby,$_desc)->execute();
	...
	static function cartToObject($_obj,$_currency){
		...
		$products = DB::select()->from('products')->where('id','IN',$ids)->execute();
		$productMap = array();
		foreach ($products as $item) {
