...
/*        $fileurl5 = "/home/data/www/htdocs/clothes/googleproduct/choies_criteo_feed.uk.csv";
        if (!file_exists($fileurl5))
        {
            $k = 'UK';
            self::daochucsv($k);
...
/*        $fileurl3 = "/home/data/www/htdocs/clothes/Export/cjfeed-au.csv";
        ...
        //guo add
/*        $fileurl5 = "/home/data/www/htdocs/clothes/googleproduct/choies_criteo_feed.au.csv";
        if (!file_exists($fileurl5))
        {
            $k = 'AU';
            self::daochucsv($k);
   ...
   public function action_mihqtgyllsTxtca()
    {
        ignore_user_abort(true);
        set_time_limit(0);
        $fileurls = array("CA" => "/home/data/www/htdocs/clothes/googleproduct/choies_googleshopping_feed.ca.txt"); //定义 一个文件路径，初始默认为UK（这个路径在哪？）
    ...
    public static function cjdatafeed($lang = null)
    {
        ...
        foreach ($products as $product)
        {
            ...
            if ($lang)
            {
                ...
                $product_type = isset($set_names[strtoupper($lang)][$set_name]) ? $set_names[strtoupper($lang)][$set_name] : '';
            }
            else
            {
                if ($default_catalog)
...
/*                if ($k == "US" || $k == "EN" || $k =="AU" || $k=="UK")
    ...
    public function _daochutxtforsize($k)
    {
        ...
        set_time_limit(0);
        ini_set('memory_limit', '512M');
        $pla = DB::select('promotion')->from('pla')->where('type','=',1)->where('country','=',$k)->execute()->as_array();
        $local_directory = '/home/data/www/htdocs/clothes/googleproduct/';
        ...
        foreach ($products as $product)
        {
            ...
            if($k == 'DE' || $k == "FR" || $k == "ES")
            {
                $data[] = $product['sku'].'-'.$sizechima.'-'.$k;
            }
            else
            {
                $data[] = $product['sku'].'-'.$k.'-'.$sizechima;                
            }
        ...
        {     
            ...
            if ($set_name)
            {
                ...
                {       
                    ...
                    {
                        $product_type = isset($set_names["US"][$set_name]) ? $set_names["US"][$set_name] : '';
                    }       
    ...
    public function _daochutxttwoch($k)
    {
        ...
        foreach ($products as $product)
        {
            ...
            if(strpos($product['name'],'Kimono') != false){
                    ...
                    $product_type = isset($set_names[$k][$set_name]) ? $set_names[$k][$set_name] : '';
                }   
