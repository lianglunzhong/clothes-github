...
class Controller_activity extends Controller_Webpage
{
    public function action_flash_sale()
    {
        ...
        $gets = array();
        $cache_key = $mobile_key . Request::instance()->uri() . '_' . implode('_', $gets);
        ...
        {
            $flash_sale = DB::select('id', 'template', 'image_src', 'meta_title', 'meta_keywords', 'meta_description')->from('catalogs')->where('name', '=', 'flash sale')->execute('slave')->current();
            $time = time();
            $weekly = DB::query(Database::SELECT, 'SELECT c.product_id, s.price, s.catalog, s.expired 
                FROM catalog_products c LEFT JOIN spromotions s ON c.product_id=s.product_id 
                WHERE c.catalog_id=' . $flash_sale['id'] . ' AND s.type = 6 AND s.expired - unix_timestamp() BETWEEN 50400 AND 691200 ORDER BY c.position DESC, s.expired')
            ...
            {
                $banner = DB::select('image_src', 'image_link', 'image_alt',	'image_map','pimage_src')->from('catalogs_' . $this->language)->where('id', '=', $flash_sale['id'])->execute('slave')->current();
            ...
            {
                $banner = DB::select('image_src', 'image_link', 'image_alt',	'image_map','pimage_src')->from('catalogs')->where('id', '=', $flash_sale['id'])->execute('slave')->current();
                    $result = DB::select()->from('banners')
                        ->where('site_id', '=', $this->site_id)
                        ->where('type', '=', 'side')
                        ->where('visibility', '=', 1)
            ...
            foreach($usds as $u)
            {
                $catalog_id = DB::select('id')->from('catalogs')->where('link', '=', $u)->execute('slave')->get('id');
                if(!$catalog_id)
                    continue;
                $products = DB::query(Database::SELECT, 'SELECT P.id FROM catalog_products C LEFT JOIN products P ON C.product_id = P.id WHERE C.catalog_id = ' . $catalog_id . ' AND P.visibility = 1 ORDER BY P.status DESC, C.position DESC, P.position DESC LIMIT 0, 6')->execute('slave')->as_array();
            ...
            $this->template->content = View::factory(LANGPATH . '/activity/flash_sale')
    //            ->set('daily', $daily)
                ->set('weekly', $weekly)
            ...
            Cache::instance('memcache')->set($cache_key, $this->template, 86400*2);
        }
    }
    ...
    public function action_flash_sale11()
    {
        ...
        $flash_sale = DB::select('id', 'template', 'image_src', 'meta_title', 'meta_keywords', 'meta_description')->from('catalogs')->where('name', '=', 'flash sale')->execute()->current();
        $time = time();
...
//                ->execute()->as_array();
        ...
        if($this->language)
        {
            $banner = DB::select('image_src', 'image_link', 'image_alt',    'image_map')->from('catalogs_' . $this->language)->where('id', '=', $flash_sale['id'])->execute()->current();
                $result = DB::select()->from('banners')
                    ->where('site_id', '=', $this->site_id)
                    ->where('type', '=', 'side')
                    ->where('visibility', '=', 1)
        ...
        {
            $banner = DB::select('image_src', 'image_link', 'image_alt',    'image_map')->from('catalogs')->where('id', '=', $flash_sale['id'])->execute()->current();
                $result = DB::select()->from('banners')
                    ->where('site_id', '=', $this->site_id)
                    ->where('type', '=', 'side')
                    ->where('visibility', '=', 1)
        ...
        foreach($usds as $u)
        {
            $catalog_id = DB::select('id')->from('catalogs')->where('link', '=', $u)->execute()->get('id');
            if(!$catalog_id)
                continue;
            $products = DB::query(Database::SELECT, 'SELECT P.id FROM catalog_products C LEFT JOIN products P ON C.product_id = P.id WHERE C.catalog_id = ' . $catalog_id . ' AND P.visibility = 1 ORDER BY P.status DESC, C.position DESC, P.position DESC LIMIT 0, 6')->execute()->as_array();
...
/*        echo '<pre>';
        ...
        $this->template->content = View::factory(LANGPATH . '/activity/flash_sale11')
//            ->set('daily', $daily)
            ->set('weekly', $weekly)
            ->set('flash_sale', $flash_sale)
            ->set('banner', $banner)
...
$this->template->content = View::factory(LANGPATH . '/activity/addfile');
...
$this->template->content = View::factory(LANGPATH . '/activity/swimsuit');
    ...
    public function action_love_wins()
    {
        ...
        $this->template->content = View::factory(LANGPATH . '/activity/lovewins');
    }
    public function action_pre_black_friday2016()
    {
        if(LANGUAGE=='es')
        {
            $this->template->content = View::factory(LANGPATH . '/activity/blackfriday-es');
        }else{
            $this->template->content = View::factory(LANGPATH . '/activity/blackfriday-en');
        }

    }
    public function action_sale_party_for_the_black_friday_and_cyber_monday2016()
    {
        if(LANGUAGE=='es')
        {
            $this->template->content = View::factory(LANGPATH . '/activity/11-22-es');
        }
        else
        {
            $this->template->content = View::factory(LANGPATH . '/activity/11-22');
        }
    }
...
//        print_r($products);exit;
        $this->template->content = View::factory('activity/party_wear')->set('products', $products);
    }
    ...
    public function action_update_questionnaire()
    {
        ...
        if ($_POST)
        {
            ...
            if ($result)
            {
                $insert = array('customer_id' => $customer_id, 'coupon_id' => 66988, 'site_id' => $this->site_id);
                DB::insert('customer_coupons', array_keys($insert))->values($insert)->execute();
                $rcpt = $email;
...
Choies Team<br/>
        ...
        $this->template->content = View::factory('activity/update_questionnaire');
    }

    public function action_update_success()
    {
        $this->template->content = View::factory('activity/update_success');
    }
    
    public function action_pastel_hues()
    {
        $this->template->content = View::factory('/activity/pastel_hues');
    }
    
    public function action_catalog()
    {
        $link = $this->request->param('id');
        $catalog_id = DB::select('id')->from('catalogs')->where('link', '=', $link)->execute()->get('id');
        if(!$catalog_id)
        ...
        else
        {
            ...
            $this->template->content = View::factory(LANGPATH . '/activity/catalog')->set('products', $products)->set('catalog', $catalog)->set('pagination', $pagination);
    ...
    public function action_marketing_survey()
    {
        ...
        $customer_id = Customer::logged_in();
        $this->template->content = View::factory('activity/marketing_survey');
    }
...
Choies Team<br/>
            ...
            $this->template->content = View::factory(LANGPATH . 'activity/marketing_survey_questions');
        }
        }else{
    ...
    public function action_ms_success1()
    {
        ...
        if ($customer_id)
        {
            ...
            if ($has_submit)
            {
                $this->template->content = View::factory(LANGPATH . '/activity/ms_success1');
            }else{
                $this->request->redirect('activity/marketing_survey');
            }
        }else{
    ...
    public function action_ms_success2()
    {
        ...
        if ($customer_id)
        {
            ...
            if ($has_submit)
            {
                $this->template->content = View::factory(LANGPATH . '/activity/ms_success2');
            }else{
                $this->request->redirect('activity/marketing_survey');
            }
        }else{
    ...
    public function action_ms_success3(){
        $this->template->content = View::factory(LANGPATH . '/activity/ms_success3');
    }
    public function action_only_florals(){
        $this->template->content = View::factory(LANGPATH . '/activity/only_florals');
    }
    public function action_classic_white_shirt(){
        $this->template->content = View::factory(LANGPATH . '/activity/classic_white_shirt');
    }
    public function action_marketing_survey_result(){
        $this->template->content = View::factory(LANGPATH . '/activity/marketing_survey_result');
    }

    public function action_stripes_collection()
    {
        $this->template->content = View::factory(LANGPATH . '/activity/stripes_collection');
    }
    ...
    public function action_kimono_style_vote()
    {
        ...
        $this->template->content = View::factory(LANGPATH . '/activity/kimono')
            ->set('user_id', $user_id)
            ->set('has', $has)
            ->set('sum', $sum)
    ...
    public function action_kimonos_collection()
    {
        ...
        $this->template->keywords = "Choies Kimonos, kimono jacket, Trend, Look, Outfit, Choies Coats, Jackets, Shoes, Skirts, Bags";
        $this->template->description = "Choies is a latest high street fashion shopping website offering limited-time sale events with quality products in women's and men's fashion, jewelry and accessories, beauty products. Choies offers discounts of 50 to 60% off retail prices. Choies is free to join and registration takes only seconds.";
        $this->template->content = View::factory(LANGPATH . '/activity/kimonos_collection');
    }
    ...
    public function action_skirt_looks()
    {
        ...
        $this->template->content = View::factory(LANGPATH . '/activity/skirt_looks')
            ->set('data', $data);
    }
    ...
    {
        $this->template->content = View::factory(LANGPATH . '/activity/tropical_designs');
    }
    ...
    public function action_thanksgiving_looks()
    {
        ...
        $this->template->content = View::factory(LANGPATH . '/activity/thanksgiving_looks')
    ...
    public function action_black_friday_daily_deal()
    {
        ...
        $this->template->content = View::factory('/activity/black_friday_daily_deal')
            ->set('nows', $nows)
            ->set('ends', $ends)
            ->set('end_day', $end_day);
    }
    ...
    public function action_quick_response_code()
    {
        ...
        if($customer_id)
        {
            $has_code = DB::select('id')->from('customer_coupons')->where('coupon_id', '=', $code_id)->where('customer_id', '=', $customer_id)->execute()->get('id');
            if(!$has_code)
            {
                $insert = array('customer_id' => $customer_id, 'coupon_id' => $code_id, 'site_id' => $this->site_id);
                DB::insert('customer_coupons', array_keys($insert))->values($insert)->execute();
            }
        }
        $this->template->content = View::factory('/activity/quick_response_code')->set('customer_id', $customer_id);
    }
    ...
    */
      public function action_back_to_school666()
    {
        ...
        $custom_filter = array();
          ...
          // $easy1 = DB::select('id', 'template', 'image_src', 'meta_title', 'meta_keywords', 'meta_description')->from('catalogs')->where('name', '=', 'Dresses')->execute()->current();
        ...
        $count_products = $catalog->count_products();
            $pagination = Pagination::factory(array(
                        ...
                        'total_items' => $count_products,
                        'items_per_page' => 100,
                        'view' => LANGPATH . '/pagination_1'));
        // $time = time();  
          // $weekly1 = DB::query(Database::SELECT, 'SELECT c.product_id
          // FROM products p INNER JOIN catalog_products c ON p.id = c.product_id
...
$products = $catalog->products($pagination->offset, $pagination->items_per_page, $orderby, $queue, $custom_filter);
        ...
        $customer_id = Customer::logged_in();
                    if($customer_id){
                        $wishlists = DB::select('product_id')->from('wishlists')->where('customer_id', '=', $customer_id)->where('product_id', 'IN', $products)->execute()->as_array();
                    }else{
                        $wishlists = array();
                    }
        
        $this->template->content = View::factory(LANGPATH . '/activity/homecome')
    ...
    {
        $this->template->content = View::factory('/activity/christmas_shopping_guide');
    }
    ...
    public function action_c_xmas_giveaway()
    {
        ...
        echo View::factory('activity/xmas_giveaway')
    ...
    public function action_new_year_slogan_wanted()
    {
        ...
        $comments = $comments ? array_slice($comments, $pagination->offset, $pagination->items_per_page) : array();
        echo View::factory('activity/new_year_slogan_wanted')
            ->set('comments', $comments)
            ->set('pagination', $pagination->render())
            ->set('step', $step);
        exit;
...
'ASMX2112',
        ...
        echo View::factory('activity/vote_for_the_best_design')
            ->set('skuArr', $skuArr)
            ->set('comments', $comments)
            ->set('count', $count)
            ->set('votes', $votes)
...
'DRES0422B037W',
 ...
 $has_submit = DB::query(Database::SELECT, 'SELECT sku FROM giveaway WHERE  user_id=' . $customer_id)->execute('slave')->as_array();   
        ...
        $this->template->content = View::factory(LANGPATH . '/activity/activity_vote')
        ->set('skuArr', $skuArr)
        ->set('votes', $votes)
        ->set('cu', $cu);
    //    ->set('a',$a);
    ...
    {
       $this->template->content = View::factory(LANGPATH . '/activity/getcode');        
    }
...
'PYA8NF',
    ...
    $this->template->content = View::factory(LANGPATH . '/activity/vip')
    ->set('skuArr', $skuArr);        
    }
    ...
    $this->template->content = View::factory(LANGPATH . '/activity/china');
    }
    ...
    {
        echo View::factory(LANGPATH . '/luck_draw');exit;
    }   
    ...
    public function action_senddraw()
    {
        ...
        //更改当前用户抽奖状态
        $upstate = DB::query(Database::UPDATE, 'update customers set draw_state = 1 where id = "'.$customer_id.'"')->execute();
    ...
    public function action_singles_day_sale()
    {
        ...
        $this->template->content = View::factory(LANGPATH.'/singles-day-sale')->set('data',$data);
    }
    ...
    public function action_testdefault()
    {
        ...
        {
        $catalogs = DB::select('id', 'link')->from('catalogs')->where('visibility', '=', 1)->execute()->as_array();
    ...
    public function action_lottery()
    {
        ...
        $this->template->description = "Join in Singles Day Sale with the lowest price. CHOiES offers you women's Clothing, Dresses , Accessories & Shoes!";
        $this->template->content = View::factory(LANGPATH.'activity/lottery')->set('data',$data)->set('pagination', $pagination->render());
    }
                    ...
                    if ($lottery)
                    {
                        ...
                        if($coupon1['coupon']==0){
                            ...
                            if ($insert)
                            {
                                ...
                                $ok=DB::insert('customer_coupons', array_keys($c_coupon))->values($c_coupon)->execute();
...
'TSHI1010A053C' =>array('14.99','7.99', '7.99' ),
        ...
        $this->template->content = View::factory(LANGPATH.'/activity/blackfriday')->set('blackarr',$blackarr)->set('blackprice',$price);
    }

    public function action_holiday_gift_guide()
    {
        $this->template->content = View::factory(LANGPATH.'/activity/holidaygift')->set('blackarr',$blackarr)->set('blackprice',$price);        
    }

    public function action_giveaway()
    {
        $this->template->content = View::factory(LANGPATH.'/activity/giveaway')->set('blackarr',$blackarr)->set('blackprice',$price);        
    }
    ...
    public function action_points_redeem_coupons()
    {
        ...
        {
            $hots = DB::query(Database::SELECT, 'SELECT product_id FROM catalog_products WHERE catalog_id = 32 ORDER BY position DESC LIMIT 0, 28')->execute()->as_array();  
            $cache->set($keyhots, $hots, 86400 * 7);
        }

        $this->template->title = 'points';
        $this->template->description = "Join in Singles Day Sale with the lowest price. CHOiES offers you women's Clothing, Dresses , Accessories & Shoes!";
        $this->template->content = View::factory(LANGPATH.'/activity/points_redeem_coupons')
            ->set('data',$data)
            ->set('hots',$hots);
    }
    ...
    public function action_choies_questionnaire201603()
    {
        ...
        if($_POST)
        {
            ...
            if($result)
            {
                $insert = array('customer_id' => $customer_id, 'coupon_id' => 907463, 'site_id' => $this->site_id);
                DB::insert('customer_coupons', array_keys($insert))->values($insert)->execute();
        ...
        $this->template->content = View::factory('activity/choies_questionnaire201603');  
    }

    public function action_survey_success()
    {
        $this->template->content = View::factory('activity/survey_success'); 
    }
    ...
    {
        $this->template->content = View::factory(LANGPATH .'/activity/model');      
    }
    ...
    public function action_special()
    {
        ...
        else
        {
            ...
            $pagination = Pagination::factory(array(
                    ...
                    'items_per_page' => 48,
                    'view' => LANGPATH . '/pagination_r'));
            $product = DB::select('link','id')->from('products')
                ->where('id', 'in', $proids)
                ...
                ->execute('slave')
                ->as_array();
            $this->template->content = View::factory(LANGPATH . '/activity/blackfriday1')
                ->set('product', $product)
                ->set('pagination', $pagination->render());
        }
