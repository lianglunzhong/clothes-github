...
class Controller_Ajax extends Controller_Webpage
{
    ...
    public function action_wishlist_data()
    {
        ...
        if($product_idsint)
        {
            ...
            {
                $pids = array();
                $wishlists = DB::query(Database::SELECT, 'SELECT product_id FROM wishlists WHERE customer_id = ' . $customer_id . ' AND product_id IN (' . $product_idsint . ')')->execute();
    ...
    public function action_marks_data()
    {
        ...
        if (!($ready_shippeds = $cache->get($key395)))
        {
            ...
            $readys= DB::select('product_id')->from('catalog_products')->where('catalog_id', '=', 395)->execute()->as_array();
        ...
        if (!$flash_sales_products = $cache->get($flash_sale_key))
        {
            ...
            $flash_sales = DB::select('product_id')->from('spromotions')->where('type', '=', 6)->where('expired', '>', time())->execute()->as_array();
        ...
        {
            $marks = DB::select('catalog_id', 'product_id', 'mark_name')->from('marks')->execute()->as_array();
        ...
        foreach($marks as $m)
        {
            ...
            elseif($m['catalog_id'] == $catalog_id)
        ...
        if($user_session['id'])
        {
            ...
            if($point_id)
            {
                ...
                if($user_points >= $point_id)
                {
                    ...
                    $coupons = DB::query(Database::SELECT, 'select code FROM coupons where code = "'.$coupon_code.'"')->execute()->get('code');
                    
                    if($coupons)
                    {
                        $count = DB::query(Database::SELECT, 'select COUNT(*) as sum FROM coupons where value = '.$point_name.' and code like "'.$coupon_code.'%"')->execute()->current();
    ...
    public function action_more_product()
    {
        ...
        foreach($newproduct_id as $newproduct)
        {
            ...
            if($lang == 'de' || $lang == 'fr')
            {
                $transname = Product::instance($newproduct,$lang)->transname();
                $product_title = $transname;    
            }
    ...
    */
            ...
            if($product_id)
            {
                ...
                if(!$relate_products = $cache->get($key))
                {
                    ...
                    if(!empty($relate_array))
                    {
                        ...
                        $same_set_relates = DB::query(Database::SELECT, 'SELECT id, sku, link, price, set_id FROM products WHERE set_id = ' . $set_id . ' AND visibility = 1 AND status = 1 AND id IN (' . implode(',', $relate_array) . ') LIMIT 0, 14')->execute();
                        $ga_relates_array = DB::query(Database::SELECT, 'SELECT id, sku, link, price, set_id FROM products WHERE set_id != ' . $set_id . ' AND visibility = 1 AND status = 1 AND id IN (' . implode(',', $relate_array) . ') LIMIT 0, 28')->execute();
                    ...
                    $cache->set($key, $relate_products, 7200);
                }
                foreach($relate_products as $key1 => $relate1)
                {
                    foreach($relate1 as $key2 => $relate)
                    {
                        ...
                        $relate_products[$key1][$key2]['link'] = LANGPATH . $relate_products[$key1][$key2]['link'];
                    }
                }
    ...
    public function action_topseller_relate()
    {
        ...
        if($_POST)
        {
            ...
            if(!$relate_products = $cache->get($key))
            {
                ...
                $relate_array = DB::query(Database::SELECT, 'SELECT p.id, p.sku, p.link, p.price FROM products p left join catalog_products cp on p.id = cp.product_id WHERE cp.catalog_id =32 and p.visibility = 1 AND p.status = 1 and p.set_id not in (0,29) order by p.hits desc LIMIT 0, 28')->execute('slave')->as_array();
    ...
    public function action_catalog_filters()
    {
        if($_REQUEST)
        {
            ...
            {
                $cache = Cache::instance('memcache');
                $cache_key = 'catalog_all_filters1_' . $catalog_id;
                    ...
                    if(!empty($response['hits']['hits']))
                    {
                        foreach($response['hits']['hits'] as $res)
                        {
                            $catalog_all_filters[$res['_source']['id']] = array(
                                'size' => $res['_source']['size_value'],
                                'color' => $res['_source']['color_value'],
                ...
                if(LANGUAGE)
                {
                    $filters['color_names'] = $this->get_color_by_language(LANGUAGE);
                }
                ...
                if(empty($filters['term']['color_value']) && empty($filters['match']) && empty($filters['range']) && empty($gets['pick']) && empty($gets['sort']))
                {
                    ...
                    for($i = $from;$i < $from + $size;$i ++)
                    {
                        if(isset($all_products[$i]))
                        {
                            ...
                            if($i < $show_to)
                            {
                                $product_ins = Product::instance($all_products[$i]);
                                ...
                                if(LANGUAGE == 'de' || LANGUAGE == 'fr')
                                {
                                    ...
                                    $transname = $product_ins->transname();
                                    $product_title = $transname; 
                                }
                    ...
                    if(!empty($search_res['hits']['hits']))
                    {
                        ...
                        foreach ($search_res['hits']['hits'] as $item)
                        {
                            ...
                            if($s_row < $show_size)
                            {
                                ...
                                $mark = '';
                                $product_ins = Product::instance($item['_source']['id']);
                                $lang = LANGPATH;
                                $product_title = empty($lang) ? $item['_source']['name'] : $item['_source']['name_es'];
                                if(LANGUAGE == 'de' || LANGUAGE == 'fr')
                                {
                                    $new_product_ins = Product::instance($item['_source']['id'], LANGUAGE);
                                    $transname = $new_product_ins->transname();
                                    $product_title = $transname;
                                }
                                $product_infos[] = array(
