...
class Site
{
    ...
    public function _load($id = 0, $domain)
    {
        ...
        if (!($data = $cache->get($key, array())))
        {
            if (!$id)
            {
                $data = DB::select()->from('sites')->where('domain', '=', $domain)->execute()->current();
            }
            else
            {
                $data = DB::select()->from('sites')->where('id', '=', $id)->execute()->current();
            }
            if ($data)
                $cache->set($key, $data);
        }
    ...
    public function route()
    {
        ...
        Route::set('product1', '(<language>/)' . $this->data['product'] . '/<id>', array('link' => '[\w-]+', 'language' => '^[a-z]{2}+')
                )
                ->defaults(array(
                    'controller' => 'site',
                    'action' => 'product1',
        ...
        {
        $catalogs = DB::select('id', 'link')->from('catalogs')->where('site_id', '=', $this->data['id'])->where('visibility', '=', 1)->execute()->as_array();
            $cache->set($cache_key, $catalogs, 1800);
        }
        
        /*$catalogs = DB::select('id', 'link')->from('catalogs')->where('site_id', '=', $this->data['id'])->where('visibility', '=', 1)->execute();*/
        foreach ($catalogs as $catalog)
        {
            ...
            Route::set('catalog1/' . $catalog['id'], '(<language>/)<id>(/<link>(/<price>(/<filter>)))', array('id' => $catalog['link'], 'link' => '[\w-]+', 'price' => '[\w-]+', 'filter' => '[\w-]+', 'language' => '^[a-z]{2}+'))
                    ->defaults(array(
                        'controller' => 'site',
                        'action' => 'catalog1'
                    ));
            Route::set('catalog1/' . $catalog['id'] . '-1', '(<language>/)<id>(/<link>(/<price>(/<filter>)))', array('id' => $catalog['link'] . '-c-' . $catalog['id'], 'link' => '[\w-]+', 'price' => '[\w-]+', 'filter' => '[\w-]+', 'language' => '^[a-z]{2}+'))
                    ->defaults(array(
                        'controller' => 'site',
                        'action' => 'catalog1'
                    ));
    ...
    public function root_catalogs()
    {
        ...
        {
            $result = DB::select('id')->from('catalogs')
        ...
        public function currencies($now_currrency = '')
    {
        ...
        if($now_currrency)
        {
            ...
            else
            {
                ...
                $cache_key = 'site_currency1_' . $now_currrency;
        ...
        else
        {
            ...
            if (!($data = $cache->get($key)))
            {
                $data = DB::select()->from('currencies')->where('name', 'IN', $currency_names)->execute()->as_array();
    ...
    public function add_flow($cid = '', $type = '', $name = '')
    {
        ...
        {
            $fid = DB::select('id')->from('celebrity_flows')->where('celebrity_id', '=', $cid)->and_where('type', '=', $type)->and_where('name', '=', $name)->execute()->get('id');
            if ($fid)
            {
                DB::query(Database::UPDATE, 'UPDATE celebrity_flows SET flow=flow+1 WHERE id=' . $fid)->execute();
            }
            else
            {
                $data = array(
                ...
                DB::insert('celebrity_flows', array_keys($data))->values($data)->execute();
            }
            return false;
        }
    }
    ...
    {
        $types = array('add_to_cart', 'cart_view', 'continue', 'checkout', 'ppec', 'cart_login', 'cart_checkout', 'proceed', 'globebill', 'ppjump', 'card_pay','cart_to_cookie','cookie_to_cart',);
        ...
        else
        {
            ...
            $dayid = DB::select('id')->from('site_clicks')->where('day', '=', $day)->execute()->get('id');
            if ($dayid)
            {
                $result = DB::query(Database::UPDATE, 'UPDATE site_clicks SET `' . $type . '` = `' . $type . '` + 1 WHERE id = ' . $dayid)->execute();
            }
            else
            {
                $result = DB::query(Database::INSERT, 'INSERT INTO site_clicks (`day`, `' . $type . '`) values (' . $day . ', 1)')->execute();
            }
    ...
    public static function version_file($file = "")
    {
        ...
        if($domain == 'www.choies.com' AND $https != 'on')
        {
            $file_dir =  DOCROOT;
    ...
    public function ready_shippeds($data_id)
    {
        ...
        if(!empty($cache_ready_shippeds_content) AND !isset($_GET['cache']))
        {
          ...
          $ready_shippeds = DB::query(Database::SELECT, 'select a.product_id from catalog_products as a,products as b where a.catalog_id = '.$data_id.' and a.product_id = b.id and b.visibility = 1 and b.status = 1 order by a.id desc limit 9')->execute()->as_array();$cacheins->set($cache_ready_shippeds_key, $ready_shippeds, 3600); 
          //设置手机版推荐产品
        }
    ...
    public static function getalltag($lang)
    {
        ...
        {
            if(!$lang){
            $result = DB::select()->from('tags')->order_by('position', 'DESC')
                    ->execute()->as_array();            
            }else{
            $result = DB::select()->from('tags'.'_'.$lang)->order_by('position', 'DESC')
                    ->execute()->as_array();               
            }
