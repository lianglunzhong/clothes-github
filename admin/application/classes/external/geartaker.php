 ...
 */
class External_Geartaker extends External
{
	...
	public function format($order)
	{
		...
		$customer = DB::select()->from('customers')->where('id', '=', $order['customer_id'])->execute()->current();
	...
	public function detail($ordernum)
	{
		...
		// order products
		$order_products = DB::select()->from('order_items')->where('order_id', '=', $order['id'])->execute()->as_array();
		if($order_products)
		{
			$i = 0;
		...
		// order shipments
		$order_shipments = DB::select()->from('order_shipments')->where('order_id', '=', $order['id'])->execute()->as_array();
		if($order_shipments)
		{
			$i = 0;
			foreach( $order_shipments as $order_shipment )
			{
				...
				$carrier = DB::select()->from('carriers')->where('carrier', '=', $order_shipment['carrier'])->execute()->current();
				$shipment['method'] = $carrier['carrier'].' ['.$carrier['carrier_name'].']';
		...
		// order payments
		$order_payments = DB::select()->from('order_payments')->where('order_id', '=', $order['id'])->execute()->as_array();
		if($order_payments)
		{
			$i = 0;
			foreach( $order_payments as $order_payment )
			{
				...
				$payment['status'] = $order_payment['payment_status'];
