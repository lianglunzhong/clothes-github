...
//            $isSMTP = TRUE;
        $lang = DB::select('lang')->from('customers')->where('site_id', '=', 1)->where('email', '=', $rcpt)->execute()->get('lang');
    ...
    public static function Sendcustomer($data, $mail_lang, $type, $from=NULL, $is_bcc=NULL, $output=FALSE, $isSMTP=false)
    {
        ...
        if( Kohana::config('webpower.'.$mail_lang.'.active.action')==1)
        {
            //WP-ORDER_UNPAID Mail
            ini_set('soap.wsdl_cache_enabled','0');
            ...
            try {
                $result = $client->addRecipients(   
                    ...
                    true
                        ...
                        if($result->status == "OK")
                        {
                            Kohana_log::instance()->add('-'.$type, $rcpt . ': OK');
                        }
                        elseif($result->status == "DUPLICATE")
                        {
                            Kohana_log::instance()->add('WP-'.$type, $rcpt . ': '.$result->statusMsg);   
        ...
        {
            echo 'connect webpower api faild';
            die;
        }
    }
}
