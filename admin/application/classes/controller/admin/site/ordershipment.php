...
class Controller_Admin_Site_Ordershipment extends Controller_Admin_Site
{
    ...
    public function action_data()
    {
        ...
        $result = DB::query(DATABASE::SELECT, 'SELECT COUNT(`orders`.`id`) AS num FROM `orders` WHERE `orders`.site_id=' . $this->site_id . ' AND `orders`.`parent_id` IS NULL AND '
        ...
        $result = DB::query(DATABASE::SELECT, 'SELECT `orders`.* FROM `orders` WHERE `orders`.site_id=' . $this->site_id . ' AND `orders`.`parent_id` IS NULL AND '
    ...
    function action_bulk()
    {
        ...
        while ($row = fgetcsv($handle))
        {
            ...
            {
                $order_shipping = DB::select('id')->from('order_shipments')->where('site_id', '=', $this->site_id)->and_where('ordernum', '=', $ordernum)->execute('slave')->as_array();
                if (count($order_shipping) > 1)
                {
                    DB::update('order_shipments')
                ...
                else
                {
                    ...
                    DB::insert('order_shipments', array_keys($shipment1))
...
//                        echo $order_no . '-' . $track_no . '-' . $track_link . '-' . $track_carrier;exit;
            ...
            {
                $shipping_date = DB::select('shipping_date')->from('orders')->where('ordernum', '=', $order_no)->execute('slave')->get('shipping_date');
                if ($shipping_date)
                {
                ...
                {
                    DB::update('orders')->set(array('shipping_date' => time()))->where('ordernum', '=', $order_no)->execute();
                    ...
                    // DB::update('order_items')->set(array('status' => 'shipped'))->where('order_id', '=', $order_id)->and_where('status', '=', 'new')->execute();
                    $items = array();
                ...
                if ($ret)
                {
                    ...
                    // DB::update('order_items')->set(array('status' => 'shipped'))->where('order_id', '=', $order_id)->execute();
    ...
    function action_bulk_price()
    {
        ...
        while ($row = fgetcsv($handle))
        {
            ...
            {
                $result = DB::update('order_shipments')
...
//			$date_end += 28800;
        ...
        foreach ($result as $order)
        {
            ...
            $shipping = DB::select('carrier', 'ship_date', 'ship_price', 'tracking_code')->from('order_shipments')->where('order_id', '=', $order['id'])->execute('slave')->current();
            echo date('Y-m-d', $shipping['ship_date']) . ',';
            echo $order['deliver_time'] ? date('Y-m-d', $order['deliver_time']) . ',' : ',';
            $products = DB::select('product_id')->from('order_items')->where('order_id', '=', $order['id'])->execute('slave')->as_array();
            $weight = empty($products) ? 0 : DB::select(array(DB::expr('SUM(weight)'), 'sum'))->from('products')->where('id', 'IN', $products)->execute('slave')->get('sum');
            echo $weight . ',';
    ...
    public function action_manual_update_shipment()
    {
        ...
        foreach($csv_items as $csv_item)
        {
            ...
            if ((!empty($ordernum)) && (!empty($tracking_no)) && (!empty($shipping_method))) {
                ...
                $order_shipments = DB::query(Database::SELECT, "SELECT `id`,`order_id`,`ordernum` FROM `order_shipments` WHERE `ordernum`='" . $ordernum . "'")->execute('slave')->as_array();
                if (count($order_shipments) > 0) {
                    $hasupdate_ordernum = $hasupdate_ordernum . $ordernum . "<br>";
                    continue;
                }
                $order = DB::query(Database::SELECT, "SELECT `id`,`ordernum`,`shipping_status` FROM `orders` WHERE `ordernum`='" . $ordernum . "'")->execute('slave')->as_array();
                if (count($order) > 0) {
                    $order_id = $order[0]['id'];
                    $order_items = DB::query(Database::SELECT, 'SELECT `id`,`sku`,`quantity` FROM `order_items` WHERE `order_id`=' . $order_id)->execute('slave')->as_array();
