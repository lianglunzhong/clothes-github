...
class Controller_Admin_Site_Data extends Controller_Admin_Site
{
    ...
    public function action_images()
    {
        ...
        foreach ($skus as $sku)
        {
            ...
            $result = DB::select('id')->from('products')
                    ->where('sku', '=', $sku)
                    ->where('site_id', '=', $site->get('id'))
                    ->execute('slave')->current();
...
//          }
        ...
        foreach( $items as $item_val )
        {
            ...
            if(in_array($id, $ids))
            {
                ...
                $cost_pro = DB::select('total_cost')->from('products')->where('id','=',$item_val['item_id'])->execute('slave')->current();
        ...
        foreach( $orders as $order_val )
        {
            ...
            if(in_array($id, $ids))
            {
                ...
                $show = DB::select()->from('celebrity_products')->where('ordernum','=',$order_val['ordernum'])->execute('slave')->current();
                if($show)
                {
                    $data[$id]['show_num'] += 1;
                }
...
//                  and '.$filter_sql.' ORDER BY '.$sidx.' '.$sord.' LIMIT '.$limit.' OFFSET '.$start;
        ...
        {
            // $id_pp = DB::select()->from('order_payments')->where('order_id','=',$value['id'])->where('payment_method','=','PPJump')
                // ->where('payment_status','=','refund')->execute('slave')->current();
            $stack = DB::select('tracking_code')->from('order_shipments')->where('order_id','=',$value['id'])->execute('slave')->current();
