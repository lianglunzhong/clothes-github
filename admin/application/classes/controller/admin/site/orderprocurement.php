...
//			$date_end += 28800;
        ...
        foreach ($result as $key => $procurement)
        {
            ...
            $price = DB::select('price')->from('order_items')
...
//                $limit = 20;
    ...
    public function action_bulk_instock()
    {
        ...
        while ($data = fgetcsv($handle))
        {
            ...
            $p_info = DB::select('id', 'set_id', 'attributes')->from('products')->where('sku', '=', trim($data[1]))->execute('slave')->current();
...
//            if (!$cost)
//                $cost = DB::select('total_cost')->from('products')->where('sku', '=', $data['sku'])->execute('slave')->get('total_cost');
//            $cost = round($cost, 2);
