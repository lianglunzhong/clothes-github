...
class Controller_Admin_Site_Api extends Controller_Admin_Site
{
    ...
    public function action_sendmail1()
    {
        ...
        foreach ($result as $order)
        {
            ...
            if ($update)
            {
                $update1 = DB::update('order_payments')->set(array('vip_status' => 1))->where('trans_id', '=', $order['trans_id'])->and_where('customer_id', '=', $order['customer_id'])->execute();
                if (!$update1)
                    continue;
...
//                 }
    ...
    {
        $products = DB::select()->from('products')->execute('slave')->as_array();
        $success = '';
        $amount = 0;
    ...
    public function action_sync_return()
    {
        if ($_POST)
        {
            ...
            foreach ($orders as $onum)
            {
                ...
                if ($order['amount'] <= 0)
                    continue;
                $payments = DB::select()->from('order_payments')->where('order_id', '=', $oid)->and_where('payment_status', '=', 'success')->and_where('amount', '>', 0)->execute('slave')->current();
                if (empty($payments))
                    continue;
...
';
    ...
    public function action_globebill_payment()
    {
        ...
        foreach($orders as $ordernum => $order)
        {
            ...
            if($insert)
            {
                DB::update('orders')->set(array('payment_status' => 'success'))->where('id', '=', $order_id)->execute();
        ...
        if ($_POST)
        {
            ...
            foreach ($orders as $onum)
            {
                ...
                if ($order['payment_status'] != 'pending')
                    continue;
                $payments = DB::select()->from('order_payments')->where('order_id', '=', $oid)->and_where('payment_status', '=', 'pending')->and_where('amount', '>', 0)->execute('slave')->current();
                if (empty($payments))
                    continue;
                DB::update('orders')->set(array('payment_status' => 'success'))->where('id', '=', $oid)->execute();
                DB::update('order_payments')->set(array('payment_status' => 'success'))->where('order_id', '=', $oid)->and_where('payment_status', '=', 'pending')->execute();
                $post_var = "order_num=" . $order['ordernum']
                    . "&order_amount=" . $payments['amount']
                    . "&order_currency=" . $payments['currency']
                    . "&card_num=" . $order['cc_num']
...
';
    ...
    {
        $celebritys = DB::select('id', 'email')->from('celebrits')->where('customer_id', '=', 0)->limit(500)->offset(0)->execute('slave');
        foreach ($celebritys as $cel)
        {
            $customer_id = DB::select('id')->from('customers')->where('email', '=', trim($cel['email']))->execute('slave')->get('id');
            if ($customer_id)
            {
                DB::update('celebrits')->set(array('customer_id' => $customer_id))->where('id', '=', $cel['id'])->execute();
            }
        }
    }
    ...
    public function action_products()
    {
        ...
        foreach ($catalogs as $catalog_id)
        {
            ...
            if (!$skus)
                exit;
            $skuArr = explode("\n", trim($skus));
            $products = DB::select('sku', 'description', 'attributes')->from('products')->where('sku', 'IN', $skuArr)->execute('slave');
...
//                                }
    ...
    {
        $date = 1373616279;
        $result = DB::select()->from('point_records')
            ->where('created', '>=', $date)
        ...
        {
            DB::query(Database::UPDATE, 'UPDATE customers SET points=points+' . $data['amount'] . ' WHERE id=' . $data['customer_id'])->execute();
    ...
    public function action_vip()
    {
        ...
        foreach ($customers as $id)
        {
            ...
            DB::update('customers')->set($update)->where('id', '=', $id)->execute();
        }
    }
    ...
    public function action_customers()
    {
        ...
        {
            $ip = DB::select('ip')->from('orders')->where('customer_id', '=', $customer['id'])->and_where('ip', 'NOT IN', array(0, 2147483647))->execute('slave')->get('ip');
            if ($ip)
            {
                echo 'UPDATE customers SET ip=' . $ip . ' WHERE id=' . $customer['id'];
                exit;
                DB::query(Database::UPDATE, 'UPDATE customers SET ip=' . $ip . ' WHERE id=' . $customer['id'])->execute();
...
//                                DB::delete('giveaway')->where('id', '=', $comment['id'])->execute();
    ...
    public function action_vip1()
    {
        ...
        $currency = Site::instance()->currencies();
        $vips = DB::select()->from('vip_types')->execute('slave')->as_array();
        $result = DB::select('id', 'order_total')->from('customers')->where('vip_level', '=', 2)->limit(500)->offset(1000)->execute('slave');
        foreach ($result as $c)
        {
            ...
            if ($total - $c['order_total'] > 2)
            {
                ...
                $update = DB::update('customers')->set($data)->where('id', '=', $c['id'])->execute();
                if ($update)
                    echo $c['id'] . '<br>';
            }
        }
...
//                var_dump($result);exit;
        foreach ($result as $order)
        {
            ...
            {
                $admin = DB::select('admin')->from('celebrits')->where('email', '=', $order['email'])->execute('slave')->get('admin');
                $amount = 0;
                $products = DB::select('product_id')->from('order_items')->where('order_id', '=', $order['id'])->and_where('status', '<>', 'cancel')->execute('slave');
                foreach ($products as $p)
                {
    ...
    {
        $result = DB::query(Database::SELECT, 'SELECT * FROM `orders` WHERE `payment_method` = "PPEC" ORDER BY `customer_id` ASC')->execute('slave');
        $customer = 0;
        $success = array();
        foreach ($result as $data)
        {
            ...
            if (!$data['shipping_address'])
                continue;
            $has_add = DB::select('id')->from('addresses')->where('customer_id', '=', $data['customer_id'])->execute('slave')->get('id');
            if (!$has_add)
            {
                $add = array(
                    'site_id' => 1,
                ...
                DB::insert('addresses', array_keys($add))->values($add)->execute();
    ...
    {
        $result = DB::select('id', 'sku')->from('products')->where('set_id', '=', 374)->execute('slave');
...
';
    ...
    public function action_catalog_products_cout()
    {
        ...
        $catalogs = DB::select('id')->from('catalogs')->where('visibility', '=', 1)->where('on_menu', '=', 1)->execute('slave');
    ...
    public function action_product_all_buy()
    {
        if($_POST)
        {
            ...
            $skuArr = explode("\n", $skus);
            $products = DB::select('id', 'sku', 'name', 'attributes', 'description', 'configs')->from('products')->where('sku', 'IN', $skuArr)->execute('slave');
            foreach($products as $product)
            {
...
';
    ...
    public function action_product_all_buy1()
    {
        if($_POST)
        {
            ...
            {
                $product = DB::select('id', 'sku', 'name', 'attributes', 'description', 'brief', 'keywords', 'configs')->from('products')->where('sku', '=', trim($sku))->execute('slave')->current();
                if(empty($product))
                    continue;
...
';
    ...
    public function action_not_main_catalog_products()
    {
        ...
        $products = DB::query(Database::SELECT, 'SELECT id, sku FROM `products` WHERE visibility = 1 AND status = 1 AND stock <> 0')->execute('slave');
        foreach($products AS $p)
        {
...
//sku修改图片时通知wholesale
    ...
    public function action_test_order()
    {
        ...
        foreach ($orders as $o)
        {
            ...
            $remark = array();
            $remarks = DB::select('remark')->FROM('order_remarks')->WHERE('order_id', '=', $o['id'])->execute();
            ...
            $admin_id = DB::select('admin')->from('celebrits')->where('email', '=', $o['email'])->execute()->get('admin');
            if ($admin_id)
    ...
    public function action_simple_spromotion_memcache()
    {
        if($_POST)
        {
            ...
            $skus = explode("\n", $string);
            $product_ids = DB::select('id', 'sku')->from('products')->where('sku', 'in', $skus)->execute('slave');
            $cache = Cache::instance('memcache');
