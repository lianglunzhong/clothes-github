...
/*        $maparray = array(
    ...
    public function action_data()
    {
        ...
        foreach ($result as $review)
        {
            ...
            $review['sku']=DB::select('sku')->from('products')->where('id', '=', $review['product_id'])->execute()->get('sku');
    ...
    public function action_transbanner() 
    {
        $result = DB::select()->from('banners')
            ->where('visibility', '=', 1)
            ->where('lang', 'in', array('de','fr'))
            ->where('type', '=', 'phonecatalog')
            ->order_by('id', 'DESC')
            ->limit(14)
        ...
        {
            $t .= $value['alt'].'+'.$value['title'].'+'; 
        }

        $t = substr($t, 0,-1);
        $words1 = Product::googletransapi($blank='en',$target=$lang,$t);

        if($words1 != 1)
        {
            $words1 = json_decode($words1);
            $words1 = $words1->data->translations[0]->translatedText;
            #$words1 = '1111+2222222222+3333+4+5+6+7+8+9+10+11+12';
            ...
            if($replarr1)
            {
                ...
                {
                    if(count($replarr1) / 2 == count($result))
                    {
                        $update = DB::update('banners')->set(array('alt' => $replarr1[$i],'title' => $replarr1[$i+1]))->where('id', '=', $value['id'])->execute();

                        $i +=2;
                    }
                    else
                    {
                        echo 'error ,翻译数量跟banner数量不匹配';
                    }
                }
            }
        }
