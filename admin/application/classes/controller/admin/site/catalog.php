...
class Controller_Admin_Site_Catalog extends Controller_Admin_Site
{
    ...
    public function action_edit()
    {
        ...
        //判断是三级分类才出现选择是否显示套话
        $has_child = DB::select()->from('catalogs')->where('parent_id', '=', $id)->execute('slave')->count();
...
/*            $ret2 = DB::update('products')->set(array('position' => $po))
   ...
   public  function action_products_zero13()
    ...
    public function action_delete()
    {
        ...
        $id = $this->request->param('id');
        $children = DB::select('id')->from('catalogs')->where('parent_id', '=', $id)->execute('slave')->as_array();
        if (count($children))
        {
    ...
    public function action_export()
    {
        ...
        if ($catalogs == 'roots')
        {
            $cids = DB::select('id')->from('catalogs')
                    ->where('site_id', '=', $this->site_id)
        ...
        elseif ($subcatalog)
        {
            ...
            foreach ($pids as $pid)
            {
                $tmp = DB::select('id')->from('catalogs')
                        ->where('site_id', '=', $this->site_id)
...
//                                {
//                                        $catalog_id = DB::select('id')->from('catalogs')->where('name', '=', $item->data)->execute()->get('id');
...
//                                }
    ...
    public function action_sorts_add()
    {
        if ($_POST)
        {
            ...
            $catalog_id = DB::select('id')->from('catalogs')->where('name', '=', trim($catalog))->execute('slave')->get('id');
            if (!$catalog_id)
            {
...
//                    $data[$key] = iconv('gbk', 'utf-8', $val);
                    ...
                    else
                    {
                        ...
                        $name = trim($data[0]);
                        $catalog_id = DB::select('id')->from('catalogs')->where('name', '=', $name)->execute('slave')->get('id');
    ...
    public function action_brands_list()
    {
        if($_POST)
        {
            ...
            else
            {
                ...
                $has = DB::select('id')->from('brands')->where('label', '=', $label)->execute('slave')->get('id');
                if($has)
                {
                ...
                else
                {
                    ...
                    DB::insert('brands', array_keys($data))->values($data)->execute();
                    Message::set('Add brand success', 'success');
                }
            }
        }
        $brands = DB::select()->from('brands')->order_by('id', 'DESC')->execute('slave')->as_array();
    ...
    public function action_brandcsv()
    {
        ...
        foreach($arr as $value)
        {
            ...
            if(!$has)
            {
                ...
                $add = DB::insert('brands', array_keys($array))->values($array)->execute();
                $success[] = $data[0];                        
            }
        ...
        if($_POST)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    DB::update('brands')->set($data)->where('id', '=', $id)->execute();
    ...
    public function action_brand_delete()
    {
        ...
        if($id)
        {
            DB::delete('brands')->where('id', '=', $id)->execute();
