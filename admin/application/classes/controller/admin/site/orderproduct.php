...
//                $sord = '';
        ...
        $result = DB::query(DATABASE::SELECT, 'SELECT COUNT(O.`id`) AS num FROM `orders` O LEFT JOIN customers C ON O.customer_id = C.id WHERE C.is_vip >= 1 AND O.is_active=1 AND O.site_id=' . $this->site_id . $activesql . ' AND '
        ...
        $result = DB::query(DATABASE::SELECT, 'SELECT O.id,O.ordernum,O.email,O.created,O.verify_date,O.shipping_date,O.payment_status,O.shipping_status,O.refund_status,O.currency,O.amount,O.payment_method,O.deliver_time,O.lang,C.firstname,C.lastname,C.is_vip 
            FROM `orders` O LEFT JOIN customers C ON O.customer_id = C.id WHERE C.is_vip >= 1 AND O.is_active=1 AND  O.site_id=' . $this->site_id . $activesql . ' AND '
	...
	public function action_export_vipdata()
	{
		if($_POST){
            ...
            FROM `orders` O LEFT JOIN customers C ON O.customer_id = C.id WHERE C.vip_level > 1 AND O.is_active=1 AND '.implode(' AND ',$sql).' ')->execute('slave')->as_array();
			foreach ($result as $value)
			{
				$namecount = '';
    ...
    public function action_message()
    {
        if($_POST)
        {
            ...
            if($message_id)
            {
                ...
                if($set_read == 'on')
                {
                    DB::update('order_messages')->set(array('status' => 1))->where('id', '=', $message_id)->execute();
                    Message::set('Set message read success');
                }
                else
                {
                    DB::update('order_messages')->set(array('status' => 0))->where('id', '=', $message_id)->execute();
...
//                $sord = '';
        ...
        if ($filters)
        {
            foreach ($filters->rules as $item)
            {
                ...
                {
                    $order_id = DB::select("id")->from('orders')->where('ordernum', '=', $item->data)->execute('slave')->get('id');
        ...
        $result = DB::query(DATABASE::SELECT, "SELECT COUNT(M.id) AS num FROM `order_messages` M LEFT JOIN `orders` O ON M.order_id = O.id WHERE M.message != '' and " . $filter_sql)->execute();
        $count = $result[0]['num'];
        $total_pages = 0;
        if ($count > 0)
        {
        ...
        $result = DB::query(DATABASE::SELECT, "SELECT M.id, O.ordernum, M.message, M.created, M.status FROM `order_messages` M LEFT JOIN `orders` O ON M.order_id = O.id WHERE M.message != '' and " . $filter_sql 
        ...
        if($month_get)
        {
            ...
            foreach($result AS $order)
            {
                if($order['status'] == 'cancel')
                    continue;
                $product = DB::select('set_id', 'total_cost')->from('products')->where('id', '=', $order['product_id'])->execute('slave')->current();
                if(empty($product))
                    continue;
                ...
                elseif(!isset($country_data[$order['shipping_country']]))
                {
                    ...
                    $shipping_price = DB::select('ship_price')->from('order_shipments')->where('order_id', '=', $order['id'])->execute('slave')->get('ship_price');
