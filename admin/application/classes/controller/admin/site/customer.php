...
class Controller_Admin_Site_Customer extends Controller_Admin_Site
{
    ...
    public function action_list()
    {
        ...
        if( ! ($customers_statistics = $cache->get($key)))
        {
            ...
            for ($i = 0; $i < 7; $i++)
            {
                ...
                $today_register = DB::query(Database::SELECT, "SELECT COUNT(customers.id) AS count FROM `customers`
                                            WHERE flag=0 AND site_id =" . $this->site_id . " AND created >= " . $midnight . " AND created < " . $next_day)
                    ->execute('slave')->get('count');

                $today_login = DB::query(Database::SELECT, "SELECT COUNT(customers.id) AS count FROM `customers`
                                            WHERE 	last_login_time >= " . $midnight . " AND 	last_login_time < " . $next_day)
                    ->execute('slave')->get('count');
    ...
    public function action_edit_basic_address()
    {
        ...
        foreach($arr as $k=>$v){
            ...
            DB::update('addresses')->set($data)->where('id', '=', $k)->execute();
        }
                    message::set('已修改');
            $this->request->redirect('admin/site/customer/list');
      }
    ...
    public function action_data()
    {
        ...
        {
			$users_email = DB::select('name')->from('users')->where('id','=',$customer['users_admin'])->where('created', '>', 0)->execute('slave')->get('name');
    ...
    public function action_export1()
    {
        if ($_POST)
        {
            ...
            $query = DB::select('created', 'email', 'firstname', 'lastname')->from('customers');
            $site = Site::instance();
    ...
    public function action_export()
    {
        if ($_POST)
        {
            ...
            if($submit == 'Export')
            {
                ...
                $customers = DB::query(Database::SELECT, 'SELECT `customers`.* FROM `customers` WHERE `customers`.site_id=' . $this->site_id . $sql)
                    ->execute('slave');
                ...
                foreach ($customers as $customer)
                {
                    if ($customer['country'])
                        ...
                        $country = DB::select('country')->from('addresses')->where('customer_id', '=', $customer['id'])->execute('slave')->get('country');
...
'DRES0422B037W',
    ...
    public function action_export_vip()
    {
        ...
        $vips = DB::select('email', 'vip_level', 'points')->FROM('customers')->where('vip_level', '>', 0)->order_by('vip_level')->order_by('id')->execute();
        foreach($vips as $vip)
        {
            echo $vip['email'] . ',';
            echo $vip['vip_level'] . ',';
    ...
    public function action_orders_data()
    {
        ...
        foreach ($result as $customer)
        {
            $orders = DB::select('amount', 'rate')->from('orders')->where('customer_id', '=', $customer['id'])->and_where('payment_status', '=', 'verify_pass')->execute('slave')->as_array();
            $order_total = 0;
    ...
    public function action_vip_add()
    {
        ...
        foreach ($result as $order)
        {
            ...
            {
                DB::update('order_payments')->set(array('vip_status' => 1))->where('id', '=', $order['id'])->execute();
                continue;
            }
            ...
            $update = DB::update('customers')->set(array('order_total' => $add_amount))->where('id', '=', $order['customer_id'])->execute();
            if ($update)
            {
                $update1 = DB::update('order_payments')->set(array('vip_status' => 1))->where('customer_id', '=', $order['customer_id'])->and_where('trans_id', '=', $order['trans_id'])->execute();

                if (!$update1)
                    continue;
                $c_update = DB::update('customers')->set(array('vip_level' => $level))->where('id', '=', $order['customer_id'])->execute();
...
Choies Team
        ...
        foreach ($result as $order)
        {
            ...
            $update = DB::update('customers')->set($data)->where('id', '=', $order['customer_id'])->execute();
            if ($update)
            {
                DB::update('order_payments')->set(array('vip_status' => 1))->where('id', '=', $order['id'])->execute();
            }
...
//                        WHERE id = 3')->execute('slave');
        foreach ($births as $customer)
        {
            ...
            if (!$send_day OR $send_day - time() > 31536000)
            {
                ...
                $update = DB::query(Database::UPDATE, 'UPDATE customers SET points=points+' . $points . ',give_points=1 WHERE id=' . $customer['id'])->execute();
                if ($update)
                {
...
//                if (strpos($rcpt, '@hotmail.') or strpos($rcpt, '@live.'))
    ...
    public function action_coupon_mail(){
        $sql = "select C.id,M.email,C.code,M.firstname,C.expired    
            from coupons C , customer_coupons L , customers M 
        ...
        foreach ($result as $value) {
            ...
            DB::query(DATABASE::UPDATE,"update coupons set is_mailed=1 where id=".$value['id'])->execute();
    ...
    public function action_update_country()
    {
        ...
        foreach($customers as $c)
        {
            ...
            if($country_code)
                DB::update('customers')->set(array('ip_country' => $country_code))->where('id', '=', $c['id'])->execute();
            else
                DB::update('customers')->set(array('ip_country' => '-'))->where('id', '=', $c['id'])->execute();
        }
    ...
    public function action_vip()
    {
        if($_POST)
        {
            ...
            else
            {
                ...
                foreach ($customers as $customer)
                {
                    $orders = DB::select('id', 'created')->from('orders')->where('customer_id', '=', $customer['id'])->where('payment_status', 'IN', array('verify_pass', 'success'))->where('refund_status', '=', '')->execute('slave');
                    $last_order_date = 0;
                    $order_qty = 0;
                    foreach($orders as $order)
                    {
...
public function action_output_customer_by_order_category_once()
    ...
    public function action_newvip(){
		/*
			...
			*/
			...
			//SELECT customer_id,sum(amount/rate) as sum_amount FROM `orders` where payment_status="verify_pass" and is_active=1 and verify_date BETWEEN unix_timestamp('2015-01-01') AND unix_timestamp('2015-02-01') GROUP BY customer_id HAVING(sum_amount >= 199) ORDER BY customer_id
			$result = DB::query(Database::SELECT, 'SELECT id,customer_id,email,sum(amount/rate) as sum_amount FROM `orders` where payment_status="verify_pass" and refund_status<>"refund" and is_active=1 and '.implode(' AND ',$sql).' GROUP BY customer_id HAVING(sum_amount >= 199) ORDER BY customer_id')->execute('slave')->as_array();
			echo "总数为".count($result);
			//$result = DB::query(Database::SELECT, 'SELECT a.email as email,sum(a.count) as count FROM (select email,amount * rate count from `orders` where payment_status="verify_pass" and is_active=1 and '.implode(' AND ',$sql).' ORDER BY id ) as a where a.count >= 199  group by a.email ')->execute('slave')->as_array();
			//查询这段时间的新vip用户
            $user_new=array();
			$data['is_vip']=1;
			$data['vip_start']=time();
			...
			foreach($result as $k=>$v){
				if($v['customer_id'] && $v['sum_amount']>=199){
					//echo "ok";
					DB::update('customers')->set($data)->where('id', '=', $v['customer_id'])->execute();
				}
			}
			echo "ok";
        exit;
	...
	public function action_newvip1()
    {
		...
		*/
		...
		$result = DB::query(Database::SELECT, 'SELECT customer_id,email,sum(amount/rate) as sum_amount FROM `orders` where payment_status="verify_pass" and refund_status<>"refund" and is_active=1 and verify_date>'.$startvip.' GROUP BY customer_id HAVING(sum_amount >= 199) ORDER BY customer_id limit '.$limit.' offset '.($page - 1) * $limit.' ')->execute('slave')->as_array();
		foreach($result as $k=>$v){
			if($v['customer_id'])
			{
				$vip_ok=DB::query(Database::SELECT, 'SELECT id,ordernum,verify_date FROM `orders` where customer_id='.$v['customer_id'].' and payment_status="verify_pass" and refund_status<>"refund" and is_active=1 ORDER BY id desc limit 1')->execute('slave')->current();
				//去年下单时间
				if($vip_ok['verify_date']>strtotime('2016-01-01 00:00:00')){
					...
					$result1 = DB::query(Database::SELECT, 'SELECT id,customer_id,email,sum(amount/rate) as sum_amount FROM `orders` where customer_id='.$v['customer_id'].' and payment_status="verify_pass" and refund_status<>"refund" and is_active=1 and verify_date > '. $last_time .' and  verify_date <= '. $vip_ok['verify_date'] .'')->execute('slave')->current();
		...
		foreach($result as $k=>$v){
			if($v['customer_id'])
			{
				...
				if($vip_ok['verify_date']>strtotime('2016-01-01 00:00:00')){
					$last_time=strtotime(date('Y',$vip_ok['verify_date'])-1 .'-'.date('m-d H:i:s',$vip_ok['verify_date']));
					$result1 = DB::query(Database::SELECT, 'SELECT id,customer_id,verify_date,email,sum(amount/rate) as sum_amount FROM `orders` where customer_id='.$v['customer_id'].' and payment_status="verify_pass" and refund_status<>"refund" and is_active=1 and verify_date > '. $last_time .' and  verify_date <= '. $vip_ok['verify_date'] .'')->execute('slave')->current();
		...
		foreach($over_time as $k1=>$v1){
			if($v1['is_vip']>=1){
				$up1 .=$v1['email']."<br/>";
				DB::update('customers')->set(array('is_vip'=>0))->where('id', '=', $v1['id'])->execute();
			}
		}
		*/
	...
	**/
	public function action_wpvipsendemail()
	{
		...
		foreach($vipuserarr as $k=>$v){
			if($v['is_vip']>0){
				$vip_ok=DB::query(Database::SELECT, 'SELECT id,ordernum,verify_date FROM `orders` where customer_id='.$v['id'].' and payment_status="verify_pass" and refund_status<>"refund" and is_active=1 ORDER BY id desc limit 1')->execute('slave')->current();
				$last_time=strtotime(date('Y',$vip_ok['verify_date'])-1 .'-'.date('m-d H:i:s',$vip_ok['verify_date']));
				$sum_amount=DB::query(Database::SELECT, 'SELECT email,sum(amount/rate) as sum_amount FROM `orders` where customer_id='.$v['id'].' and payment_status="verify_pass" and refund_status<>"refund" and is_active=1 and verify_date>'.$last_time.' ')->execute('slave')->as_array();
	...
	*/
	public function action_getvipall()
	{
		...
		foreach($customers as $k=>$v){
			...
			$users_email = DB::select('name')->from('users')->where('id','=',$v['users_admin'])->where('created', '>', 0)->execute('slave')->get('name');
			echo $users_email, ',';
			if($v['users_update']){
	...
	public function action_getvipallone()
	{
		...
		if(count($result)>0){
			foreach($result as $k1=>$v1){
				...
				$users_email = DB::select('name')->from('users')->where('id','=',$v1['users_admin'])->where('created', '>', 0)->execute('slave')->get('name');
				echo $users_email, ',';
	...
	public function action_user_admin()
	{
        if ($_POST)
        {
			...
			//获取负责人id
			$user_id = DB::select('id')->from('users')->where('email','=',$user_email)->where('created', '>', 0)->execute('slave')->get('id');
            if (!$emailArr)
            {
                Message::set('Email cannot be empty!', 'error');
                $this->request->redirect('/admin/site/customer/list');
            ...
            foreach ($emails as $email)
            {
                ...
                if ($customer_id)
                {
					...
					if(!$customer_admin){
						...
						DB::update('customers')->set(array('users_admin' => $user_id,'users_admin_main' => $users_admin_main['name'],'users_update' => time()))->where('id', '=', $customer_id)->execute();
					}else{
						DB::update('customers')->set(array('users_admin' => $user_id,'users_admin_main' => $users_admin_main['name'],'users_update' => time()))->where('id', '=', $customer_id)->execute();
					}
                }
	...
	public function action_getemailorder()
	{
		if($_POST){
			...
			echo "email,amount,rate,payment_status,count,verify_date\n";
				...
				$result = DB::query(Database::SELECT, 'select email,amount,rate,payment_status,amount / rate count,verify_date from `orders` where email = "'.$v.'" and payment_status="verify_pass" and is_active=1 and '.implode(' AND ',$sql).' ')->execute('slave')->as_array();
					if(count($result)>0){
						foreach($result as $k1=>$v1){
						//开始导表
						echo $v1['email'], ',';
	...
	public function action_get1emailorder()
	{
		if($_POST){
			...
			$result = DB::query(Database::SELECT, 'select ordernum,email,sum(amount/rate) as amount1,payment_status,verify_date from `orders` where payment_status="verify_pass" and is_active=1 and '.implode(' AND ',$sql).'  GROUP BY email ORDER BY email')->execute('slave')->as_array();
			if(count($result)>0){
					foreach($result as $k1=>$v1){
					//开始导表
					echo $v1['ordernum'], ',';
	...
	public function action_getadminorder()
	{
		if($_POST){
			...
			$result = DB::query(Database::SELECT, 'select o.ordernum,o.email,c.users_update,o.amount / o.rate count,c.users_admin,o.payment_status,o.verify_date from `orders` o  LEFT JOIN customers c ON o.customer_id=c.id where o.payment_status="verify_pass" and c.users_admin>0 and o.is_active=1 and '.implode(' AND ',$sql).'  ORDER BY email')->execute('slave')->as_array();
			if(count($result)>0){
					foreach($result as $k1=>$v1){
					//开始导表
        ...
        if($_POST){
            ...
            foreach ($emails as $email) {
                ...
                if ($customer_id) {
                    ...
                    if($flag!=3) {
                        $update = DB::update('customers')->set(array('flag' => 3))->where('id', '=', $customer_id)->execute();
                        if($update == 0){
    ...
    public function action_add_cart()
    {
        ...
        echo "sku,add_qty\n";
        //查找15天内下心愿单的信息
        $sql = 'select item_id,count(id) as con from cartproduct where created<'.$to_date.' and created>='.$from_date.' and is_cart = 1 group by item_id order by con desc limit 0,300';
    ...
    public function action_webpowerload()
    {
        if($_POST)
        {
            ...
            if (!$email or !$type)
            {
                echo 'Email or type cannot be empty';
                exit;
            }
            else 
            {
                ...
                {
                    $customer_id = Customer::instance()->is_register($e);
                    if($customer_id)
                    {   #Newsletter订阅
                        $array = array();
                        $firstname = Customer::instance($customer_id)->get('firstname');
                        $name = !empty($firstname) ? $firstname : 'Customer';
                        $array = array(
                                array('name'=>'email','value'=>$e),
                                array('name'=>'firstname','value'=>$name)
                            );
                        $postarr[]['fields'] = $array;
                    }
/*                    elseif($customer_id && $type ==99)
                    {
                        #注册欢迎
                        $array = array();
                        $firstname = Customer::instance($customer_id)->get('firstname');
                        $coupon_code = 'SIGNUP15OFF' . $customer_id;
                        $coupon = Coupon::instance($coupon_code)->get();
                        $expired = '';
                        ...
                        $array = array(
                                array('name'=>'email','value'=>$e),
                                array('name'=>'coupon_code','value'=>$coupon_code),
                                array('name'=>'expired','value'=>$expired)

                            );
                        $postarr[]['fields'] = $array;
                    }
                    elseif($customer_id && $type ==94)
                    {
                        #支付成功组
                        $array = array();
                        $firstname = Customer::instance($customer_id)->get('firstname');
...
/*                echo '<pre>';
                ...
                $mail_lang = 'active';
                $send = Mail::Sendcustomer($postarr,$mail_lang,$type);
        ...
        else
        {
            echo
            '
    <h3>批量传输数据到webpower平台</h3>
    <form method="post">
        <textarea name="email" cols="20" rows="20"></textarea>
        <br /><br />
        <select name="type">
            <option value="">请选择</option>
            <option value="99">新版 注册欢迎组</option>
            <option value="94">新版 支付成功组</option>
            <option value="88">11.10 新版 Newsletter订阅邮件组</option>
        </select>
        <br /><br />
