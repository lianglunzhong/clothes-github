...
//            if(strpos($rcpt, 'hotmail.com'))
    ...
    public function action_order_item_outstock()
    {
        if($_POST)
        {
            ...
            {
                $orderData = DB::select('ordernum', 'email', 'shipping_firstname', 'payment_status')->from('orders')->where('id', '=', $order_id)->execute('slave')->current();
                if($orderData['payment_status'] == 'success' OR $orderData['payment_status'] == 'verify_pass')
                {
                    ...
                    foreach($itemsArr as $item_id)
                    {
                        ...
                        if($item_id)
                        {
                            $update = DB::update('order_items')->set(array('status' => 'cancel', 'erp_line_status' => '缺货-' . $admin))->where('id', '=', $item_id)->execute();
                            if($update)
                            {
                                $updateItems[] = DB::select('name', 'sku', 'price', 'attributes','quantity')->from('order_items')->where('id', '=', $item_id)->execute('slave')->current();
                            }
                        }
                    }
