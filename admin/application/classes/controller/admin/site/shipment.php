...
class Controller_Admin_Site_Shipment extends Controller_Admin_Site
{
    ...
    private function construct()
    {
        ...
        $data = DB::select()->from('order_shipments')
            ->where('id', '=', $id)
            ->execute('slave')
            ->current();
        if (!$data['id'])
    ...
    public function action_data()
    {
        ...
        $result = DB::query(DATABASE::SELECT, 'SELECT COUNT(`order_shipments`.`id`) AS num FROM `order_shipments` WHERE '
        ...
        $result = DB::query(DATABASE::SELECT, 'SELECT `order_shipments`.* FROM `order_shipments` WHERE '
