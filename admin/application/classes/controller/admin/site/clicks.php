...
//                $limit = 20;
        ...
        foreach ($result as $data)
        {
            ...
            $cele_c_num = DB::query(Database::SELECT, 'SELECT COUNT(orders.id) AS count FROM `orders`,celebrits 
            ...
            $cele_p_num = DB::query(Database::SELECT, 'SELECT COUNT(orders.id) AS count FROM `orders`,celebrits 
                                        WHERE orders.email = celebrits.email AND orders.is_active = 1 AND orders.erp_fee_line_id=1
            ...
            $cele_ppc_num = DB::query(Database::SELECT, 'SELECT COUNT(orders.id) AS count FROM `orders`,celebrits 
                                        WHERE orders.email = celebrits.email AND orders.is_active = 1 AND orders.erp_fee_line_id=1
...
//                $limit = 20;
    ...
    public function action_brand_data()
    {
        ...
        foreach ($result as $data)
        {
            ...
            $total_skus = DB::select(DB::expr('COUNT(id) as count'))->from('products')
            ...
            {
                $total_clicks = DB::select(DB::expr('SUM(clicks+quick_clicks) AS sum_clicks'))->from('products')
    ...
    public function action_brand_account_data()
    {
        ...
        $count = DB::query(DATABASE::SELECT, 'SELECT COUNT(i.id) as count FROM `order_items` i LEFT JOIN orders o ON i.order_id=o.id RIGHT JOIN products p ON i.product_id=p.id 
        ...
        $result = DB::query(DATABASE::SELECT, 'SELECT o.ordernum, i.id, i.order_id, i.sku, i.name, i.quantity, i.attributes, i.status, o.email, o.amount, o.currency, i.price, p.price AS orig_price, p.cost, p.total_cost, p.weight, o.verify_date, o.points, o.amount_coupon 
                                FROM `order_items` i LEFT JOIN orders o ON i.order_id=o.id RIGHT JOIN products p ON i.product_id=p.id 
        ...
        foreach ($result as $data)
        {
            if(!isset($sys_currencies[$data['currency']]))
                continue;
            $admin = '';
            $cel = DB::select('admin')->from('celebrits')->where('email', '=', $data['email'])->execute('slave')->get('admin');
            if ($cel)
            ...
            if ($data['amount'] > 0 AND $admin == '' AND !in_array($data['status'], array('cancel', 'return')))
            {
                if ($data['points'] OR $data['amount_coupon'])
                {
                    ...
                    $total_price = DB::select(DB::expr('SUM(price * quantity) AS total'))->from('order_items')->where('order_id', '=', $data['order_id'])->execute('slave')->get('total');
