...
//                            user::instance($data['admin'])->get('name'),
    ...
    public function action_add()
    {
        if ($_POST)
        {
            ...
            $result = DB::select('id')->from('celebrits')->where('email', '=', $data['email'])->execute('slave')->get('id');
            if ($result)
            {
    ...
    public function action_edit()
    {
        ...
        if ($_POST)
        {
            ...
            $data['admin'] = Session::instance()->get('user_id');
            $data['updated'] = time();
            $customer_id = DB::select('id')->from('customers')->where('email', '=', $data['email'])->execute('slave')->get('id');
            if($customer_id)
                $data['customer_id'] = $customer_id;
            $result = DB::update('celebrits')->set($data)->where('id', '=', $id)->execute();
        ...
        $celebrity = DB::select()->from('celebrits')->where('id', '=', $id)->and_where('is_able', '=', 1)->execute('slave')->current();
        $admin = $celebrity['admin'];
        $user = Session::instance()->get('user_id');
        $parent = DB::select('parent_id')->from('users')->where('id', '=', $admin)->execute('slave')->get('parent_id');
    ...
    {
        $result = DB::update('celebrits')->set(array('is_able' => 0))->where('id', '=', $id)->execute();
        if ($result)
        {
    ...
    {
        $result = DB::update('celebrits')->set(array('is_able' => 1))->where('id', '=', $id)->execute();
        if ($result)
        {
    ...
    public function action_delete($id)
    {
        ...
        DB::delete('celebrity_products')->where('celebrity_id', '=', $id)->execute();
        $result = DB::delete('celebrits')->where('id', '=', $id)->execute();
        if ($result)
        {
    ...
    public function action_assign()
    {
        if ($_POST)
        {
            ...
            {
                $result = DB::update('celebrits')->set(array('admin' => $user))->where('id', '=', $celebrity_id)->execute();
                if ($result)
                {
    ...
    public function action_bulk_assign()
    {
        if ($_POST)
        {
            ...
            else
            {
                ...
                $admins = DB::select('id')->from('users')->where('parent_id', '=', $user_id)->execute('slave')->as_array();
                foreach ($ids as $id)
                {
                    $c_admin = DB::select('admin')->from('celebrits')->where('id', '=', $id)->execute('slave')->get('admin');
                    $admin = array('id' => $c_admin);
                    if (in_array($admin, $admins) OR $c_admin == $user_id)
                    {
                        DB::update('celebrits')->set(array('admin' => $user))->where('id', '=', $id)->execute();
                    }
                }
                Message::set('Assign celebrity success!', 'success');
                $this->request->redirect('admin/site/celebrity/list');
    ...
    public function action_email_assign()
    {
        if ($_POST)
        {
            ...
            else
            {
                ...
                $user_id = Session::instance()->get('user_id');
                //$admins = DB::select('id')->from('users')->where('parent_id', '=', $user_id)->execute('slave')->as_array();
                $admins = DB::select('id')->from('users')->where('active', '=', 1)->execute('slave')->as_array();
                $user_arr = array('id' => $user);
                foreach ($emails as $email)
                {
                    $c_admin = DB::select('id', 'admin')->from('celebrits')->where('email', '=', $email)->execute('slave')->current();
                    ...
                    {
                        DB::update('celebrits')->set(array('admin' => $user))->where('id', '=', $c_admin['id'])->execute();
                    }
                }
                Message::set('Assign celebrity success!', 'success');
                $this->request->redirect('admin/site/celebrity/list');
    ...
    public function action_order_data()
    {
        ...
        if ($filters)
        {
            foreach ($filters->rules as $item)
            {
                ...
                elseif ($item->field == 'admin')
                {
                    $filter_data = DB::select('id')->from('users')->where('name', '=', $item->data)->execute('slave')->current();
        ...
        elseif ($_GET)
        {
            foreach ($_GET as $key => $val)
            {
                ...
                elseif ($key == 'admin')
                {
                    $filter_data = DB::select('id')->from('users')->where('name', '=', $val)->execute('slave')->current();
                    $filter_sql .= " AND celebrits.admin='" . $filter_data['id'] . "'";
                }
                elseif ($key == 'url')
                {
        ...
        {
            $shipment = DB::select('tracking_code', 'ship_date')->from('order_shipments')->where('ordernum', '=', $data['ordernum'])->execute('slave')->current();
...
//                                $admin = User::instance($c['admin'])->get('name');
                $products = DB::select('sku')->from('order_items')->where('order_id', '=', $c['id'])->execute('slave')->as_array();
                $track_code = DB::select('tracking_code')->from('order_shipments')->where('ordernum', '=', $c['ordernum'])->execute('slave')->get('tracking_code');
                foreach ($products as $product)
                {
                    $urls = DB::select('url')->from('celebrity_products')->where('ordernum', '=', $c['ordernum'])->and_where('sku', '=', $product['sku'])->execute('slave')->as_array();
                    if (count($urls) > 0)
                    {
                        foreach ($urls as $url)
                        {
    ...
    public function action_edit_url()
    {
        if ($_POST)
        {
            ...
            {
                DB::delete('celebrity_products')->where('celebrity_id', '=', $_POST['celebrity_id'])->and_where('url', '=', $d_url)->execute();
            }
            ...
            if (!empty($urlArr))
            {
                ...
                {
                    $url = trim($url);
                    $result = DB::select('url')->from('celebrity_products')
                            ->where('id', '=', $key)
                    ...
                    if ($result['url'])
                    {
                        ...
                        $result1 = DB::update('celebrity_products')->set($data)->where('id', '=', $key)->execute();
                        if ($result1)
                            $success++;
                    }
                }
        ...
        $id = $this->request->param('id');
        $email = DB::select('email')->from('celebrits')->where('id', '=', $id)->execute('slave')->current();
        $sku = $_GET['sku'];
        ...
        $url = DB::select('id', 'url', 'show_date')->from('celebrity_products')
    ...
    public function action_upload()
    {
        ...
        while ($data = fgetcsv($handle))
        {
            ...
            try
            {
                ...
                $result = DB::select('id')->from('celebrits')->where('email', '=', $celebrity['email'])->execute('slave')->current();
                if ($result['id'])
                {
    ...
    public function action_flow_data()
    {
        ...
        if ($filters)
        {
            foreach ($filters->rules as $item)
            {
                if ($item->field == 'admin')
                {
                    $admin = DB::select('id')->from('users')->where('name', '=', $item->data)->execute('slave')->get('id');
                    $_filter_sql[] = "celebrits.admin=" . $admin;
                }
                elseif ($item->field == 'email')
                {
                    $celebrity_id = DB::select('id')->from('celebrits')->where('email', '=', $item->data)->execute('slave')->get('id');
...
//			$date_end += 28800;
    ...
    public function action_fee_add()
    {
        try
        {
            if ($_POST)
            {
                ...
                $admin = Session::instance()->get('user_id');
                $c_id = DB::select('id')->from('celebrits')->where('email', '=', $email)->and_where('admin', '=', $admin)->execute('slave')->get('id');
                if (!$c_id)
                {
    ...
    public function action_contacted_data()
    {
        ...
        foreach ($result as $data)
        {
            $celebrity_id = DB::select('id')->from('celebrits')->where('email', '=', $data['email'])->execute('slave')->get('id');
            $response['rows'][$i]['id'] = $data['id'];
            $response['rows'][$i]['cell'] = array(
                0,
                $data['id'],
    ...
    public function action_contacted_delete()
    {
        if ($_POST)
        {
            ...
            {
                $user_id = Session::instance()->get('user_id');
                $parent = DB::select('parent_id', 'role_id')->from('users')->where('id', '=', $user_id)->execute('slave')->current();
          ...
          //      $array = array();
                if ($data[0])
                {
                    ...
                    $email1 = $data[0];
                            ...
                            $updates = DB::delete('celebrits')->where('email', '=', $email1)->execute();
                            print_r($updates);
    ...
    public function action_import_basic()
    {
        ...
        while ($data = fgetcsv($handle))
        {
            ...
            else
            {
                ...
                $email = trim($data[0]);
                $celebrity_id = DB::select('id')->from('celebrits')->where('email', '=', $email)->execute()->get('id');
                if ($celebrity_id AND !empty($basic))
                {
                    ...
                    {
                        $result = DB::update('celebrits')->set($basic)->where('id', '=', $celebrity_id)->execute();
                        if($result)
                        {
                            $success .= $email . '<br>';
                            $amount ++;
