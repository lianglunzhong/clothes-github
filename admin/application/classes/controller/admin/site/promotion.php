...
class Controller_Admin_Site_Promotion extends Controller_Admin_Site
{
    ...
    public function action_coupon_list()
    {
        ...
        if ($email)
        {
            ...
            if ($customer_id)
            {
                $result = DB::select('coupon_id')->from('customer_coupons')->where('customer_id', '=', $customer_id)->execute('slave')->as_array();
                foreach ($result as $c)
                {
                    $coupons[] = $c['coupon_id'];
                }
...
//                        ->where('expired', '>', time())
            ...
            if ($coupon->saved() == true)
            {
                ...
                if ($emails)
                {
                    ...
                    foreach ($emailArr as $email)
                    {
                        ...
                        if ($customer_id)
                        {
                            $has = DB::select('id')->from('customer_coupons')
                                    ->where('site_id', '=', $this->site_id)
    ...
    public function action_coupon_del($id)
    {
        ...
        if ($coupon->delete())
        {
            DB::delete('customer_coupons')->where('coupon_id', '=', $id)->execute();
            message::set('删除折扣号成功');
        }
        else
        {
    ...
    public function action_coupon_sinup_del()
    {
        $signup15off = DB::select('id', 'code')
                ...
                ->and_where('limit', '=', 0)
                ->execute('slave')->as_array();
        DB::delete('customer_coupons')->where('coupon_id', 'IN', $signup15off);
        DB::delete('coupons')
            ->where('code', 'LIKE', 'SIGNUP15OFF%')
            ->and_where('limit', '=', 0)
            ->execute();
    ...
    public function action_new_relate()
    {
        if ($_POST)
        {
            $catalog_id = DB::select('id')->from('catalogs')->where('link', '=', 'new-product')->execute('slave')->get('id');
            $today = time();
            ...
            if($no_set)
                ...
                $sql = '';
            // $new_products = DB::query(Database::SELECT, 'SELECT id,sku FROM products WHERE visibility = 1 AND display_date >= ' . $_2Weekslater . ' AND display_date < ' . $today . $sql)->execute('slave');
            $new_products = DB::query(Database::SELECT, 'SELECT id,sku FROM products WHERE visibility = 1 AND display_date >= 1457336492 AND display_date < 1466058945')->execute('slave');
            $products = array();
            $productArr = array();
            $cache = Cache::instance('memcache');
            ...
            {  
                ...
                $product_catalogs = DB::query(Database::SELECT,"SELECT catalog_id FROM catalog_products WHERE product_id = ".$p['id'])->execute()->as_array('catalog_id','catalog_id');
    ...
    public function action_coupon_data()
    {
        ...
        $count = DB::query(Database::SELECT, 'SELECT count(o.id) AS count FROM customer_coupons o LEFT JOIN customers c ON o.customer_id = c.id 
        ...
        $result = DB::query(DATABASE::SELECT, 'SELECT o.id, o.customer_id, c.email FROM customer_coupons o LEFT JOIN customers c ON o.customer_id = c.id 
    ...
    public function action_coupon_customer_delete($id)
    {
        if ($id)
        {
            $coupon_id = DB::select('coupon_id')->from('customer_coupons')->where('id', '=', $id)->execute('slave')->get('coupon_id');
            $delete = DB::delete('customer_coupons')->where('id', '=', $id)->execute();
            if ($delete)
    ...
    public function action_coupon_orders()
    {
        if ($_POST)
        {
            ...
            if ($code)
            {
                ...
                $result = DB::select('ordernum', 'created', 'payment_status', 'amount', 'currency')->from('orders')->where('coupon_code', '=', $code)->order_by('created', 'desc')->execute('slave');
