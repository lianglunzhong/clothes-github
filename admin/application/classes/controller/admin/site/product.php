    ...
    public function action_list()
    {
        ...
        if( ! ($products_statistics = $cache->get($key)))
        {
            ...
            for ($i = 0; $i < 7; $i++)
            {
                ...
                $today_display_count = DB::query(Database::SELECT, "SELECT COUNT(id) AS count FROM `products`
                ...
                $today_nosale_count = DB::query(Database::SELECT, "SELECT COUNT(id) AS count FROM `products`
                                            WHERE status=0 AND site_id =" . $this->site_id . " AND updated >= " . $midnight . " AND updated < " . $next_day)
                    ->execute('slave')->get('count');

                $today_invisible_count = DB::query(Database::SELECT, "SELECT COUNT(id) AS count FROM `products`
                                            WHERE visibility=0 AND site_id =" . $this->site_id . " AND updated >= " . $midnight . " AND updated < " . $next_day)
                    ->execute('slave')->get('count');

                $visible_sale_count = DB::query(Database::SELECT, "SELECT COUNT(id) AS count FROM `products`
                                            WHERE status=1 AND visibility=1 AND site_id =" . $this->site_id . " AND updated < " . $next_day)
                    ->execute('slave')->get('count');

                $today_orderitem_count = DB::query(Database::SELECT, "SELECT IFNULL(sum(i.quantity),0) AS count FROM `order_items` i INNER JOIN `orders` o ON i.order_id=o.id
...
/*        echo '<pre>';
 ...
 foreach($row1 as $row){
    ...
    public function action_all_price(){
        $languages = Kohana::config('sites.'.$this->site_id.'.language');
        foreach($languages as $l){
            if($l === 'en')
                continue;
            DB::query(Database::UPDATE, "UPDATE products_$l t inner join products p on t.id=p.id SET t.visibility = p.visibility,t.status = p.status,t.price = p.price,t.stock = p.stock WHERE p.visibility!=t.visibility or t.status != p.status or t.price != p.price")->execute();
        }
        Message::set('同步小语种产品描述成功!');
        $this->request->redirect('/admin/site/product/list');
    }
    ...
    public function edit_simple_config($original_product)
    {
        ...
        {  
            ...
            $need_catalog = 0;
            if($data['visibility'] == 1 && $data['status'] == 1)
            {
                $need_catalog = 1;
            }
                ...
                */
                ...
                if($data['no_limit_stock'] == 0)
                {
                    ...
                    foreach ($stocks as $attr => $stock)
                    {
                        if ($stock !== '')
                        {
                            $has = DB::select('id')->from('product_stocks')->where('product_id', '=', $product_id)->where('attributes', '=', $attr)->where('site_id', '=', $this->site_id)->execute()->get('id');
                            if ($has)
                                DB::update('product_stocks')->set(array('stocks' => $stock))->where('id', '=', $has)->execute();
                            else
                            {
                                $array = array(
                                    'product_id' => $product_id,
                                ...
                                DB::insert('product_stocks', array_keys($array))->values($array)->execute();
                            }
                            $item_stocks = array();
                            $result = DB::select('attributes', 'stocks')->from('product_stocks')->where('product_id', '=', $product_id)->execute();
                            foreach($result as $r)
                            {
        ...
        $result = DB::select('attributes', 'stocks')->from('product_stocks')->where('product_id', '=', $original_product['id'])->execute();
        $product_stocks = array();
        foreach ($result as $val)
        {
            $product_stocks[$val['attributes']] = $val['stocks'];
        }
        $content_data['product_stocks'] = $product_stocks;
        //产品操作历史 zpz 20160125
...
//                                $data[$key] = iconv('gbk', 'utf-8', $val);
            ...
            else
            {
                ...
                foreach ($data as $key => $value)
                {
                    ...
                    {
                        $admin = DB::select('id')->from('users')->where('name', '=', $value)->execute('slave')->get('id');
...
//                    $data[$key] = iconv('gbk', 'utf-8', $val);
    ...
    public function action_products()
    {
        if ($_POST)
        {
            ...
            $userArr = DB::select('id', 'name')->from('users')->execute('slave')->as_array();
            $users = array();
    ...
    public function action_product_visible()
    {
        ...
        if ($_POST)
        {
            ...
            if ($result)
            {
                ...
                //set price 
                Product::bulk_pricecache('sku', $productArr, '', 1);
...
//                                }
                if ($item->field == 'admin')
                {
                    $user_id = DB::select('id')->from('users')->where('name', '=', $item->data)->execute('slave')->get('id');
    ...
    public function action_export_urlproduct()
    {
        ...
        else
        {
            ...
            foreach ($urlArr as $url)
            {
                ...
                if (strlen($url) < 20)
                    continue;
                $product = DB::select('id', 'sku', 'clicks', 'quick_clicks')->from('products')->where('taobao_url', '=', $url)->execute('slave')->as_array();
        ...
        if ($_POST)
        {
            ...
            if ($type == 1){
                ...
                $result = DB::query(Database::SELECT, 'SELECT A.*,B.name AS picker_name
                    ...
                    FROM `products` p LEFT JOIN sets s ON p.set_id=s.id LEFT JOIN users u ON p.admin=u.id 
...
';
    ...
    public function action_offline_export_sku()
    {
        if ($_POST)
        {
            ...
            $result = DB::select('sku')->from('products')->where('offline_factory', '=', $offline_factory)->execute('slave');
    ...
    public function action_stock_import()
    {
        ...
        while ($data = fgetcsv($handle))
        {
            try
            {
                ...
                $has = DB::select('id')->from('product_stocks')
                ...
                if ($has)
                {
                    $result = DB::update('product_stocks')->set(array('stocks' => $stocks))->where('id', '=', $has)->execute();
                }
                else
                {
                    $array = array(
...
//                        'admin' => $user_id
                    ...
                    $result = DB::insert('product_stocks', array_keys($array))->values($array)->execute();
                }
                if ($result)
                {
                    $item_stocks = array();
    ...
    public function action_attribute_outstock()
    {
        ...
        $result = DB::query(DATABASE::SELECT, 'SELECT DISTINCT p.id, p.sku, p.admin,p.source FROM product_stocks s 
            LEFT JOIN products p ON s.product_id = p.id WHERE p.stock = -1 AND p.visibility = 1 AND p.status = 1 AND s.stocks = 0')
            ->execute('slave')->as_array();

        foreach($result as $p)
        {
            $s_stock = DB::query(DATABASE::SELECT, 'SELECT SUM(stocks) as s_stock FROM product_stocks WHERE product_id = '.$p['id'])->execute('slave')->get('s_stock');
            if(!$s_stock)
            {
    ...
    public function action_status_visibility()
    {
        ...
        else
        {
            ...
            {
                $sku = trim($sku);
                $info = DB::query(DATABASE::SELECT,"SELECT `visibility`,`status` FROM `products` WHERE sku='".$sku."' limit 1")
                    ->execute('slave');
   ...
   //     $types = Kohana::config('promotion.types');
    ...
    public function action_importstore()
    {
        if ($_POST)
        {
            ...
            if (!$offline_factory)
                die('Input param error!');
            $result = DB::select('sku')->from('products')->where('store', '=', $sku)->execute('slave');
            echo $sku . ' SKUS:<br>';
            foreach ($result as $p)
            {
                echo $p['sku'] . '<br>';
   ...
   public function action_export_proname()
    {
       ...
       $skuArr = explode("\n", $skus);
        ...
        foreach($country as $v){
            ...
            foreach ($skuArr as $sku)
            {
                $info = DB::query(DATABASE::SELECT,"SELECT email,shipping_firstname,shipping_lastname,shipping_country,shipping_address,ordernum,shipping_state,shipping_city,shipping_phone,shipping_zip,ordernum FROM `orders` WHERE ordernum='".$sku."' limit 0,1")->execute('slave');
...
<input type="submit" value="Submit">
    ...
    public function action_from_id_export_products()
    {
        ...
        else
        {
            ...
            $info = DB::query(DATABASE::SELECT,"SELECT `id`,`sku` FROM `products` WHERE id in (".$id_sql.")")
                    ->execute('slave');
    ...
    public function action_visibility_update()
    {
        ...
        foreach($languages as $lang)
        {
            ...
            $result = DB::query(Database::SELECT, 'SELECT p.id, p.visibility AS visi, d.visibility AS s_visi FROM `products` p LEFT JOIN products_' . $lang . ' d ON p.id=d.id WHERE d.visibility != p.visibility')->execute('slave');
            foreach($result as $data)
            {
    ...
    public function action_draw_portion()
    {
        if($_POST)
        {
            ...
            if($draw_arr['coupon_id'] != '0'){
                $coupon_id = DB::query(Database::SELECT, 'select id from coupons where id = "'.$draw_arr['coupon_id'].'"')->execute()->current();
                if(!$coupon_id){
...
//                        ->set('times', $times)
    ...
    public function action_edittag()
    {
        ...
        if ($_POST)
        {
            ...
            if($lang){
               $result = DB::update('tags_'.$lang)->set($data)->where('id', '=', $id)->execute();
            }else{
               $result = DB::update('tags')->set($data)->where('id', '=', $id)->execute();
            }
        ...
        if($lang){
          $banner = DB::select()->from('tags_'.$lang)->where('id', '=', $id)->execute('slave')->current();  
        }else{
            $banner = DB::select()->from('tags')->where('id', '=', $id)->execute('slave')->current();  
        }
    ...
    public function action_addtag()
    {
        ...
        {   
            ...
            $data['position'] = $_POST['position'];
            if($lang){
               $result = DB::insert('tags_'.$lang, array_keys($data))->values($data)->execute();  
            }else{
               $result = DB::insert('tags', array_keys($data))->values($data)->execute(); 
            }
 ...
 $result = DB::query(DATABASE::SELECT, "select p.sku,ps.attributes,ps.stocks from product_stocks ps INNER JOIN products p on ps.`product_id` = p.id and p.stock = -1")->execute();   
    ...
    public function action_product_stock_status()
    {
        ...
        header('Content-Disposition: attachment; filename=product_stock.csv');  
        echo "\xEF\xBB\xBF" . "sku,attributes,stocks\n";   
        $result = DB::query(DATABASE::SELECT, "select product_id,attributes,stocks from product_stocks where isdisplay = 0 and stocks !=0 order by id desc")->execute('slave')->as_array(); 
    ...
    public function action_export_productstatus()
    {
        ...
        header('Content-Disposition: attachment; filename=product_stock.csv'); 
        echo "\xEF\xBB\xBF" . "sku,attributes,stocks\n";
         $result = DB::query(DATABASE::SELECT, "SELECT DISTINCT product_id, stocks, attributes FROM product_stocks WHERE stocks !=0")->execute('slave')->as_array(); 
    ...
    public function action_giftsku()
    {
        if($_POST)
        {
            $cache = Cache::instance('memcache');
            $sku = $cache->get('giftsku','');
            $skustr = '';
            if($sku)
            {
                foreach ($sku as $key => $value) 
                {
                    $skustr .= Product::instance($value)->get('sku').',';
                }

                $skustr = substr($skustr, 0,-1);                
            }

            echo json_encode($skustr);
            die;
        }
    }
...
/*            ignore_user_abort(true);
            ...
            foreach ($skusarr as $key => $value)
            {
                ...
                $stocks=0;
                if($product_id) {
                    $stocks = DB::query(DATABASE::SELECT, "select ifnull(sum(ps.stocks),0) as stocks_sum from product_stocks ps LEFT JOIN products p on ps.`product_id` = p.id  WHERE p.id=" . $product_id)->execute()->get('stocks_sum');
                }
...
';
    ...
    public function action_getstock()
    {
        $product_id = Arr::get($_GET, 'product_id', '');
        $stock = Product::instance($product_id)->get_stocks();
        echo '<pre>';
        print_r($stock);
        exit;
    }

    public function action_export_cost_once()
    {
        header('Content-Type: application/vnd.ms-excel');
        header('Content-Disposition: attachment; filename="product.csv"');
        
        echo "\xEF\xBB\xBF" . "sku,原售价,现售价,美金成本,RMB成本,重量\n";
        
        $sql='SELECT id FROM products WHERE visibility =1 AND status =1';

        $products = DB::query(DATABASE::SELECT, $sql)->execute('slave')->as_array();

        foreach ($products as $id)
        {
            $product_instance = Product::instance($id['id']);
            $sku = $product_instance->get('sku');
            $price = $product_instance->get('price');
            $n_price = $product_instance->price();
            $cost = $product_instance->get('cost'); 
            $total_cost = $product_instance->get('total_cost'); 
            $weight = $product_instance->get('weight'); 
