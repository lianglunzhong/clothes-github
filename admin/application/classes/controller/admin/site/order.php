...
class Controller_Admin_Site_Order extends Controller_Admin_Site
{
    ...
    {
        /*
          $orders = DB::select('id')->from('orders')->where('payment_status', '=', 'verify_pass')->and_where('verify_date', '=', Null)->execute('slave')->as_array();
          if (!empty($orders))
          {
          foreach ($orders as $order_id)
          {
          $verify_date = DB::select('created')->from('order_payments')->where('order_id', '=', $order_id['id'])->and_where('payment_status', '=', 'verify_pass')->execute('slave')->current();
          if ($verify_date)
          {
...
//                DB::query(Database::DELETE, 'DELETE FROM orders WHERE email IS NULL AND currency IS NULL')->execute();
        ...
        if( ! ($order_statistics = $cache->get($key)))
        {
            ...
            for ($i = 0; $i < 7; $i++)
            {
                ...
                $order_celebrity = DB::query(Database::SELECT, 'SELECT COUNT(orders.id) AS count FROM `orders`,celebrits 
                ...
                //手机订单数量统计
                $order_mobile = DB::query(Database::SELECT, 'SELECT COUNT(id) AS count FROM `orders`
...
//          $date_end += 28800;
        ...
        $result = DB::query(DATABASE::SELECT, 'SELECT COUNT(`orders`.`id`) AS num FROM `orders` WHERE `orders`.is_active=1 AND `email` IS NOT NULL AND ' . $where_clause)->execute('slave');
      
        $count = $result[0]['num'];
        array_push($arr[0],$count);
        array_push($arr[1],'100%');

        $result1 = DB::query(DATABASE::SELECT, 'SELECT COUNT(`orders`.`id`) AS num FROM `orders` WHERE `orders`.is_active=1 AND payment_status = "verify_pass" AND `email` IS NOT NULL AND ' . $where_clause)->execute('slave');
        $count1 = $result1[0]['num'];
        $c1 = round($count1/$count * 100,2);
        array_push($arr[0],$count1);
        array_push($arr[1],$c1.'%');

        $result2 = DB::query(DATABASE::SELECT, 'SELECT COUNT(`orders`.`id`) AS num FROM `orders` WHERE `orders`.is_active=1 AND payment_status in ("pending","partial_paid","success") AND `email` IS NOT NULL AND ' . $where_clause)->execute('slave');
        $count2 = $result2[0]['num'];
        $c2 = round($count2/$count * 100,2);
        array_push($arr[0],$count2);
        array_push($arr[1],$c2.'%');

        $result3 = DB::query(DATABASE::SELECT, 'SELECT COUNT(`orders`.`id`) AS num FROM `orders` WHERE `orders`.is_active=1 AND payment_status = "new" AND `email` IS NOT NULL AND ' . $where_clause)->execute('slave');
        $count3 = $result3[0]['num'];
        $c3 = round($count3/$count * 100,2);
        array_push($arr[0],$count3);
        array_push($arr[1],$c3.'%');

        $result4 = DB::query(DATABASE::SELECT, 'SELECT COUNT(`orders`.`id`) AS num FROM `orders` WHERE `orders`.is_active=1 AND payment_status = "failed" AND `email` IS NOT NULL AND ' . $where_clause)->execute('slave');
        $count4 = $result4[0]['num'];
        $c4 = round($count4/$count * 100,2);
        array_push($arr[0],$count4);
        array_push($arr[1],$c4.'%');

        $result5 = DB::query(DATABASE::SELECT, 'SELECT COUNT(`orders`.`id`) AS num FROM `orders` WHERE `orders`.is_active=1 AND payment_status = "cancel" AND `email` IS NOT NULL AND ' . $where_clause)->execute('slave');
    ...
    public function action_verifydown()
    {
        ...
        $result1 = DB::query(DATABASE::SELECT, 'SELECT COUNT(`orders`.`id`) AS num FROM `orders` WHERE `orders`.is_active=1 AND shipping_status = "new_s" AND `email` IS NOT NULL AND ' . $where_clause)->execute('slave');
        $count1 = $result1[0]['num'];
        $c1 = round($count1/$count * 100,2);
        array_push($arr[0],$count1);
        array_push($arr[1],$c1.'%');

        $result2 = DB::query(DATABASE::SELECT, 'SELECT COUNT(`orders`.`id`) AS num FROM `orders` WHERE `orders`.is_active=1 AND shipping_status ="processing" AND `email` IS NOT NULL AND ' . $where_clause)->execute('slave');
        $count2 = $result2[0]['num'];
        $c2 = round($count2/$count * 100,2);
        array_push($arr[0],$count2);
        array_push($arr[1],$c2.'%');

        $result3 = DB::query(DATABASE::SELECT, 'SELECT COUNT(`orders`.`id`) AS num FROM `orders` WHERE `orders`.is_active=1 AND shipping_status = "partial_shipped" AND `email` IS NOT NULL AND ' . $where_clause)->execute('slave');
        $count3 = $result3[0]['num'];
        $c3 = round($count3/$count * 100,2);
        array_push($arr[0],$count3);
        array_push($arr[1],$c3.'%');

        $result4 = DB::query(DATABASE::SELECT, 'SELECT COUNT(`orders`.`id`) AS num FROM `orders` WHERE `orders`.is_active=1 AND shipping_status = "shipped" AND `email` IS NOT NULL AND ' . $where_clause)->execute('slave');
        $count4 = $result4[0]['num'];
        $c4 = round($count4/$count * 100,2);
        array_push($arr[0],$count4);
        array_push($arr[1],$c4.'%');

        $result5 = DB::query(DATABASE::SELECT, 'SELECT COUNT(`orders`.`id`) AS num FROM `orders` WHERE `orders`.is_active=1 AND shipping_status = "delivered" AND `email` IS NOT NULL AND ' . $where_clause)->execute('slave');
        $count5 = $result5[0]['num'];
        $c5 = round($count5/$count * 100,2);
        array_push($arr[0],$count5);
        array_push($arr[1],$c5.'%');

        $result6 = DB::query(DATABASE::SELECT, 'SELECT COUNT(`orders`.`id`) AS num FROM `orders` WHERE `orders`.is_active=1 AND shipping_status = "pickup" AND `email` IS NOT NULL AND ' . $where_clause)->execute('slave');
    ...
    public static function geteffect($date,$date_end,$a)
    {
        ...
        $result1 = DB::query(DATABASE::SELECT, 'SELECT o.id,AVG(o.logistics_days) as av FROM `orders` o INNER JOIN order_shipments os ON o.id = os.order_id WHERE  o.logistics_days>0  AND o.shipping_status = "delivered"  AND ' . $filter . ' and ' . $where_clause)->execute('slave')->as_array();
...
GROUP BY o.customer_id")
    ...
    public function action_getdownrun(){
        ...
        echo "\xEF\xBB\xBF" . "Created,Email,Name,times,country,flag\n";  
        $result = DB::query(Database::SELECT, "SELECT o.customer_id, count( o.customer_id ) AS con, o.created,c.firstname,c.lastname, c.email,o.shipping_country,o.flag FROM orders o LEFT JOIN customers c ON o.customer_id = c.id WHERE o.amount >0 GROUP BY o.customer_id")->execute('slave'); 
    ...
    public function action_select_info_by_oids()
    {
            ...
            else
            {
                ...
                {
                    $oid = trim($oid);
                    $info = DB::query(DATABASE::SELECT,"SELECT `trans_id`,`currency`,`amount`,`created` FROM `order_payments` WHERE payment_status='success' AND order_id='".$oid."' limit 1")
                        ->execute('slave');
       ...
       if(empty($seval)){
        ...
        $id = $arr[0]['id'];
         ...
         $order_data = $order->basic();
		   ...
		   unset($order_data['id']);
		   $result = DB::insert('orders', array_keys($order_data))->values($order_data)->execute();
			if($result){
				$data['res'] = $result[0];
    ...
    public function action_address()
    {
        ...
        if ($v->check())
        {
            ...
            if(Arr::get($_POST, 'isdefault', 0))
            {
                ...
                $datas['is_default'] = 0;
                $update = DB::update('addresses')->set($datas)->where('customer_id', '=', $customer_id)->execute();
                ...
                $data['is_default'] = 1;   
                DB::insert('addresses', array_keys($data))->values($data)->execute();
            }
...
//                        $order->update_amount();
    ...
    {
        $id = (int) $this->request->param('id');
        $order_id = DB::select('order_id')->from('order_remarks')->where('id', '=', $id)->execute('slave')->get('order_id');
...
//                $sord = '';
        ...
        if ($filters)
        {
            foreach ($filters->rules as $item)
            {
                ...
                {
                    $user_id = DB::select('id')->from('users')->where('name', '=', $item_data)->execute('slave')->get('id');
        ...
        $result = DB::query(DATABASE::SELECT, 'SELECT COUNT(`orders`.`id`) AS num FROM `orders` WHERE `orders`.is_active=1 AND `email` IS NOT NULL AND `orders`.site_id=' . $this->site_id . $activesql . ' AND '
        ...
        $result = DB::query(DATABASE::SELECT, 'SELECT `orders`.* FROM `orders` WHERE `orders`.is_active=1 AND `email` IS NOT NULL AND  `orders`.site_id=' . $this->site_id . $activesql . ' AND '
...
//                                ->get('total');
            ...
            $admin_id = DB::select('admin')->from('celebrits')->where('customer_id', '=', $value['customer_id'])->execute('slave')->get('admin');
    ...
    public function action_export_data()
    {
        if($_POST)
        {
            ...
            $result = DB::query(DATABASE::SELECT, 'SELECT `orders`.* FROM `orders` WHERE `orders`.is_active=1 AND `email` IS NOT NULL AND  `orders`.site_id=' . $this->site_id . ' AND '
    ...
    public function action_balance_data()
    {
        ...
        $result = DB::query(DATABASE::SELECT, 'SELECT COUNT(`orders`.`id`) AS num FROM `orders` WHERE `orders`.site_id=' . $this->site_id . ' AND `orders`.`parent_id` IS NOT NULL AND '
                . $filter_sql)->execute('slave');
        $count = $result[0]['num'];
        $total_pages = 0;
        if ($count > 0)
        ...
        $result = DB::query(DATABASE::SELECT, 'SELECT `orders`.* FROM `orders` WHERE `orders`.site_id=' . $this->site_id . ' AND `orders`.`parent_id` IS NOT NULL AND '
                . $filter_sql . ' ORDER BY ' . $sidx . ' ' . $sord . ' LIMIT ' . $limit . ' OFFSET ' . $start)->execute('slave');
        $response = array();
        $response['page'] = $page;
        $response['total'] = $total_pages;
...
//                                }
                ...
                echo '"' . $attributes . ';",';
                $admin_id = DB::select('admin')->from('celebrits')->where('email', '=', $order->get('email'))->execute('slave')->get('admin');
                if ($admin_id)
...
//			$date_end += 28800;
        ...
        foreach ($ids as $id)
        {
            ...
            echo $email . ',';
            $admin = DB::select('admin')->from('celebrits')->where('email', '=', $email)->execute('slave')->get('admin');
...
//			$date_end += 28800;
    ...
    public function action_delete()
    {
        ...
        if ($order->get('id'))
        {
            ...
            DB::delete('orders')->where('id', '=', $id)->execute();
...
//                echo $ret ? 'success' : $error;
        if ($ret)
        {
            ...
            if($points > 0)
            {
                ...
                DB::update('point_payments')->set(array('note' => 'order_discard'))->where('order_id', '=', $id)->execute();
            }
    ...
    public function action_recover($id)
    {
        ...
        if($ret)
        {
            ...
            if($points > 0)
            {
                ...
                DB::update('point_payments')->set(array('note' => 'order_recover'))->where('order_id', '=', $id)->execute();
            }
        }
...
//                    else
    ...
    public function action_email_edit()
    {
        if ($_POST)
        {
            ...
            $customer_id = DB::select('id')->from('customers')->where('email', '=', $email)->execute('slave')->get('id');
            if (!$customer_id)
            ...
            foreach ($orders as $ordernum)
            {
                ...
                $update = DB::update('orders')->set(array('customer_id' => $customer_id, 'email' => $email))->where('id', '=', $order_id)->execute();
                if ($update)
                {
                    $success[] = $ordernum;
...
public function action_export_detail()
    {
        if ($_POST)
        {
            ...
            if ($type == 0)
            {
                ...
                $admins = array();
                $users = DB::select('id', 'name')->from('users')->where('active', '=', 1)->execute('slave');
                ...
                $where_clause = implode(' AND ', $conditions);
                $result = DB::query(DATABASE::SELECT, 'SELECT o.ordernum, o.created, o.verify_date, o.email, o.customer_id, o.amount, o.currency, o.amount_shipping, o.coupon_code, o.amount_coupon, o.points, o.amount_point, o.shipping_country,i.product_id, i.sku, i.attributes,i.quantity, i.price,o.payment_status,o.order_insurance,o.payment_method, p.set_id, p.price AS p_price, p.total_cost, p.weight, p.offline_picker,p.display_date FROM order_items i INNER JOIN orders o ON i.order_id=o.id INNER JOIN products p ON i.product_id = p.id WHERE ' . $where_clause . ' ORDER BY o.created')->execute('slave');

                header("Content-type:application/vnd.ms-excel; charset=UTF-8");
                header('Content-Disposition: attachment; filename="' . $file_name . '"');
                echo "\xEF\xBB\xBF" . "下单时间,付款状态,验证通过时间,订单号,是否红人单,邮箱,国家,Currency,订单总额,邮费,折扣号码,折扣号减免金额,使用积分,积分减免金额,购买产品sku,购买尺码,sku上架时间,选款人,所属set,Product Price,Price（销售价）,成本,重量,数量,运费险,支付方式\n";
                foreach ($result as $data)
                {
            ...
            else
            {
                ...
                $where_clause = implode(' AND ', $conditions);
                $result = DB::query(DATABASE::SELECT, 'SELECT o.ordernum, o.created, o.email, o.customer_id, o.amount, o.currency, o.amount_shipping, o.coupon_code, o.amount_coupon,o.payment_status, o.points, o.amount_point, o.ip,o.products,o.shipping_country,o.lang, o.payment_method,o.order_insurance,o.payment_method, o.erp_fee_line_id, p.created AS p_created
                    FROM order_payments p LEFT JOIN orders o ON p.order_id=o.id WHERE ' . $where_clause . ' ORDER BY p.created')
                        ->execute('slave');

                header("Content-type:application/vnd.ms-excel; charset=UTF-8");
                header('Content-Disposition: attachment; filename="' . $file_name . '"');
                echo "\xEF\xBB\xBF" . "下单时间,支付时间,订单号,是否红人单,邮箱,Currency,订单总额,邮费,折扣号码,折扣号减免金额,使用积分,积分减免金额,IP,产品总数,发货国家,语言,支付方式,是否手机单,运费险,支付方式\n";
                foreach ($result as $data)
                {
                    echo date('Y-m-d H:i:s', $data['created']) . ',';
    ...
    public function action_payment_add()
    {
        if($_POST)
        {
            ...
            if($data['order_id'] AND $data['trans_id'] AND $data['amount'])
            {
                ...
                $has_trans = DB::select('id')->from('order_payments')->where('site_id', '=', $this->site_id)->where('trans_id', '=', $data['trans_id'])->execute('slave')->get('id');
                if (!$has_trans)
                {
                    $insert = DB::insert('order_payments', array_keys($data))->values($data)->execute();
                    if ($insert)
                    {
    ...
    public function action_payment_delete()
    {
        ...
        if($id)
        {
            $data = DB::select()->from('order_payments')->where('id', '=', $id)->execute()->current();
            $delete = DB::delete('order_payments')->where('id', '=', $id)->execute();
            if($delete)
            {
                Order::instance($data['order_id'])->add_history(array(
    ...
    public function action_search_ordernum()
    {
        if ($_POST)
        {
            ...
            elseif($type == 'track')
            {
                ...
                $order_id = DB::select('order_id')->from('order_payments')->where('trans_id', '=', $track)->execute('slave')->get('order_id');
...
';
        if($_POST)
        {
            ...
            {
                $payemntData = DB::select('payment_status', 'payment_method')->from('orders')->where('ordernum', '=', trim($o))->execute('slave')->current();
            ...
            foreach ($orders as $key => $onum)
            {
                ...
                $orderData = DB::select()->from('orders')->where('ordernum', '=', $return_data['orderNo'])->execute('slave')->current();
                ...
                {
                    $update = DB::update('orders')->set(array('payment_status' => $return_status))->where('id', '=', $orderData['id'])->execute();
                    if($update)
                    {
    ...
    public function action_export_white_orders()
    {
        ...
        {
            $admin_id = DB::select('admin')->from('celebrits')->where('email', '=', $order['email'])->execute('slave')->get('admin');
            if ($admin_id){
                continue;
            }
            echo $order['ordernum'], ',';
    ...
    public function action_add_item2order(){
        if ($_POST){
            ...
            $error_msg = "";
            $product_info = DB::query(DATABASE::SELECT,"select * from `products` where `sku`='".trim($_POST['sku'])."'")
                ->execute('slave')->current();
            if($product_info){
                foreach ($ordernum_arr as $ordernum) {
                    //++++验证处理，禁止添加产品：order,  order_purchase
                    $order = DB::select()->from('orders')->where('ordernum', '=', $ordernum)->execute('slave')->current();
    ...
    public function action_ajax_item(){
        ...
        $data = array();
        $product = DB::select()->from('products')->where('sku', '=', $sku)->execute('slave')->current();
    ...
    public function action_ajax_stock(){
        ...
        $product = DB::select()->from('products')->where('sku', '=', $sku)->where('site_id', '=', $this->site_id)->execute('slave')->current();
    ...
    public function action_select_ordernum_by_tracking_code()
    {
        ...
        else
        {
            ...
            {
                $tcd = trim($tcd);
                $info = DB::query(DATABASE::SELECT,"SELECT ordernum FROM `order_shipments` WHERE tracking_code='".$tcd."' limit 1")
                    ->execute('slave');
    ...
    public function action_select_tracking_code_by_ordernum()
    {
        ...
        if($_POST)
        {
            ...
            else
            {
                ...
                {
                    $tcd = trim($tcd);
                    $info = DB::query(DATABASE::SELECT,"SELECT tracking_code,tracking_link FROM `order_shipments` WHERE ordernum='".$tcd."' order by id desc limit 1")
                        ->execute('slave');
            ...
            foreach ($order_row as $key => $v) 
            {
                ...
                if (!empty($return_data['RESPONSE']))
                {
                    ...
                    if($data['succeed'] ==  1){
                        ...
                        DB::update('orders')->set(array('payment_status' => 'success'))->where('id', '=', $v['id'])->execute();
...
'<form action="" method="post">
    ...
    {
        $order_row = DB::query(DATABASE::SELECT, 'SELECT id,ip,ordernum FROM `orders` WHERE payment_method = "SOFORT" and  payment_status in ("failed","pending")  ORDER BY id DESC LIMIT 0,30')->execute()->as_array();
    ...
    public function  action_export_wholesale(){
        if($_POST){
            ...
            FROM `orders` O LEFT JOIN customers C ON O.customer_id = C.id WHERE O.order_from='wholesale' AND O.`email` IS NOT NULL AND O.site_id=" . $this->site_id ." AND ".implode(" AND ",$sql))->execute('slave')->as_array();

            foreach ($result as $value)
            {
                $admin_id = DB::select('admin')->from('celebrits')->where('email', '=', $value['email'])->execute('slave')->get('admin');
    ...
    public function action_export_amount200(){
        if($_POST) {
            ...
            FROM `orders` O LEFT JOIN customers C ON O.customer_id = C.id WHERE ROUND((O.amount/O.rate),4)>=200 AND O.`email` IS NOT NULL AND O.site_id=" . $this->site_id ." AND ".implode(" AND ",$sql))->execute('slave')->as_array();
...
';  
    ...
    public function action_exportordergift()
    {
        $starttime = explode(' ',microtime());
        $from = strtotime('2015-09-01');
        $to = strtotime('2016-09-30');

        $to1 = strtotime('2016-10-01'); 

        #增加id判断 加快查询速度
        $result1 = DB::query(DATABASE::SELECT, "SELECT DISTINCT email,created from orders WHERE id > 644859 and created > ".$from . " and created < ".$to." and payment_status = 'verify_pass' and shipping_country = 'US' limit 0,5200")->execute('slave')->as_array();  
        $result2 = DB::query(DATABASE::SELECT, "SELECT DISTINCT email from orders WHERE created > ".$to1 . " and payment_status = 'verify_pass' and shipping_country = 'US'")->execute('slave')->as_array();  
        ...
        {
            if(!in_array($v['email'], $email2))
            {
                array_push($email1, $v['email']);
            }
        }  

        header('Content-type:text/html;charset=GBK');//设定编码，防止js输出乱码。
        header('Content-type:application/vnd.ms-excel');//输出的类型
        header('Content-Disposition: attachment; filename="customer.csv"'); //下载显示
        echo iconv("UTF-8", "GBK//IGNORE",'email'."\r\n");
...
/*        echo count($email1);
    ...
    public function action_allcreate()
    {
        if ($_POST)
        {
            $starttime = explode(' ',microtime());
    ...
    <h3>根据email批量创建订单</h3>
    <form method="post">
        <textarea name="emails" cols="20" rows="20"></textarea>
        <br />
    <input type="submit" value="submit">
    </form>    
';  
