...
class Controller_Admin_Site_News extends Controller_Admin_Site
{
    ...
    {
        $products = DB::select('id', 'link')->from('products')
            ->where('visibility', '=', 1)
            ->limit(10)
            ->execute();
    ...
    public function action_productcache()
    {
        $id = Arr::get($_GET, 'id', '');
        $key = '/12product_price1233/' . $id. '/0'; 
        $cache = Cache::instance('memcache');
        $cache_value = $cache->get($key);
        $price = Product::instance($id)->price();
        $product_catalogs = DB::query(Database::SELECT,"SELECT catalog_id FROM catalog_products WHERE product_id = ".$id)->execute()->as_array('catalog_id','catalog_id');
        $cache_key = 'product_catalogs1_' . $id;
        $cache_value1 = $cache->get($cache_key);
        echo '<pre>';
        print_r($cache_value);
        echo '<hr>';
        print_r($price);
        echo '<hr>';
        print_r($product_catalogs);
        echo '<hr>';
        print_r($cache_value1);
        die;
    }

    public function action_productconcache()
    {
        $id = Arr::get($_GET, 'id', '');
        $key = "/product1/".$id;
        $cache = Cache::instance('memcache');
